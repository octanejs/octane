...
export function useKeyPress(
	...
	const modifierPressed = useRef(false, subSlot(slot, 'modifier'));

	// we need to remember the pressed keys in order to support combinations
	const pressedKeys = useRef<PressedKeys>(new Set([]), subSlot(slot, 'keys'));
	...
	const [keyCodes, keysToWatch] = useMemo<[Array<Keys>, Keys]>(
		function parseKeyCodes() {
			if (keyCode !== null) {
				const keyCodeArr = Array.isArray(keyCode) ? keyCode : [keyCode];
				const keys = keyCodeArr
					.filter(function isStringKeyCode(kc) {
					 ...
					 */
					.map(function splitKeyCombination(kc) {
						return kc.replace('+', '\n').replace('\n\n', '\n+').split('\n');
					});
				const keysFlat = keys.reduce(function flattenKeys(res: Keys, item) {
	...
	useEffect(
		function listenForKeyPress() {
			...
			if (keyCode !== null) {
				const downHandler = function downHandler(event: KeyboardEvent) {
				...
				const upHandler = function upHandler(event: KeyboardEvent) {
					const keyOrCode = useKeyOrCode(event.code, keysToWatch);
				...
				const resetHandler = function resetHandler() {
					pressedKeys.current.clear();
					setKeyPressed(false);
				};
...
function isMatchingKey(keyCodes: Array<Keys>, pressedKeys: PressedKeys, isUp: boolean): boolean {
	return (
		keyCodes
			 ...
			 */
			.filter(function filterBySize(keys) {
			 ...
			 */
			.some(function someMatchingKey(keys) {
				return keys.every(function hasKey(k) {
