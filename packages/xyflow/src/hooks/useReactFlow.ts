 ...
 *```
 */
export function useReactFlow<NodeType extends Node = Node, EdgeType extends Edge = Edge>(
	...rest: [slot?: symbol]
				...
				setEdges((prevEdges) =>
					prevEdges.map((edge) => {
						if (edge.id === id) {
							...
							return options.replace && isEdge(nextEdge)
								? (nextEdge as EdgeType)
								: { ...edge, ...nextEdge };
						}
				...
				getIntersectingNodes: (nodeOrRect, partially = true, nodes) => {
					...
					return (nodes || store.getState().nodes).filter((n) => {
						...
						if (
							internalNode &&
							!isRect &&
							(n.id === nodeOrRect.id || !internalNode.internals.positionAbsolute)
						) {
							return false;
						}
