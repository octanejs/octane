...
import { VersioningExtension, VersionSnapshot } from '@blocknote/core/extensions';
 ...
 */
export function useVersionUsers<U extends User = User>(userIds: string[]): Map<string, U> {
 ...
 */
export function useSnapshotLabel(snapshot: VersionSnapshot | undefined): string | undefined {
	const byIds =
		snapshot?.by === undefined ? [] : Array.isArray(snapshot.by) ? snapshot.by : [snapshot.by];
