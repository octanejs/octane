import { offset, useFloating, useTransitionStyles } from '@octanejs/floating-ui';
...
function getBoundingClientRectCell(referencePosCell: DOMRect | null) {
	return new DOMRect(referencePosCell!.x, referencePosCell!.y, referencePosCell!.width, 0);
}
...
function useTableHandlePosition(
	...
	const { refs, update, context, floatingStyles } = useFloating({
		open: show,
		placement: orientation === 'row' ? 'left' : orientation === 'col' ? 'top' : 'bottom-end',
		middleware: [
			offset(
				orientation === 'row' ? -10 : orientation === 'col' ? -12 : { mainAxis: 1, crossAxis: -1 },
			),
		],
	});
