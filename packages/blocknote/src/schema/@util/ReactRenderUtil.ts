...
import { Root, createRoot } from 'octane';

export function renderToDOMSpec(
	fc: (refCB: (ref: HTMLElement | null) => void) => React.OctaneNode,
	...
	if (editor?.elementRenderer) {
		...
		if (!editor?.headless) {
			throw new Error('elementRenderer not available, expected headless editor');
		}
		root = createRoot(div);
