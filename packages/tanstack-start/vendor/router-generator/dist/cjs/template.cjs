...
function fillTemplate(config, template, values, formatter) {
	...
	return formatter ? formatter(replaced) : require_utils.format(replaced, config);
}
	...
	switch (target) {
		...
		case "octane": return {
			...
			rootRoute: {
				...
				imports: {
					tsrImports: () => "import { Outlet, createRootRoute } from '@tanstack/octane-router';",
					tsrExportStart: () => "export const Route = createRootRoute(",
					tsrExportEnd: () => ");"
				}
			},
			route: {
				...
				imports: {
					tsrImports: () => "import { createFileRoute } from '@tanstack/octane-router';",
					tsrExportStart: (routePath) => `export const Route = createFileRoute(${serializeRoutePath(routePath)})(`,
					tsrExportEnd: () => ");"
				}
			},
			lazyRoute: {
				...
				imports: {
					tsrImports: () => "import { createLazyFileRoute } from '@tanstack/octane-router';",
					tsrExportStart: (routePath) => `export const Route = createLazyFileRoute(${serializeRoutePath(routePath)})(`,
					tsrExportEnd: () => ");"
				}
			}
