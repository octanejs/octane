...
function isStaticMember(node, objectName, propertyName) {
	return t.isMemberExpression(node) && !node.computed && t.isIdentifier(node.object, { name: objectName }) && t.isIdentifier(node.property, { name: propertyName });
}
function isOctaneSingleRootStamp(statement, componentName) {
	if (!t.isExpressionStatement(statement)) return false;
	const expression = statement.expression;
	return t.isAssignmentExpression(expression, { operator: "=" }) && isStaticMember(expression.left, componentName, "$$singleRoot") && t.isBooleanLiteral(expression.right, { value: true });
}
function isOctaneWarmPlan(statement, componentName) {
	if (!t.isExpressionStatement(statement)) return false;
	const expression = statement.expression;
	return t.isAssignmentExpression(expression, { operator: "=" }) && isStaticMember(expression.left, componentName, "__warm") && t.isArrowFunctionExpression(expression.right) && !expression.right.async && expression.right.params.length === 1 && t.isIdentifier(expression.right.params[0], { name: "__wp" });
}
function isOctaneLocationStamp(statement, componentName) {
	if (!t.isTryStatement(statement) || statement.finalizer || !statement.handler || statement.handler.param || statement.block.body.length !== 1 || statement.handler.body.body.length !== 0 || !statement.handler.body.innerComments?.some((comment) => comment.value.trim() === "frozen component")) return false;
	...
	if (!t.isAssignmentExpression(expression, { operator: "=" }) || !isStaticMember(expression.left, componentName, "__oct_loc")) return false;
	return t.isStringLiteral(expression.right) && /:\d+:\d+$/.test(expression.right.value) || t.isMemberExpression(expression.right) && !expression.right.computed && t.isIdentifier(expression.right.property, { name: "__oct_loc" });
}
...
//# sourceMappingURL=octane-split-route-components.js.map