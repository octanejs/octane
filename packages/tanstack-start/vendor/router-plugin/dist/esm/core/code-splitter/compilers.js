...
function getRouteHmrSignatureSource(ast, filename) {
	...
	for (const node of signatureAst.program.body) collectLocalBindingsFromStatement(node, localBindings);
	...
	signatureAst.program.body = signatureAst.program.body.filter((statement) => !t.isImportDeclaration(statement));
...
function compileCodeSplitReferenceRoute(opts) {
	...
	if (hmrAdded) {
		...
		ast.program.body.push(...createRouteHmrStatement(stableRouteOptionKeys, {
