...
> extends BaseRouteApi<TId, TRouter> {
  ...
  constructor({ id }: { id: TId }) {
    ...
    this.useNavigate = ((...args: Array<any>) => {
      ...
      return internalHooks.useNavigate(
        { from: (router.routesById as Record<string, any>)[this.id as string].fullPath },
        subSlot(slot, 'r:n'),
      )
    }) as typeof this.useNavigate
    this.Link = ((props: Record<string, unknown>) => {
      const router = useRouter()
      const fullPath = (router.routesById as Record<string, any>)[this.id as string].fullPath
