// Generated from lucide-react@1.45.0 and @lucide/icons@1.45.0.
// Run `pnpm lucide:generate`; do not edit by hand.

import iconData from '@lucide/icons/icons/receipt-turkish-lira';
import createLucideIcon from '../createLucideIcon';

const ReceiptTurkishLira = createLucideIcon(iconData);

export { default as __iconData } from '@lucide/icons/icons/receipt-turkish-lira';
/** @deprecated Use __iconData.node. */
export const __iconNode = iconData.node;
export default ReceiptTurkishLira;
