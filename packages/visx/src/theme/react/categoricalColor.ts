...
export function normalizeCategoricalIndex(index: number, source: string) {
	if (!Number.isFinite(index)) {
		warn(
			`[@octanejs/visx/theme] ${source} received a non-finite categorical color index; using index 0.`,
		);
		return 0;
	}
	...
	if (integerIndex !== index) {
		warn(
			`[@octanejs/visx/theme] ${source} received a fractional categorical color index; truncating to ${integerIndex}.`,
		);
	}
