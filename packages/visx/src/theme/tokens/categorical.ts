...
export default function normalizeCategoricalColors(colors: readonly string[]): readonly string[] {
	if (colors.length < MIN_CATEGORICAL_COLOR_COUNT) {
		warn(
			`[@octanejs/visx/theme] categorical palettes should include at least ${MIN_CATEGORICAL_COLOR_COUNT} colors; received ${colors.length}. The palette will be normalized to fill ${CATEGORICAL_COLOR_COUNT} runtime colors.`,
		);
	}
