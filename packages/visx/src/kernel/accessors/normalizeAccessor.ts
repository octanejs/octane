...
export default function normalizeAccessor<D, V>(accessor: AccessorInput<D, V>): Accessor<D, V> {
	...
	if (typeof input === 'symbol') {
		throw new TypeError('@octanejs/visx/kernel: symbol accessors are not supported in v1.');
	}

	if (typeof input !== 'function') {
		throw new TypeError('@octanejs/visx/kernel: accessors must be a string key or function.');
	}
