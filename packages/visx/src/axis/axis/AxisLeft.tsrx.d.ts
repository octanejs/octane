// Generated by scripts/generate-tsrx-types.mjs.
import type { SharedAxisProps, AxisScale } from '../types';
export declare const leftTickLabelProps: {
    readonly dx: "-0.25em";
    readonly dy: "0.25em";
    readonly fill: "#222";
    readonly fontFamily: "Arial";
    readonly fontSize: 10;
    readonly textAnchor: "end";
};
export default function AxisLeft<Scale extends AxisScale>({ axisClassName, labelOffset, tickLength, tickLabelProps, ...restProps }: SharedAxisProps<Scale>): import("react").JSX.Element;
