...
/**
 * Verifies every vendored @xstate/store-react upstream byte against
 * upstream/SHA256SUMS and confirms the required React test artifacts are present.
 */
export function verifyXstateStoreUpstream(root = packageRoot) {
	const upstream = join(root, 'upstream');
	const sums = join(upstream, 'SHA256SUMS');
	if (!existsSync(sums)) throw new Error('Missing packages/xstate-store/upstream/SHA256SUMS');
	...
	if (JSON.stringify(actualFiles) !== JSON.stringify(expectedFiles)) {
		throw new Error('Vendored XState Store React file inventory differs from upstream/SHA256SUMS');
	}
...
if (isMain) {
	const result = verifyXstateStoreUpstream();
	console.log(
		`XState Store React upstream evidence is current (${result.files} byte-exact files).`,
	);
}
