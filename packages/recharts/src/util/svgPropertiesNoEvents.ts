...
import type { OctaneElement } from './OctaneTypes';
 ...
 * - An Octane element, in which case it returns the props of the element filtered to only SVG properties
 * - Anything else, in which case it returns null
 *
 * Known prop bags retain their property types. Unknown input keeps the broad
 * return type because arbitrary element props may contain non-SVG values.
 *
 * @param input - The input to filter, which can be a record, an Octane element, or other types.
 * @returns A record of SVG properties if the input is a record or React element, otherwise null.
 */
type UnknownSvgProps = Partial<Record<SVGElementPropKeysType, unknown>> | null;
export type SVGPropsFromUnknown<T> = unknown extends T
	? UnknownSvgProps
	: T extends string | number | boolean | bigint | symbol | null | undefined
		? null
		: T extends OctaneElement<infer P>
			? SVGPropsNoEvents<P>
...
export function svgPropertiesNoEventsFromUnknown<T>(input: T): SVGPropsFromUnknown<T>;
export function svgPropertiesNoEventsFromUnknown(input: unknown): UnknownSvgProps {
	if (input == null) {
		return null;
	}
