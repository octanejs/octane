/**
 * Vitest globalSetup for the `recharts` project — the recharts analogue of
 * octane's differential precompile. Compiles every `.tsrx` fixture under
 * packages/recharts/tests/_fixtures through `@tsrx/react` + esbuild and writes
 * the React-runtime JS into THIS package's cache, rewriting
 * `@octanejs/recharts` → `recharts` (and `octane` → `react`) so the React side
 * runs the real recharts (the byte-for-byte SVG oracle).
 */
...
function compileOne(srcPath: string): void {
	...
	const rewritten = transformed.code
		.replace(
			/from\s+["']@octanejs\/recharts(\/[^"']*)?["']/g,
			(_m, sub) => `from "recharts${sub || ''}"`,
		)
