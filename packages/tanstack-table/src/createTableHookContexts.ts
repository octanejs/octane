// Scoped table/cell/header contexts.
//
// Plain TypeScript: `createContext` is not a hook, and `useContext` is exempt
// from octane's call-site slot keying (it is context-identity keyed), so this
// module needs neither compilation to `.tsrx` nor a sidecar.
import { createContext, useContext } from 'octane';
import type { Cell, CellData, Header, RowData, TableFeatures } from '@tanstack/table-core';
import type { Context } from 'octane';
import type { OctaneTable, TableHookContexts } from './types';

/**
 * Creates a fresh, scoped set of table/cell/header contexts (plus matching
 * context hooks) that you can pass into `createTableHook`. This mirrors
...
>(): TableHookContexts<TFeatures, TData> {
	 ...
	 * scoped contexts.
	 */
	function useCellContext<TValue extends CellData = CellData>(): Cell<TFeatures, any, TValue> {
	 ...
	 */
	function useHeaderContext<TValue extends CellData = CellData>(): Header<TFeatures, any, TValue> {
