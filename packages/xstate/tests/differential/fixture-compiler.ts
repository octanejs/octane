...
export function compileFixture(
	...
	// `xstate` is deliberately NOT rewritten: both sides drive the identical
	// pinned actor core, so any output difference is attributable to the binding.
	const rewritten = transformed.code
		.replace(
			/from\s+["']@octanejs\/xstate(\/[^"']*)?["']/g,
			(_match, subpath) => `from "@xstate/react${subpath || ''}"`,
		)
