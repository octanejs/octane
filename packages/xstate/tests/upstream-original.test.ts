...
import { runPristineUpstreamSuite } from '../../../scripts/react-parity/xstate-pristine-runtime.mjs';

const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), '../../..');

type Identity = { file: string; fullName: string; status?: string };

// @parity-case pristine:xstate-original-suite
it('runs the pinned @xstate/react 6.1.0 suite unchanged', function runsPinnedXstateSuite() {
	const expected = JSON.parse(
		readFileSync(resolve(repoRoot, 'packages/xstate/audit/pristine-runtime.json'), 'utf8'),
	).tests as Identity[];
