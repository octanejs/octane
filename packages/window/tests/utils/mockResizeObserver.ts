...
export function setDefaultElementSize({ height, width }: { height: number; width: number }) {
...
class MockResizeObserver implements ResizeObserver {
	...
	#notify(elements: HTMLElement[]) {
		...
		const entries = elements.map((element) => {
			...
			let contentRect: DOMRectReadOnly = elementToDOMRect.get(element) ?? defaultDomRect;
