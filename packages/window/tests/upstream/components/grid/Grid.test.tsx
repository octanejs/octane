...
import { act, useEffect, useLayoutEffect, useRef } from 'octane';
...
import type { CellComponentProps, GridImperativeAPI } from '../../../../src/components/grid/types';
import { useGridCallbackRef } from '../../../../src/components/grid/useGridCallbackRef';
...
describe('Grid', () => {
	...
	describe('custom keys', () => {
		...
		test('preserves state when list order changes', async () => {
			...
			const columnKey = vi.fn(({ columnIndex, rowIndex }) => data[rowIndex][columnIndex]);
			...
			function LocalCellComponent({
				...
				useEffect(() => {
					if (idDuringMountRef.current === id) {
						renderLog.push(`row: ${rowIndex}, column: ${columnIndex}, id: ${id}`);
						return;
					}
