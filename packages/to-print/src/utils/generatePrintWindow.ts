...
export function generatePrintWindow(
	...
	if (printIframeProps) {
		if (printIframeProps.allow) {
			// "" is not a meaningful `allow` value
			printWindow.allow = printIframeProps.allow;
		}

		if (printIframeProps.referrerPolicy !== undefined) {
			// Ensure "" is valid, which applies the browser's default referrer policy
			printWindow.referrerPolicy = printIframeProps.referrerPolicy;
		}

		if (printIframeProps.sandbox !== undefined) {
			// Ensure "" is valid, which applies all sandbox rules
