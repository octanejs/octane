...
import { normalizeProtocolRelativeUrl, PROTOCOL_RELATIVE_URL_REGEX } from './url';
...
export function createRouter(init: RouterInit): Router {
	...
	let dataRoutes = new DataRoutes(
		convertRoutesToDataRoutes(init.routes, mapRouteProperties, undefined, manifest),
	);
	...
	let pendingPopstateNavigationDfd: ReturnType<typeof createDeferred<void>> | null = null;
	...
	let pendingRevalidationDfd: ReturnType<typeof createDeferred<void>> | null = null;
	...
	async function startNavigation(
		...
		opts?: {
			...
			overrideNavigation?: Omit<NavigationStates['Loading'], 'matches' | 'historyAction'>;
		...
		let matches =
			opts?.initialHydration && state.matches && state.matches.length > 0 && !initialMatchesIsFOW
				? // `matchRoutes()` has already been called if we're in here via `router.initialize()`
					state.matches
				: matchRoutesImpl(routesToUse, location, basename, false, dataRoutes.branches);
		let flushSync = (opts && opts.flushSync) === true;
	...
	async function handleLoaders(
		...
		let loadingNavigation =
			overrideNavigation || getLoadingNavigation(location, matches, historyAction, submission);
		...
		let shouldUpdateFetchers =
			didUpdateFetcherRedirects || didAbortFetchLoads || revalidatingFetchers.length > 0;
	...
	function getUpdatedRevalidatingFetchers(revalidatingFetchers: RevalidatingFetcher[]) {
		let workingFetchers = new Map(state.fetchers);
		revalidatingFetchers.forEach((rf) => {
	...
	function abortStaleFetchLoads(landedId: number, fetchers: Map<string, Fetcher>): boolean {
		let yeetedKeys = [];
	...
	function saveScrollPosition(location: Location, matches: DataRouteMatch[]): void {
	...
	function getSavedScrollPosition(location: Location, matches: DataRouteMatch[]): number | null {
	...
	function _internalSetRoutes(newRoutes: DataRouteObject[]) {
		...
		dataRoutes.setHmrRoutes(
			convertRoutesToDataRoutes(newRoutes, mapRouteProperties, undefined, manifest),
		);
	}
	...
	if (init.instrumentations) {
		router = instrumentClientSideRouter(
			router,
			init.instrumentations.map((i) => i.router).filter(Boolean) as InstrumentRouterFunction[],
		);
	}
...
export function createStaticHandler(
					...
					async () => {
						let res = await generateMiddlewareResponse(
							async (
								...
								opts: {
									filterMatchesToLoad?: ((match: DataRouteMatch) => boolean) | undefined;
								} = {},
							) => {
	...
	async function loadRouteData(
		...
		Omit<StaticHandlerContext, 'location' | 'basename' | 'actionData' | 'actionHeaders'> | Response
	> {
...
function throwStaticHandlerAbortedError(request: Request, isRouteRequest: boolean) {
...
function isSubmissionNavigation(
	...
	return (
		opts != null &&
		(('formData' in opts && opts.formData != null) || ('body' in opts && opts.body !== undefined))
	);
}
...
function isNewRouteInstance(currentMatch: DataRouteMatch, match: DataRouteMatch) {
...
function shouldRevalidateLoader(loaderMatch: DataRouteMatch, arg: ShouldRevalidateFunctionArgs) {
...
function isSameRoute(newRoute: RouteObject, existingRoute: RouteObject): boolean {
...
function runServerMiddlewarePipeline(
	args: (LoaderFunctionArgs<RouterContextProvider> | ActionFunctionArgs<RouterContextProvider>) & {
...
function runClientMiddlewarePipeline(
	args: Omit<DataStrategyFunctionArgs<RouterContextProvider>, 'fetcherKey' | 'runClientMiddleware'>,
...
function normalizeRedirectLocation(
	...
	if (isAbsoluteUrl(location)) {
		...
		let url = PROTOCOL_RELATIVE_URL_REGEX.test(normalizedLocation)
			? new URL(normalizeProtocolRelativeUrl(normalizedLocation, currentUrl.protocol))
...
function findNearestBoundary(matches: DataRouteMatch[], routeId?: string): DataRouteMatch {
...
function dataWithResponseInitToResponse<D>(data: DataWithResponseInit<D>): Response {
	return Response.json(data.data, data.init ?? undefined);
}

function dataWithResponseInitToErrorResponse<D>(data: DataWithResponseInit<D>): ErrorResponseImpl {
...
function isDataStrategyResults(result: unknown): result is Record<string, DataStrategyResult> {
	return (
		result != null &&
...
export function isDataWithResponseInit(value: any): value is DataWithResponseInit<unknown> {
	return (
		typeof value === 'object' &&
		value != null &&
		'type' in value &&
