/**
 * Vitest globalSetup for the `remix-router` project — the router analogue of
 * octane's differential precompile. Runs ONCE in pure Node before any test
 * loads, compiles every `.tsrx` fixture under
 * `packages/remix-router/tests/_fixtures` through `@tsrx/react` + esbuild,
 ...
 * rewrite `@octanejs/remix-router` → `react-router`, so the React
 * side of each fixture runs the REAL react-router adapter (the byte-for-byte
...
// THIS package's deps (react-router, react, react-dom). The differential
...
function compileOne(srcPath: string): void {
	...
	// `@octanejs/remix-router` (and any subpath) → the matching real
	// react-router specifier; `octane` → react.
	let rewritten = transformed.code
		.replace(
			/from\s+["']@octanejs\/remix-router(\/[^"']*)?["']/g,
			(_m, sub) => `from "react-router${sub || ''}"`,
		)
