...
import { replace } from '../../src/lib/router/utils';
...
describe('redirects', () => {
	...
	it('supports relative routing in redirects (from child navigation loader)', async () => {
		...
		await nav1.loaders.index.resolve('INDEX');
		let nav2 = await nav1.loaders.child.redirectReturn('..', undefined, undefined, ['parent']);
	...
	it('supports relative routing in redirects (from index navigation loader)', async () => {
		...
		let nav2 = await nav1.loaders.index.redirectReturn('..', undefined, undefined, ['parent']);
		await nav2.loaders.parent.resolve('PARENT 2');
		expect(t.router.state).toMatchObject({
			location: {
				pathname: '/parent',
	...
	it('supports relative routing in redirects (from parent fetch loader)', async () => {
		...
		await fetch.loaders.parent.redirectReturn('..', undefined, undefined, ['parent']);
	...
	it('supports relative routing in redirects (from child fetch loader)', async () => {
		...
		let nav = await fetch.loaders.child.redirectReturn('..', undefined, undefined, ['parent']);
	...
	it('supports relative routing in redirects (from index fetch loader)', async () => {
		...
		let nav = await fetch.loaders.index.redirectReturn('..', undefined, undefined, ['parent']);
	...
	it('supports . redirects', async () => {
		...
		let nav2 = await nav1.loaders.parent.redirectReturn('./child', undefined, undefined, [
			'parent',
			'child',
			'index',
		]);
	...
	it('supports relative routing in navigation action redirects', async () => {
		...
		let nav2 = await nav1.actions.child.redirectReturn('..', undefined, undefined, ['parent']);
		await nav2.loaders.parent.resolve('PARENT');
		expect(t.router.state).toMatchObject({
			location: {
				pathname: '/parent',
	...
	it('preserves query and hash in redirects', async () => {
		...
		let nav2 = await nav1.actions.child.redirectReturn('/parent?key=value#hash');
		await nav2.loaders.parent.resolve('PARENT');
		expect(t.router.state).toMatchObject({
			location: {
				pathname: '/parent',
	...
	it('preserves query and hash in relative redirects', async () => {
		...
		let nav2 = await nav1.actions.child.redirectReturn('..?key=value#hash', undefined, undefined, [
			'parent',
		]);
		await nav2.loaders.parent.resolve('PARENT');
		expect(t.router.state).toMatchObject({
	...
	it('properly handles same-origin absolute URLs', async () => {
		...
		let B = await A.actions.child.redirectReturn('http://localhost/parent', undefined, undefined, [
			'parent',
		]);
	...
	it('normalizes mixed leading separators in redirects', async () => {
		let locations = ['\\\\localhost/parent', '/\\localhost/parent', '\\/localhost/parent'];
		...
		for (let location of locations) {
			...
			let B = await A.actions.child.redirectReturn(location, undefined, undefined, ['parent']);
			await B.loaders.parent.resolve('PARENT');
			expect(t.router.state.location).toMatchObject({
				hash: '',
				pathname: '/parent',
	...
	it('automatically replaces in the history stack if you redirect to the same location (absolute)', async () => {
		...
		let C = await B.actions.parent.redirectReturn('http://localhost/parent', undefined, undefined, [
			'parent',
		]);
		await C.loaders.parent.resolve('PARENT*');
