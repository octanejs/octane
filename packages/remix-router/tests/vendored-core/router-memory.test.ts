// Ported from react-router@7.18.1 packages/react-router/__tests__/router/router-memory-test.ts — verbatim except: renamed to router-memory.test.ts, imports re-pointed at the vendored sources (../../lib/* → ../../src/lib/*), jest→vitest globals via ./_shim.
import './_shim';
/**
...
describe('a memory router', () => {
	...
	it('can create URLs without window', async () => {
		...
		router.navigate('/a');
		expect(shouldRevalidateSpy.mock.calls[0][0].currentUrl.toString()).toBe('http://localhost/');
		expect(shouldRevalidateSpy.mock.calls[0][0].nextUrl.toString()).toBe('http://localhost/a');
		router.dispose();
	});
