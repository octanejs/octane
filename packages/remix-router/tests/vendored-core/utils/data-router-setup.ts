...
import type {
	...
	RouterInit,
} from '../../../src/lib/router/router';
import type { DataRouteObject, RouteMatch } from '../../../src/lib/router/utils';
...
export function setup({ routes, initialEntries, initialIndex, ...routerInit }: SetupOpts) {
	let guid = 0;
		...
		return _routes.map((r) => {
			...
			if (r.loader) {
				enhancedRoute.loader = (...args) => {
					let navigationId =
						activeLoaderType === 'fetch' ? activeLoaderFetchId : activeLoaderNavigationId;
			...
			if (r.action) {
				enhancedRoute.action = (...args) => {
					...
					let navigationId =
						activeActionType === 'fetch' ? activeActionFetchId : activeActionNavigationId;
	...
	// Vendor-port note: upstream spread the jsdom global `window`; this port runs
	// under node, so fall back to a minimal window stub with the pieces the router
	// touches (document.createElement for the isBrowser check, pagehide listeners,
	// sessionStorage for view-transition persistence, location for redirects).
	let baseWindow: any =
	...
	function getRouteHelpers(
		...
		async function _redirect(
			...
			shims.forEach((routeId) => shimHelper(helpers.loaders, 'navigation', 'loader', routeId));
	...
	function getNavigationHelpers(href: string, navigationId: number): NavigationHelpers {
		invariant(currentRouter?.routes, 'No currentRouter.routes available in getNavigationHelpers');
		...
		let loaderHelpers = getHelpers(
			...
			navigationId,
			(routeId, helpers) =>
				activeHelpers.set(`navigation:${navigationId}:loader:${routeId}`, helpers),
		);
		let actionHelpers = getHelpers(
			(matches || []).filter((m) => m.route.action),
			navigationId,
			(routeId, helpers) =>
				activeHelpers.set(`navigation:${navigationId}:action:${routeId}`, helpers),
		);
	...
	function getFetcherHelpers(
		...
		invariant(currentRouter?.routes, 'No currentRouter.routes available in getFetcherHelpers');
		let matches = matchRoutes(inFlightRoutes || currentRouter.routes, href);
		invariant(currentRouter, 'No currentRouter available');
		let search = parsePath(href).search || '';
		let hasNakedIndexQuery = new URLSearchParams(search).getAll('index').some((v) => v === '');
		...
		let loaderHelpers = getHelpers(
			...
			navigationId,
			(routeId, helpers) => activeHelpers.set(`fetch:${navigationId}:loader:${routeId}`, helpers),
		);
		let actionHelpers = getHelpers(
			match.route.action ? [match] : [],
			navigationId,
			(routeId, helpers) => activeHelpers.set(`fetch:${navigationId}:action:${routeId}`, helpers),
		);
			...
			let promise = new Promise<void>((r) => {
				...
				let unsubscribe = currentRouter.subscribe(() => {
					...
					if (currentRouter?.basename) {
						...
						invariant(popHref, 'href passed to navigate should start with basename');
					}
	...
	// control/assert loader/actions
	async function fetch(href: string, opts?: RouterFetchOptions): Promise<FetcherHelpers>;
	...
	async function fetch(
		...
		let routeId =
			typeof routeIdOrOpts === 'string' ? routeIdOrOpts : String(currentRouter?.routes[0].id);
		opts =
