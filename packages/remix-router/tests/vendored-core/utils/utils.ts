...
export function isRedirect(result: any) {
	return result instanceof Response && result.status >= 300 && result.status <= 399;
}
...
export function findRouteById(routes: DataRouteObject[], id: string): DataRouteObject {
	let foundRoute: DataRouteObject | null = null;
