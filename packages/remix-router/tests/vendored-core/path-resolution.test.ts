// Ported from react-router@7.18.1 packages/react-router/__tests__/router/path-resolution-test.ts — verbatim except: renamed to path-resolution.test.ts, imports re-pointed at the vendored sources (../../lib/* → ../../src/lib/*), jest→vitest globals via ./_shim.
import './_shim';
import { createMemoryHistory, createPath } from '../../src/lib/router/history';
...
describe('path resolution', () => {
	...
	describe('routing to self', () => {
		...
		function assertRoutingToSelf(fooChildren, expectedPath, expectIndex) {
			...
			expect(createPath(router.state.location)).toBe(expectedPath + (expectIndex ? '?index' : ''));
			router.dispose();
