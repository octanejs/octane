// Ported from react-hook-form@7.81.0 src/__tests__/useWatch.test.tsx (jest → vitest, octane runtime).
import { beforeEach, describe, expect, it, test, vi } from 'vitest';
import { useState, useEffect } from 'octane';
import { fireEvent, render, renderHook, screen, waitFor, within } from '@octanejs/testing-library';

import { Controller } from '../../src/controller.tsrx';
import type { Control, UseFieldArrayReturn, UseFormRegister, UseFormReturn } from '../../src/types';
...
describe('useWatch', () => {
	...
	describe('when disabled prop is used', () => {
		it('should be able to disabled subscription and started with true', async () => {
			...
			const ChildComponent = ({ control }: { control: Control<FormValues> }) => {
	...
	describe('update', () => {
		...
		it('should not re-render useWatch when submit does not change watched values', async () => {
			...
			const FirstNameWatched = ({ control }: Pick<UseFormReturn<FormInputs>, 'control'>) => {
	...
	describe('reset', () => {
		...
		describe('with custom register', () => {
			it('should return default value of reset method when value is not empty', async () => {
				...
				expect((await screen.findByTestId('result')).textContent).toBe('default1');
			});
		...
		it('Should update the value immediately after reset when used with Controller', async () => {
			...
			fireEvent.input(screen.getByTestId('test-select'), {
				target: { value: 'test1' },
			});
	...
	describe('setValue', () => {
		...
		it('should watch nested object field update', () => {
			...
			expect((screen.getByRole('textbox') as HTMLInputElement).value).toEqual('333');
		});

		it('should return field value, not resolver transformed value', async () => {
			const Form = () => {
				const { control, setValue } = useForm<{ test: string }, any, { test: number }>({
