// Ported from react-hook-form@7.81.0 src/__tests__/logic/getDirtyFields.test.ts (jest → vitest, octane runtime).
import { describe, expect, it, test } from 'vitest';
import getDirtyFields from '../../../src/logic/getDirtyFields';
...
describe('getDirtyFields', () => {
	...
	it('should mark an array-valued registered field as dirty with a boolean rather than diffing elements (#13584)', () => {
		...
		expect(
			getDirtyFields({ fruits: ['apple'] }, { fruits: ['apple', 'banana'] }, undefined, fieldRefs),
		).toEqual({
			fruits: true,
		});

		expect(
			getDirtyFields({ fruits: ['apple'] }, { fruits: ['apple'] }, undefined, fieldRefs),
		).toEqual({});
	});
