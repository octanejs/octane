// Ported from react-hook-form@7.81.0 src/__tests__/logic/isNameInFieldArray.test.ts (jest → vitest, octane runtime).
import { describe, expect, it, test } from 'vitest';
import isNameInFieldArray from '../../../src/logic/isNameInFieldArray';
...
describe('isNameInFieldArray', () => {
	...
	it('should find match when field array is nested under a numeric path segment', () => {
		...
		expect(isNameInFieldArray(new Set(['steps.0.items']), 'steps.0.items.2.name')).toBeTruthy();
		expect(isNameInFieldArray(new Set(['steps.0.items']), 'steps.0.items.0.value')).toBeTruthy();
		// Deeper nesting: `a.0.b.1.items`
		expect(isNameInFieldArray(new Set(['a.0.b.1.items']), 'a.0.b.1.items.3.name')).toBeTruthy();
		// Should still return false when there's no actual match
		expect(isNameInFieldArray(new Set(['steps.0.other']), 'steps.0.items.2.name')).toBeFalsy();
