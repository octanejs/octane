// Ported from react-hook-form@7.81.0 src/__tests__/logic/appendErrors.test.ts (jest → vitest, octane runtime).
...
describe('appendErrors', () => {
	...
	it('should return error object when validateAllFieldCriteria is true', () => {
		...
		expect(
			appendErrors('test', true, errors, 'invalid_string', ['uppercase', 'lowercase', 'number']),
		).toEqual({
			message: 'test',
			type: 'required',
			types: {
