// Ported from react-hook-form@7.81.0 src/__tests__/logic/shouldRenderFormState.test.ts (jest → vitest, octane runtime).
import { beforeEach, describe, expect, it, vi } from 'vitest';
import shouldRenderFormState from '../../../src/logic/shouldRenderFormState';
import type { ReadFormState } from '../../../src/types';
...
describe('shouldRenderFormState', () => {
	...
	it('should return matched key when incoming state contains subscribed key among others', () => {
		...
		const result = shouldRenderFormState({ isValid: false, isDirty: true }, proxy, updateFormState);
	...
	it('should return true when changed state key is subscribed', () => {
		...
		const result = shouldRenderFormState({ isDirty: true }, proxy, updateFormState);
	...
	it('calls Object.keys on formState exactly once regardless of which branch is taken', () => {
		...
		const formStateKeyCalls = keysSpy.mock.calls.filter(
			(args) => args[0] != null && typeof args[0] === 'object' && 'isDirty' in (args[0] as object),
		);
	...
	describe('when root subscribe', () => {
		it('should return subscribed key name if expecting all', () => {
			...
			const result = shouldRenderFormState({ isDirty: true }, proxy, updateFormState, true);
