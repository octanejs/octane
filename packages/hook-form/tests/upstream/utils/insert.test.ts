// Ported from react-hook-form@7.81.0 src/__tests__/utils/insert.test.ts (jest → vitest, octane runtime).
...
describe('insert', () => {
	...
	it('should insert undefined as value when value to be inserted is falsy', () => {
		...
		expect(insert([1, 2, 4] as (boolean | number)[], 2, false)).toEqual([1, 2, false, 4]);
		expect(insert([1, 2, 4] as (number | string)[], 2, '')).toEqual([1, 2, '', 4]);
