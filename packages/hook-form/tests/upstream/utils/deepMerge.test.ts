// Ported from react-hook-form@7.81.0 src/__tests__/utils/deepMerge.test.ts (jest → vitest, octane runtime).
import { describe, expect, it } from 'vitest';
import deepMerge from '../../../src/utils/deepMerge';

describe('deepMerge', () => {
	it('should deep merge object correctly', () => {
		expect(deepMerge({ test: { value: 1, data: { test: 1 } } }, { test: { value2: 2 } })).toEqual({
	...
	it('should overwrite array value ', () => {
		expect(deepMerge({ test: [{ value: '2' }] }, { test: [{ value: '1' }] })).toEqual({
			test: [{ value: '1' }],
		});
	});
	...
	it('should not merge object with date type', () => {
		expect(deepMerge({ test: new Date() }, { test: new Date('1999-02-02') })).toEqual({
