// Ported from react-hook-form@7.81.0 src/__tests__/utils/deepEqual.test.ts (jest → vitest, octane runtime).
...
describe('deepEqual', () => {
	...
	it('should return true when comparing sparse array against plain object with numeric string keys (issue #13346)', () => {
		...
		expect(deepEqual({ items: sparseArray }, { items: plainObject })).toBeTruthy();
		expect(deepEqual({ items: plainObject }, { items: sparseArray })).toBeTruthy();
	});
	...
	it('should not treat different values as equal when one side reuses an object reference', () => {
		...
		expect(
			deepEqual({ first: { value: 1 }, second: { value: 2 } }, { first: shared, second: shared }),
		).toBeFalsy();

		expect(deepEqual([shared, shared], [{ value: 1 }, { value: 9 }])).toBeFalsy();

		expect(
			deepEqual({ first: shared, second: shared }, { first: { value: 1 }, second: { value: 1 } }),
		).toBeTruthy();
	});
	...
	it('should return true when comparing NaN values', () => {
		...
		expect(
			deepEqual({ user: { age: NaN, name: 'test' } }, { user: { age: NaN, name: 'test' } }),
		).toBeTruthy();

		// Mixed with other values
		expect(deepEqual({ a: NaN, b: 1, c: 'test' }, { a: NaN, b: 1, c: 'test' })).toBeTruthy();
	});
	...
	it('should compare empty non-plain objects by reference', () => {
		...
		expect(deepEqual(file, new File(['b'], 'b.jpg', { type: 'image/jpeg' }))).toBeFalsy();
		expect(deepEqual(new Blob(['a']), new Blob(['a']))).toBeFalsy();
		expect(deepEqual(new FormData(), new FormData())).toBeFalsy();
		expect(deepEqual(new Map([['test', '1']]), new Map([['test', '1']]))).toBeFalsy();
