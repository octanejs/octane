// Ported from react-hook-form@7.81.0 src/__tests__/useForm/setValue.test.tsx (jest → vitest, octane runtime).
...
// the condition is mutation-driven. The test that actually advances timers
// enables fake timers per-test and restores real timers at the end.
...
describe('setValue', () => {
	...
	it('should update nested controlled input', () => {
		...
		expect((screen.getByRole('textbox') as HTMLInputElement).value).toEqual('updateValue');
	});
	...
	it('should worked with nested array fields with object', () => {
		...
		act(() => result.current.setValue('test', [{ test: '1' }, { test: '2' }, { test: '3' }]));
	...
	it('should work with object fields', () => {
		...
		act(() => result.current.setValue('test', { bill: '1', luo: '2', test: '3' }));
	...
	describe('with dirty', () => {
		it.each(['isDirty', 'dirtyFields'])(
			...
			(property) => {
				...
				act(() => result.current.setValue('test', 'test', { shouldDirty: true }));
		...
		it.each(['isDirty', 'dirtyFields'])(
			...
			(property) => {
				...
				act(() => result.current.setValue('test', 'test', { shouldDirty: false }));
		...
		it.each(['isDirty', 'dirtyFields'])(
			...
			(property) => {
				...
				act(() => result.current.setValue('test', 'default', { shouldDirty: true }));
	...
	// a few microtasks after the body resolves; the re-render it schedules lands
	...
	it('should setValue with valueAs', async () => {
		...
		function App() {
			...
			return (
				...
				>
					<input type="date" {...register('setStringDate', { valueAsDate: true })} />
					<input type="submit" />
				</form>
			);
		}
	...
	it('should set field array correctly without affect the parent field array', async () => {
		...
		const Child = ({ control, index }: { control: Control<FormValues>; index: number }) => {
			useFieldArray({
				control,
		...
		const App = () => {
			...
			return (
				<div>
					...
					<button
						onClick={() => {
							setValue('test.0.nestedArray' as `test.0.nestedArray`, [{ name: 'append' }]);
						}}
					>
						setValue
					</button>
	...
	describe('when set field to null', () => {
		it('should be able to set correctly with register', () => {
			...
			function App() {
				...
				return (
					<div>
						<button onClick={() => setValue('user', null)}>Set user to null</button>
					</div>
				);
			}
	...
	it('should only be able to update value of object which is not registered', async () => {
		...
		expect(await screen.findByText('{"data":"2","data1":"2","data2":"3"}')).toBeVisible();
	});
