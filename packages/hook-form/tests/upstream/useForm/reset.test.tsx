// Ported from react-hook-form@7.81.0 src/__tests__/useForm/reset.test.tsx (jest → vitest, octane runtime).
...
import type { Control, FieldErrors, UseFormRegister, UseFormReturn } from '../../../src/types';
...
describe('reset', () => {
	...
	it('should set array value of multiple checkbox inputs correctly', async () => {
		...
		screen
			...
			.forEach((checkbox) => expect((checkbox as HTMLInputElement).checked).toBeTruthy());
	});
	...
	it('should keep defaultValues after reset with shouldKeepDefaultValues', async () => {
		...
		expect((screen.getAllByRole('textbox')[1] as HTMLInputElement).value).toEqual('test1');
	});
	...
	describe('when reset optional props set to keepDirtyValues', () => {
		describe('with uncontrolled components', () => {
			...
			it('should only update new reset values', async () => {
				...
				await waitFor(() =>
					expect((screen.getByPlaceholderText('First Name') as HTMLInputElement).value).toEqual(
						'bill',
					),
				);
			...
			it('should treat previously-undirty fields as dirty when keepDefaultValues is set', async () => {
				...
				fireEvent.click(screen.getByRole('button', { name: 'reset keep defaults' }));

				await waitFor(() =>
					expect((screen.getByPlaceholderText('Last Name') as HTMLInputElement).value).toEqual(
						'luo',
					),
				);
	...
	it('should allow resetting unmounted field array', () => {
		...
		const FieldArray = ({
			...
			return (
				<div>
					{fields.map((field, index) => {
						return <input key={field.id} {...register(`test.${index}.name` as const)} />;
					})}
					<button
						onClick={() => {
							append({ name: '' });
	...
	it('should update controlled input correctly with shouldUnregister set to true', () => {
		function App() {
			...
			return (
				<form>
					...
					<Controller
						render={({ field }) => (
							<input ref={field.ref} value={field.value} onInput={field.onInput} />
						)}
						name="control"
						control={control}
					/>
		...
		expect((screen.getAllByRole('textbox')[0] as HTMLInputElement).value).toEqual('uncontrolled');
		expect((screen.getAllByRole('textbox')[1] as HTMLInputElement).value).toEqual('control');
	});
	...
	it('should keep reset value for conditionally mounted controlled fields with shouldUnregister', async () => {
				...
				>
					<Controller
						...
						render={({ field: { onInput: onChange, name, ref, value } }) => (
							<input ref={ref} name={name} value={value || ''} onInput={onChange} />
						)}
					/>
					{showAge && (
						<Controller
		...
		await waitFor(() =>
			expect((screen.getAllByRole('textbox')[1] as HTMLInputElement).value).toBe('3'),
		);

		fireEvent.click(screen.getByRole('button', { name: 'submit' }));

		await waitFor(() => expect(submittedData).toEqual({ name: 'name', age: '3' }));
	});
	...
	it('should reset values but keep defaultValues', async () => {
		...
		expect(await screen.findByText('{"test":"test","test1":"test1"}')).toBeVisible();
		expect((screen.getAllByRole('textbox')[0] as HTMLInputElement).value).toEqual('changed1');
		expect((screen.getAllByRole('textbox')[1] as HTMLInputElement).value).toEqual('changed2');
	});
	...
	it('should reset the form after submitted', async () => {
		...
		expect(
			screen.getByText(`{"something":true,"test":[{"firstName":true,"lastName":true}]}`),
		).toBeVisible();
		...
		expect((screen.getAllByRole('textbox')[0] as HTMLInputElement).value).toEqual('1');
		expect((screen.getAllByRole('textbox')[1] as HTMLInputElement).value).toEqual('2');
		expect((screen.getAllByRole('textbox')[2] as HTMLInputElement).value).toEqual('3');
	});
	...
	it('should keep mounted value after reset with keep dirty values', async () => {
		function App() {
			...
			return (
				<form>
					<input {...register('value', { required: true })} defaultValue="Any default value!" />
					<p>{getValues().test}</p>
					<p>isValid = {isValid ? 'true' : 'false'}</p>
					<button type="button" onClick={() => reset({ test: '34' }, { keepDirtyValues: true })}>
						reset
					</button>
				</form>
			);
	...
	it('should keep dirty fields for dynamic controller name when keepDirty and keepDirtyValues are true', async () => {
		...
		expect(screen.getByTestId('dirtyFields').textContent).toBe('{"name_en":true}');
		...
		const App = () => {
			...
			return (
				<form>
					...
					<button type="button" onClick={() => resetField('test', { defaultValue: 'error' })}>
						resetField
					</button>
				</form>
			);
	...
	it('should clear validation errors after reset to prevent false errors on subsequent submissions (Next.js 16 Server Actions fix)', async () => {
		const resolver = vi.fn(async (data: { name: string; description?: string }) => {
		...
		expect(screen.queryByText('Description must be at least 5 characters')).not.toBeInTheDocument();
	});
	...
	it('should keep isValid value when reset is called with keepIsValid option', async () => {
		...
		// Reset with keepIsValid
		await act(async () => {
			fireEvent.click(screen.getByRole('button', { name: 'reset with keepIsValid' }));
		});
		...
		// Reset without keepIsValid
		await act(async () => {
			fireEvent.click(screen.getByRole('button', { name: 'reset without keepIsValid' }));
		});
