// Ported from react-hook-form@7.81.0 src/__tests__/useForm/watch.test.tsx (jest → vitest, octane runtime).
import { describe, expect, it, test, vi } from 'vitest';
import { useEffect, useState } from 'octane';
import { act, fireEvent, render, renderHook, screen } from '@octanejs/testing-library';

import { Controller } from '../../../src/controller.tsrx';
import type { Control, FieldValues } from '../../../src/types';
...
describe('watch', () => {
	...
	it('should watch array of inputs', () => {
		const { result } = renderHook(() => useForm<{ test: string; test1: string }>());

		expect(result.current.watch(['test', 'test1'])).toEqual([undefined, undefined]);
	...
	it('should not be able to overwrite global watch state', () => {
		function Watcher<T extends FieldValues>({ control }: { control: Control<T> }) {
			useWatch({
				control,
			});
			return null;
	...
	it('should update nameLength correctly using setValue in watch callback - CodeSandbox scenario', () => {
			...
			return (
				<div>
					<Controller
						control={control}
						name="name"
						render={({ field }) => <input type="text" {...field} data-testid="name-input" />}
					/>
					<Controller
						control={control}
						name="nameLength"
						render={({ field }) => {
							logSpy(field.value);
							return <input type="text" {...field} data-testid="nameLength-input" />;
						}}
					/>
				</div>
			);
		...
		// Type "12" into the name input
		act(() => {
			fireEvent.input(nameInput, { target: { value: '12' } });
		});
