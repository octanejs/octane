// Ported from react-hook-form@7.81.0 src/__tests__/useForm/setValues.test.tsx (jest → vitest, octane runtime).
...
describe('setValues', () => {
	...
	it('should update deep nested registered input values when setValues is called', async () => {
		const Component = () => {
			...
			return (
				<>
					<input {...register('user.profile.firstName')} aria-label="firstName" />
	...
	it('should clear errors and update isValid when revalidating with setValues', async () => {
		...
		await act(async () => {
			result.current.setValues({ email: 'test@example.com' }, { shouldValidate: true });
		});
