// Ported from react-hook-form@7.81.0 src/__tests__/useForm/useFormWithNullValues.test.tsx (jest → vitest, octane runtime).
...
describe('useForm with null values (issue #12815)', () => {
	function NumberInput({ control, name = 'views' }: { control: any; name?: string }) {
		...
		return (
			<input
				type="number"
				value={field.value ?? ''}
				onInput={(e) => field.onInput(e.target.value ? Number(e.target.value) : null)}
				onBlur={field.onBlur}
				ref={field.ref}
				data-testid={name}
			/>
	...
	it('should handle multiple fields with null values', async () => {
		...
		fireEvent.input(likesInput, { target: { value: '100' } });
