// Ported from react-hook-form@7.81.0 src/__tests__/useForm/setFocus.test.tsx (jest → vitest, octane runtime).
...
describe('setFocus', () => {
	...
	it('should focus the correct field', async () => {
		...
		await waitFor(
			() => {
				expect(document.activeElement).toBe(screen.getByPlaceholderText('test1'));
			},
			{ timeout: 100 },
		);
	});
	...
	it('should work with field arrays', async () => {
		...
		await waitFor(
			() => {
				expect(document.activeElement).toBe(screen.getByPlaceholderText('item-1'));
			},
			{ timeout: 100 },
		);
	});
