// Ported from react-hook-form@7.81.0 src/__tests__/useForm/trigger.test.tsx (jest → vitest, octane runtime).
...
describe('trigger', () => {
	...
	describe('with schema', () => {
		...
		it('should return the status of the requested field with single field validation', async () => {
			...
			await act(async () => expect(await result.current.trigger('test2')).toBeFalsy());
	...
	it('should return the status of the requested fields with array of fields for validation', async () => {
		...
		await act(async () => expect(await result.current.trigger(['test1', 'test2'])).toBeTruthy());

		await act(async () => expect(await result.current.trigger(['test3', 'test2'])).toBeFalsy());

		await act(async () => expect(await result.current.trigger(['test3'])).toBeFalsy());

		await act(async () => expect(await result.current.trigger(['test1'])).toBeTruthy());
	...
	it('should focus on errored input with build in validation', async () => {
		const Component = () => {
			...
			return (
				<>
					<input {...register('test', { required: true })} placeholder={'test'} />
					<button onClick={() => trigger('test', { shouldFocus: true })}>trigger</button>
				</>
			);
		};
	...
	it('should focus on first errored input', async () => {
		const Component = () => {
			...
			return (
				<>
					<input {...register('test', { required: true })} placeholder={'test'} />
					<input {...register('test2', { required: true })} placeholder={'test2'} />
					<button onClick={() => trigger(undefined, { shouldFocus: true })}>trigger</button>
				</>
			);
		};
	...
	it('should be able to trigger an object of fields', async () => {
		...
		function App() {
			...
			return (
				<form>
					<input {...register('test.firstName', { required: true })} placeholder="First name" />
					{errors?.test?.firstName && <p>firstName</p>}

					<input {...register('test.lastName', { required: true })} placeholder="Last name" />
	...
	it('should only trigger render on targeted input', async () => {
		...
		function Input({ name, control }: { name: FieldPath<FormValue>; control: Control<FormValue> }) {
