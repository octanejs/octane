// Ported from react-hook-form@7.81.0 src/__tests__/useForm/subscribe.test.tsx (jest → vitest, octane runtime).
import { describe, expect, it, vi } from 'vitest';
import { useEffect } from 'octane';
import { act, fireEvent, render, screen, waitFor } from '@octanejs/testing-library';

import type { UseFormSubscribe } from '../../../src/types';
	...
	it('should not call subscribe callback when setValue is called with the same value and shouldDirty option', async () => {
		...
		const App = () => {
			...
			return (
				<form>
					<input {...register('test')} />
					<button type="button" onClick={() => setValue('test', 'initial', { shouldDirty: true })}>
						Set same
					</button>
				</form>
			);
