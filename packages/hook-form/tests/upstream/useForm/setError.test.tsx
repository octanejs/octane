// Ported from react-hook-form@7.81.0 src/__tests__/useForm/setError.test.tsx (jest → vitest, octane runtime).
import { describe, expect, it, vi } from 'vitest';
import { useEffect } from 'octane';
import { act, fireEvent, render, renderHook, screen, waitFor } from '@octanejs/testing-library';

import type { DeepMap, ErrorOption, FieldError, GlobalError } from '../../../src/types';
...
describe('setError', () => {
	...
	it('should allow sequential calls to set with child after ancestor', async () => {
		const { result } = renderHook(() => useForm<{ input: { first: string; last: string } }>());
		result.current.formState.errors;
