// Ported from react-hook-form@7.81.0 src/__tests__/useForm/handleSubmit.test.tsx (jest → vitest, octane runtime).
...
describe('handleSubmit', () => {
	...
	it('should not focus if errors is exist', async () => {
		...
		expect(result.current.control._formState.errors?.test?.type).toBe('required');
	});

	it('should not focus if shouldFocusError is false', async () => {
		const mockFocus = vi.spyOn(HTMLInputElement.prototype, 'focus');

		const { result } = renderHook(() => useForm<{ test: string }>({ shouldFocusError: false }));
		...
		const App = () => {
			...
			return (
				<form>
					...
					<button
						type={'button'}
						onClick={() => handleSubmit(rejectPromiseFn)().catch((err) => setError(err.message))}
					>
						Submit
					</button>
				</form>
			...
			return (
				...
				>
					{fields.map((field, index) => {
						return <input key={field.id} {...register(`test.${index}.name`, { required: true })} />;
					})}
			...
			return (
				<>
					...
					>
						{fields.map((field, index) => {
							return (
								<input key={field.id} {...register(`test.${index}.name`, { required: true })} />
							);
						})}
