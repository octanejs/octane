// Ported from react-hook-form@7.81.0 src/__tests__/useForm/register.test.tsx (jest → vitest, octane runtime).
...
import type { FieldErrors, Resolver, ResolverResult, UseFormRegister } from '../../../src/types';
...
describe('register', () => {
		...
		function Component() {
			...
			return (
				...
				>
					...
					<input {...register('radio')} type="radio" value="No" defaultChecked />
					<input {...register('checkbox')} type="checkbox" value="Yes" />
					<input {...register('checkbox')} type="checkbox" value="No" defaultChecked />
					<button />
				</form>
			);
		}
	...
	describe('when defaultValue is provided', () => {
		...
		it('should only check checkboxes when array defaultValue includes input value', async () => {
			...
			function Component() {
				...
				return (
					<form>
						<input {...register('checkbox')} type="checkbox" value="A" aria-label="checkbox-A" />
			...
			expect((screen.getByLabelText('checkbox-A') as HTMLInputElement).checked).toBe(false);
		});
	});
	...
	it('should keep defaultValue with shouldUnregister: true when input unmounts', () => {
		...
		expect((screen.getByRole('textbox') as HTMLInputElement).value).toEqual('bill');
	...
	describe('register disabled', () => {
			...
			const Component = () => {
				...
				return (
					...
					>
						...
						<input disabled value={'test'} type={'checkbox'} {...register('test1')} />
						<input disabled value={'test'} type={'radio'} {...register('test2')} />
		...
		it('should validate value after toggling enabled/disabled on input', async () => {
			...
			await waitFor(() => expect(submit).toHaveBeenCalledWith({ test: defaultValue }));

			fireEvent.click(screen.getByText('Toggle Edit'));
			fireEvent.click(screen.getByText('Submit'));

			await waitFor(() => expect(submit).toHaveBeenCalledWith({ test: undefined }));
		});
		...
		it('should affect a group of checked attribute with disabled attribute', () => {
			...
			expect((screen.getByTestId('checkbox0') as HTMLInputElement).checked).toBeFalsy();
			expect((screen.getByTestId('checkbox1') as HTMLInputElement).checked).toBeTruthy();
			expect((screen.getByTestId('checkbox2') as HTMLInputElement).checked).toBeFalsy();
		});
		...
		it('should affect a single checked attribute with disabled attribute', () => {
			const App = () => {
				...
				return (
					<div>
						<input {...register('test')} type="checkbox" disabled={true} data-testid="checkbox1" />
						<input {...register('test1')} type="checkbox" disabled={true} data-testid="checkbox2" />
						<input
							{...register('test2')}
							type="checkbox"
							disabled={true}
			...
			function Input({
				...
				const options = {
					...
					validate: (value: string) => {
						return value && value.length > 0 ? true : 'Must have at least one checked!';
					},
				};

				return <input type="checkbox" value="a" {...register('test', options)} />;
			}
			...
			const App = () => {
				...
				return (
					<form onSubmit={handleSubmit(() => {})}>
						...
						/>
						<input {...register('lastName')} placeholder="Last Name" data-testid="lastName" />
						<input
							id="disable"
							type="checkbox"
	...
	describe('register valueAs', () => {
			...
			const Component = () => {
				...
				return (
					<form>
						...
						<input {...register('test2', { setValueAs: (data) => data === 'test' })} />
		...
		it('should still validate with an error existed', async () => {
			function App() {
				...
				const { name, ref, onBlur, onInput } = register('firstName'); // octane: register exposes onInput
			...
			return (
				<form>
					<Controller
						...
						control={control}
						render={({ field }) => (
							// octane: `onChange` here is the Checkboxes component's own prop
							// (not a DOM event); the field handler it receives is onInput.
							<Checkboxes onChange={field.onInput} value={field.value} />
						)}
					/>
					<input type="submit" />
				</form>
	...
	describe('when setValueAs is presented with inputs', () => {
		it('should not update inputs correctly with useForm defaultValues', () => {
			const App = () => {
				...
				return (
					<form>
						<input {...register('test', { setValueAs: (value) => value + '5' })} />
					</form>
				);
			};
		...
		it('should populate input as string and submit as datetime object ', async () => {
			...
			expect((screen.getByRole('textbox') as HTMLInputElement).value).toEqual('2020-10-10');
	...
	it('should be able to register input/textarea/select when embedded deeply', async () => {
		...
		// octane: ref-as-prop (no forwardRef)
		const Select = ({ ref }: { ref?: any }) => {
			return (
				<div ref={ref}>
		...
		expect((screen.getByTestId('input') as HTMLInputElement).value).toEqual('input');
		expect((screen.getByTestId('select') as HTMLSelectElement).value).toEqual('select');
		expect((screen.getByTestId('textarea') as HTMLTextAreaElement).value).toEqual('textarea');
	});
	...
	it('should trigger deps validation with schema validation', async () => {
		const App = () => {
			...
			const resolver: Resolver<Form> = async (values): Promise<ResolverResult<Form>> => {
	...
	it('should trigger custom onChange event', async () => {
		...
		expect(onChange).toHaveBeenCalledWith(
			expect.objectContaining({
				...
				cancelable: false,
				currentTarget: null,
				type: 'input', // octane: native event type (no synthetic change)
			}),
		);
	});
	...
	it('should trigger custom onBlur event', async () => {
		...
		expect(onBlur).toHaveBeenCalledWith(
			expect.objectContaining({
				bubbles: false, // octane: native blur does not bubble (no synthetic event layer)
				cancelable: false,
				currentTarget: null,
				type: 'blur',
			}),
	...
	it('should not programmatically set input file value with FileList', async () => {
				...
				<form>
					...
					{moreDetail && (
						<div>
							...
							<input type="file" {...register('Interests')} placeholder={'test'} />
						</div>
					)}
				</form>
			);
		...
		await waitFor(() => expect(screen.queryByPlaceholderText('test')).not.toBeInTheDocument());
