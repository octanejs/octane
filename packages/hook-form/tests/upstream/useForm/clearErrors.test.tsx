// Ported from react-hook-form@7.81.0 src/__tests__/useForm/clearErrors.test.tsx (jest → vitest, octane runtime).
...
describe('clearErrors', () => {
	...
	it('should remove nested error', () => {
		const { result } = renderHook(() => useForm<{ input: { nested: string } }>());
	...
	it('should prevent the submission if there is a custom error', async () => {
		const submit = vi.fn();
		const { result } = renderHook(() => useForm<{ data: string; whatever: string }>());
	...
	it('should only notify subscribers for the cleared field when using exact: true', async () => {
		...
		const FieldA = ({ control }: { control: Control<{ a: string; b: string }> }) => {
		...
		const FieldB = ({ control }: { control: Control<{ a: string; b: string }> }) => {
			const { errors } = useFormState({
				control,
				name: 'b',
				exact: true,
		...
		const App = () => {
			...
			return (
				<div>
					...
					<button onClick={() => setError('a', { message: 'error a' })}>Set Error A</button>
