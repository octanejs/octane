// Ported from react-hook-form@7.81.0 src/__tests__/useForm/resetDefaultValues.test.tsx (jest → vitest, octane runtime).
...
describe('resetDefaultValues', () => {
	it('should update default values and recompute dirtyFields/isDirty without changing form values', async () => {
		let formRef: ReturnType<typeof useForm<{ firstName: string; lastName: string }>>;
		...
		expect(screen.getByTestId('dirtyFields').textContent).toBe('{"firstName":true}');
		...
		expect((screen.getByPlaceholderText('firstName') as HTMLInputElement).value).toBe('Jane');
	...
	it('should update formState.defaultValues when resetDefaultValues is called', async () => {
		...
		expect(screen.getByTestId('defaultValues').textContent).toBe('{"name":"initial"}');

		act(() => {
			formRef.resetDefaultValues({ name: 'updated' });
		});

		expect(screen.getByTestId('defaultValues').textContent).toBe('{"name":"updated"}');
	});

	it('should work with nested objects', async () => {
		let formRef: ReturnType<typeof useForm<{ user: { name: string; age: number } }>>;
		...
		expect(screen.getByTestId('dirtyFields').textContent).toContain('"name":true');
		...
		expect((screen.getByPlaceholderText('name') as HTMLInputElement).value).toBe('Jane');
	});
	...
	it('should recompute dirty fields correctly when some fields still differ from new defaults', async () => {
		...
		fireEvent.input(screen.getByPlaceholderText('a'), {
			target: { value: 'A' },
		});
		fireEvent.input(screen.getByPlaceholderText('b'), {
			target: { value: 'B' },
		});
		fireEvent.input(screen.getByPlaceholderText('c'), {
			target: { value: 'C' },
		});

		await waitFor(() =>
			expect(screen.getByTestId('dirtyFields').textContent).toBe('{"a":true,"b":true,"c":true}'),
		);
