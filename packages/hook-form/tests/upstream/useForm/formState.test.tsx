// Ported from react-hook-form@7.81.0 src/__tests__/useForm/formState.test.tsx (jest → vitest, octane runtime).
...
import type { Control, FormState, UseFormGetFieldState } from '../../../src/types';
...
describe('formState', () => {
	describe('isValid', () => {
		...
		it('should update valid when toggle Controller', async () => {
			...
			fireEvent.input(screen.getByTestId('select'), {
				target: {
					value: 'test',
				},
			});
	...
	it('should not update form state when there is a promise reject', async () => {
		const rejectPromiseFn = vi.fn().mockRejectedValue(new Error('this is an error'));
		...
		const App = () => {
			...
			return (
				<form>
					...
					<p>{isSubmitSuccessful ? 'isSubmitSuccessful' : 'isNotSubmitSuccessful'}</p>
					<button type={'button'} onClick={() => handleSubmit(rejectPromiseFn)().catch(noop)}>
						Submit
					</button>
				</form>
			);
	...
	it('should update correct isValid formState with dynamic fields', async () => {
			...
			return (
				<form>
					<Controller
						...
						defaultValue={''}
					/>
					<input {...register('test1', { required: true })} placeholder={'test1'} />
		...
		fireEvent.input(screen.getByPlaceholderText('test1'), {
			target: { value: '1' },
		});
		...
		fireEvent.input(screen.getByPlaceholderText('list.0.firstName'), {
			target: { value: '1' },
		});
		fireEvent.input(screen.getByPlaceholderText('list.0.lastName'), {
			target: { value: '1' },
		});
	...
	it('should not update dirty fields during blur event', async () => {
		...
		const App = () => {
			...
			return (
				<form onSubmit={handleSubmit(noop)}>
					<input {...register('test', { setValueAs: (value) => value + '1' })} />
					<input type="submit" />
				</form>
			);
		};
	...
	it('should produce a new errors reference on each mutation so memoized child components re-render', async () => {
		...
		function ErrorDisplay({ errors }: { errors: FormState<FormValues>['errors'] }) {
