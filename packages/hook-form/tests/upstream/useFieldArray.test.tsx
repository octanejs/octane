// Ported from react-hook-form@7.81.0 src/__tests__/useFieldArray.test.tsx (jest → vitest, octane runtime).
...
import { FormProvider } from '../../src/useFormContext';
...
describe('useFieldArray', () => {
	...
	describe('with resolver', () => {
				...
				return (
					<form>
						{(errors.test?.root?.type ?? errors.test?.type) && (
							<p>Array error: {errors.test?.root?.message ?? errors.test?.message}</p>
						)}
						{fields.map((item, i) => (
							<div key={item.id}>
								<input {...register(`test.${i}.value` as const)} />
		...
		it('should preserve nested field errors after remove when both root and field errors exist', async () => {
			...
			await waitFor(() =>
				expect(screen.queryByText('Array error: Needs at least 4 items')).toBeInTheDocument(),
			);
			await waitFor(() =>
				expect(screen.queryByText('Item 2 error: Item 2 too short')).toBeInTheDocument(),
			);
			...
			await waitFor(() =>
				expect(screen.queryByText('Item 1 error: Item 1 too short')).toBeInTheDocument(),
			);
		});
	});
	...
	describe('with should unregister true', () => {
		it('should not unregister field if unregister method is triggered', () => {
			let getValues: any;
			const Component = () => {
				const { register, unregister, control, getValues: tempGetValues } = useForm();
				const { fields, append } = useFieldArray({ name: 'test', control });
				getValues = tempGetValues;
		...
		it('should remove field array after useFieldArray is unmounted', () => {
			...
			const FieldArray = ({ control }: { control: Control<FormValues> }) => {
				...
				return (
					<div>
						{fields.map((item, index) => {
							return <input key={item.id} name={`test.${index}.name` as const} />;
						})}
					</div>
				);
			};
		...
		it('should reset with field array', async () => {
			...
			const App = () => {
				...
				return (
					<form>
						{fields.map((field, index) => {
							return <input key={field.id} {...register(`test.${index}.value`)} />;
						})}
			...
			const App = () => {
				...
				return (
					<form>
						...
						<button type={'button'} onClick={() => reset({ test: [{ value: 'reset' }] })}>
							reset
						</button>
	...
	describe('with setValue', () => {
			...
			async (property) => {
				...
				const Component = () => {
					const {
						...
						defaultValues: {
							test: [{ name: 'default' }, { name: 'default1' }, { name: 'default2' }],
						},
					});
				...
				await act(async () => {
					setValue(
						'test',
						[{ name: 'default_update' }, { name: 'default1' }, { name: 'default2' }],
						{ shouldDirty: true },
					);
				});
		...
		it.each(['dirtyFields'])(
			...
			(property) => {
				...
				act(() => {
					setValue('test', [{ name: 'default' }, { name: 'default1' }, { name: 'default2' }], {
						shouldDirty: true,
					});
				});
		...
		it('should set nested field array correctly', async () => {
			...
			function NestedArray({ control, index }: { control: Control<FormValues>; index: number }) {
				const { fields } = useFieldArray({
					...
					<ul>
						{fields.map((item, i) => (
							<Controller
								key={item.id}
								render={({ field }) => (
									<input {...field} aria-label={`test.${index}.keyValue.${i}.name`} />
								)}
			...
			function Component() {
				...
				return (
					<form>
						...
						<button type="button" onClick={() => setValue('test.0.keyValue', [{ name: '2a' }])}>
							setValue
						</button>
					</form>
				);
			...
			const input = screen.getByLabelText('test.0.keyValue.0.name') as HTMLInputElement;

			expect(input.value).toEqual('2a');

			expect((screen.getByLabelText('test.0.firstName') as HTMLInputElement).value).toEqual('Bill');
		});
	});
	...
	describe('array of array fields', () => {
					...
					<div>
						...
						{fields.map((nestedField, index) => (
							<div key={nestedField.id}>
								<input
									{...register(`fieldArray.${arrayIndex}.nestedFieldArray.${index}.value` as const)}
								/>
								<button type="button" onClick={() => remove(index)}>
									remove
								</button>
		...
		it('should prepend correctly with default values on nested array fields', () => {
			...
			const Component = () => {
				...
				return (
					<>
						...
						<button type={'button'} onClick={() => prepend({ value: 'test', nestedArray: [] })}>
							prepend
						</button>
					</>
				);
			...
			expect((screen.getAllByRole('textbox')[0] as HTMLInputElement).value).toEqual('test');
		});
					...
					<div>
						{fields.map((item, i) => (
							<div key={item.id}>
								...
								<button type={'button'} onClick={() => remove(i)} data-testid={item.value}>
									remove
								</button>
							</div>
						))}

						<button type={'button'} onClick={() => append({ value: 'test', nestedArray: [] })}>
							append
						</button>
					</div>
				);
					...
					<div>
						{fields.map((item, i) => (
							<label key={item.id}>
								...
								<input {...control.register(`nest.${index}.nestedArray.${i}.value` as const)} />
							</label>
						))}
					</div>
				);
		...
		it('should worked with deep nested field array without chaining useFieldArray', () => {
					...
					<div>
						{fields.map((item, i) => (
							<label key={item.id}>
								...
								<input
									{...control.register(`nest.${index}.nestedArray.deepNest.${i}.value` as const)}
								/>
							</label>
						))}
						<button type={'button'} onClick={() => append({ value: 'test' })}>
			...
			expect(screen.getByRole('textbox', { name: 'nest.0.value' })).toHaveValue('newV1');
			expect(
	...
	it('should keep field values when field array gets unmounted and mounted', async () => {
		...
		const Test = ({
			...
			return (
				<div>
					{fields.map((field, i) => {
						return <input key={field.id} {...register(`test.${i}.firstName` as const)} />;
					})}
					<button
						onClick={() =>
							append({
	...
	it.each([
		...
		({ action, expectedValues, mutate }) => {
			...
			const renderedValues = screen.getByTestId('list-display').querySelectorAll('p');

			expect(Array.from(renderedValues, (element) => element.textContent)).toEqual(expectedValues);
		},
	);
	...
	it('should append deep nested field array correctly with strict mode', async () => {
		...
		expect((screen.getAllByRole('textbox')[0] as HTMLInputElement).value).toEqual('bill');
	...
	it('should not populate defaultValue when field array is already mounted', async () => {
		...
		expect((screen.getAllByRole('textbox')[2] as HTMLInputElement).value).toEqual('default');
		expect((screen.getAllByRole('textbox')[3] as HTMLInputElement).value).toEqual(
			'child of index 0',
		);
	});
	...
	it('should update field array correctly when unmounted field', () => {
		...
		function Nested({
			...
			return (
				<>
					{fields.map((field, i) => (
						<input key={field.id} {...register(`nest.${index}.nested.${i}.value`)} />
					))}
				</>
			);
		}
		...
		function App() {
			...
			function handleAddInner() {
				setValue(`nest.1.nested`, [...getValues(`nest.1.nested`), { value: `1sub-new` }]);
			}
	...
	describe('with rules', () => {
		...
		it('should no longer validate when unmounted', async () => {
				...
				return (
					<FormProvider {...formValues}>
						<form onSubmit={formValues.handleSubmit(noop)}>
							<button type="button" onClick={() => setDisplayArray((current) => !current)}>
								Toggle
							</button>
			...
			expect(screen.queryByText('Min length should be 2')).not.toBeInTheDocument();
	...
	describe('with nested field array ', () => {
							...
							return (
								<div key={field.id}>
									<ArrayField
										...
										control={control}
									/>
									<p>{errors?.fieldArray?.[0]?.nestedFieldArray?.root?.message}</p>
								</div>
							);
						})}
						<button
		...
		it('should report field array error at the nested useFieldArray level during field level action', async () => {
			...
			await act(async () => {
				fireEvent.click(screen.getByRole('button', { name: 'Add nested array' }));
			});
	...
	it('should update isValid correctly with rules props and inline validation', async () => {
		const App = () => {
			...
			return (
				<div>
					{fields.map((field, index) => (
						<input key={field.id} {...register(`test.${index}.value`, { required: true })} />
					))}
		...
		const App = () => {
			...
			return (
				<form onSubmit={handleSubmit(() => {})}>
					{errors.test?.root?.message && <p data-testid="error">{errors.test.root.message}</p>}
...
it('should not corrupt parent state when remove is called with values prop', async () => {
				...
				{fields.map((item, index) => (
					<div key={item.id}>
						<Controller
							...
							rules={{ required: true }}
							render={({ field }) => <input data-testid={`name-${index}`} {...field} />}
						/>
						<Controller
							control={control}
							name={`myfield.${index}.text` as const}
							rules={{ required: true }}
							render={({ field }) => <input data-testid={`text-${index}`} {...field} />}
						/>
						<button type="button" onClick={() => remove(index)}>
							remove-{index}
						</button>
	...
	fireEvent.input(screen.getByTestId('name-0'), {
		target: { value: 'Alice' },
	});
	fireEvent.input(screen.getByTestId('text-0'), {
		target: { value: 'hello' },
	});

	await waitFor(() => expect(onUpdateCalls.some((c) => c.myfield[0]?.name === 'Alice')).toBe(true));

	fireEvent.click(screen.getByText('add'));

	await waitFor(() => screen.getByTestId('name-1'));

	fireEvent.input(screen.getByTestId('name-1'), {
		target: { value: 'Bob' },
	});
	fireEvent.input(screen.getByTestId('text-1'), {
		target: { value: 'world' },
	});

	await waitFor(() => expect(onUpdateCalls.some((c) => c.myfield[1]?.name === 'Bob')).toBe(true));
			...
			<>
				{fields.map((field, index) => (
					<div key={field.id}>
						<label>
							A
							<input type="radio" value="A" {...register(`items.${index}.obj.value` as const)} />
						</label>
						<label>
							C
							<input type="radio" value="C" {...register(`items.${index}.obj.value` as const)} />
						</label>
					</div>
				))}
				<button
