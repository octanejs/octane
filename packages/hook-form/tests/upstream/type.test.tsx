// Ported from react-hook-form@7.81.0 src/__tests__/type.test.tsx (jest → vitest, octane runtime).
		...
		return (
			<div>
				<Controller
					...
					defaultValue=""
					control={control}
					render={({ field: { value, onInput: onChange } }) => {
...
test('should work with useWatch return correct array types', () => {
	...
	const App = () => {
		const { control } = useForm<FormValues>();
		const output: [FormValues['testString'], FormValues['testNumber'], FormValues['testObject']] =
			useWatch({
				control,
...
test('should type errors correctly with Path generic', () => {
	...
	function Input<T extends FieldValues = FieldValues>({ name, register, errors }: InputProps<T>) {
		return (
			<>
				<input {...register(name)} />
...
test('should infer context type into control', () => {
	function App() {
		...
		const { control } = useForm<{ test: Record<string, unknown>[] }, { isValid: boolean }>({
