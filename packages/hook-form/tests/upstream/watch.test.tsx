// Ported from react-hook-form@7.81.0 src/__tests__/watch.test.tsx (jest → vitest, octane runtime).
import { describe, expect, it, test, vi } from 'vitest';
import { useState } from 'octane';
import { fireEvent, render, screen, waitFor } from '@octanejs/testing-library';

import type { Control, UseFormReturn } from '../../src/types';
			...
			return (
				<form>
					...
					<Watch
						control={control}
						names={['baz.qux']}
						render={([bazQux]) => <span data-testid="bazQuxText">{bazQux}</span>}
					/>
				</form>
			);
		};
