// Ported from react-hook-form@7.81.0 src/__tests__/useFieldArray/remove.test.tsx (jest → vitest, octane runtime).
...
import type { Control, DeepMap, FieldError } from '../../../src/types';
...
describe('remove', () => {
		...
		const Component = () => {
			...
			return (
				<form>
					{fields.map((field, i) => (
						<div key={field.id}>
							<input {...register(`test.${i}.name` as const, { required: true })} />
							<button type={'button'} onClick={() => remove(i)}>
								remove
							</button>
						</div>
	...
	it.each(['isDirty', 'dirtyFields'])('should be dirtyFields when value is remove with %s', () => {
		const { result } = renderHook(() => {
	...
	it('should remove specific field if isValid is true', async () => {
		const Component = () => {
			...
			return (
				<form>
					{fields.map((field, i) => (
						<input key={field.id} {...register(`test.${i}.value` as const, { required: true })} />
					))}
	...
	it('should remove nested field array error', async () => {
		...
		const Nested = ({
			...
			const { fields, append, remove } = useFieldArray<FormValues, 'test.0.nested'>({
			...
			return (
				<fieldset>
					{fields.map((field, i) => (
						<div key={field.id}>
							<input
								{...control.register(`test.${index}.nested.${i}.test` as const, {
									required: 'required',
								})}
							/>
							{errors?.test && errors.test[index]?.nested && errors.test[index].nested[i]?.test && (
								<span data-testid="nested-error">{errors.test[index].nested[i].test.message}</span>
							)}
							<button type="button" onClick={() => remove(i)}>
								nested delete
							</button>
					...
					<button type="button" onClick={() => append({ test: 'test', key: mockKey++ })}>
						nested append
					</button>
				</fieldset>
			);
			...
			return (
				<>
					...
					<form onSubmit={handleSubmit(callback)}>
						{fields.map((_, i) => (
							<Nested key={i.toString()} errors={errors} control={control} index={i} />
						))}
						<button>submit</button>
					</form>
				</>
		...
		await waitFor(() => expect(screen.queryByTestId('nested-error')).not.toBeInTheDocument());
	...
	it("should not reset Controller's value during remove when Field Array name is already registered", async () => {
		...
		expect((screen.getAllByRole('textbox')[0] as HTMLInputElement).value).toEqual('111');
	});
	...
	it('should not omit keyName when provided', async () => {
		...
		expect(await screen.findByText('{"test":[{"id":"4567","test":"data1"}]}')).toBeVisible();
	});
	...
	it('should not omit keyName when provided and defaultValue is empty', async () => {
		...
		expect(await screen.findByText('{"test":[{"id":"whatever1","test":"12341"}]}')).toBeVisible();
	});
	...
	it('should not leave empty objects in watched values when removing from field array with values prop', async () => {
		...
		function App() {
			const [model, setModel] = useState<FormValues>({ items: [] });
	...
	it('should not re-insert removed items when using values prop with keepDirtyValues', async () => {
		...
		function App() {
			const [record, setRecord] = useState<FormValues>({
				test: [{ value: 'foo' }],
			});
		...
		const inputs = screen.getAllByRole('textbox');
		fireEvent.input(inputs[1], { target: { value: 'bar' } });
