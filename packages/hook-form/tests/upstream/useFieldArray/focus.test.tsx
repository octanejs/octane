// Ported from react-hook-form@7.81.0 src/__tests__/useFieldArray/focus.test.tsx (jest → vitest, octane runtime).
			...
			return (
				<form onSubmit={handleSubmit(noop)}>
					{fields.map((field, index) => (
						<div key={field.id}>
							<input {...register(`test.${index}.value`)} defaultValue={field.value} />
		...
		const Component = () => {
			...
			return (
				<form>
					...
					<button type="button" onClick={() => append({ value: '' }, { focusIndex: 0 })}>
						append
					</button>
					<button type="button" onClick={() => insert(1, { value: '' }, { focusIndex: 0 })}>
						insert
					</button>
				</form>
			);
		...
		const Component = () => {
			...
			return (
				<form>
					...
					<button
						type="button"
						onClick={() => prepend({ value: '' }, { focusName: 'test.1.value' })}
					>
						prepend
					</button>
					<button
					...
					<button
						type="button"
						onClick={() => insert(1, { value: '' }, { focusName: 'test.0.value' })}
					>
						insert
					</button>
				</form>
