// Ported from react-hook-form@7.81.0 src/__tests__/useFieldArray/insert.test.tsx (jest → vitest, octane runtime).
...
import type { Control, FieldPath } from '../../../src/types';
...
describe('insert', () => {
	...
	it.each(['isDirty', 'dirtyFields'])(
		...
		() => {
			...
			expect(result.current.formState.dirtyFields).toEqual({
				test: [undefined, { value1: true, value: true }, { value: true, value1: true }],
			});
		},
	);
		...
		const Component = () => {
			...
			return (
				<form>
					...
					<button type="button" onClick={() => insert(1, { value: `${fields.length}` })}>
						insert
					</button>
				</form>
			);
			...
			return (
				<form>
					...
					<button
						type="button"
						onClick={() =>
							insert(1, [{ value: `${fields.length}` }, { value: `${fields.length + 1}` }])
						}
					>
						insert array
					</button>
		...
		const Component = () => {
			...
			return (
				<form onSubmit={handleSubmit(noop)}>
					{fields.map((field, i) => (
						<input key={field.id} {...register(`test.${i}.value`, { required: true })} />
					))}
		...
		const Component = () => {
			...
			return (
				<form onSubmit={handleSubmit(noop)}>
					...
					<button type="button" onClick={() => insert(1, [{ value: '' }, { value: '' }])}>
						insert array
					</button>
					<button>submit</button>
				</form>
		...
		const Component = () => {
			...
			return (
				<form>
					...
					<button type="button" onClick={() => insert(1, { value: '' }, { shouldFocus: false })}>
						insert
					</button>
				</form>
			);
	...
	it('should append nested field value without its reference', () => {
			...
			return (
				<>
					{fields.map((item, index) => (
						<Input key={item.id} name={`test.${index}.name.deep`} control={control} />
					))}
		...
		function App() {
			...
			return (
				<form>
					<FieldArray name="test" control={control} itemDefaultValue={{ name: { deep: '' } }} />
				</form>
			);
		}
		...
		expect((screen.getAllByRole('textbox')[0] as HTMLInputElement).value).toEqual('');
	});
	...
	it('should not omit keyName when provided and defaultValue is empty', async () => {
		...
		expect(await screen.findByText('{"test":[{"id":"whatever","test":"1234"}]}')).toBeVisible();
	});

	it('should not invoke resolver per register during insert; only array-scoped + final isValid', async () => {
		const resolver = vi.fn().mockImplementation((values) => ({ values, errors: {} }));
		...
		await waitFor(() => expect(resolver.mock.calls.length).toBeGreaterThanOrEqual(1));
		const initialCalls = resolver.mock.calls.length;

		fireEvent.click(screen.getByRole('button', { name: 'insert' }));

		await waitFor(() => expect(resolver.mock.calls.length).toBe(initialCalls + 2));
	});
});
