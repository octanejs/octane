// Ported from react-hook-form@7.81.0 src/__tests__/useFieldArray/replace.test.tsx (jest → vitest, octane runtime).
...
describe('replace', () => {
			...
			return (
				<form>
					{fields.map((field, index) => {
						return <input key={field.id} {...register(`test.${index}.x` as const)} />;
					})}
	...
	it('should not omit keyName when provided', async () => {
		...
		expect(await screen.findByText('{"test":[{"id":"test","test":"data"}]}')).toBeVisible();
	});
	...
	it('should not omit keyName when provided and defaultValue is empty', async () => {
		...
		expect(await screen.findByText('{"test":[{"id":"whatever","test":"data"}]}')).toBeVisible();
	});
	...
	it('should not affect other formState during replace action', () => {
		...
		expect(screen.getByText('{"fieldArray":[{"firstName":true}]}')).toBeVisible();
	});
			...
			const App = () => {
				...
				return (
					<form>
						...
						<button
							type="button"
							onClick={() => replace([{ value: '1' }, { value: '2' }, { value: '3' }])}
						>
							replace
						</button>
					</form>
