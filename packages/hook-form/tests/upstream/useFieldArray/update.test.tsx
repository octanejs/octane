// Ported from react-hook-form@7.81.0 src/__tests__/useFieldArray/update.test.tsx (jest → vitest, octane runtime).
...
describe('update', () => {
			...
			return (
				<form>
					{fields.map((field, index) => {
						return <input key={field.id} {...register(`test.${index}.test` as const)} />;
					})}
	...
	it('should update group input correctly', () => {
		...
		const GroupInput = ({ control, index }: { control: Control<FormValues>; index: number }) => {
		...
		expect((screen.getAllByRole('textbox')[0] as HTMLInputElement).value).toEqual('firstName');
		expect((screen.getAllByRole('textbox')[1] as HTMLInputElement).value).toEqual('lastName');
	...
	it('should update field array with multiple values', () => {
		...
		expect((screen.getAllByRole('textbox')[1] as HTMLInputElement).value).toEqual('test2');
		expect((screen.getAllByRole('textbox')[2] as HTMLInputElement).value).toEqual('test3');
		expect((screen.getAllByRole('textbox')[3] as HTMLInputElement).value).toEqual('test4');
