// Ported from react-hook-form@7.81.0 src/__tests__/useFieldArray/dirtyFields.test.tsx (jest → vitest, octane runtime).
		...
		const Component = () => {
			...
			return (
				<form>
					...
					<button
						type="button"
						onClick={() => itemsCopyArray.append({ value: 'new', value2: 'new2' })}
					>
						appendCopy
					</button>
				</form>
		...
		const Component = () => {
			...
			return (
				<form>
					...
					<button type="button" onClick={() => itemsArray.append({ value: 'item1' })}>
						appendItems
					</button>
					<button
						type="button"
						onClick={() => itemsCopyArray.append({ value: 'copy1', value2: 'copy2' })}
					>
						appendCopy
					</button>
				</form>
		...
		const Component = () => {
			...
			return (
				<form>
					...
					{fields.map((field, i) => (
						<div key={field.id}>
							<input {...register(`nonusservices.VATFiling_list.${i}.id` as const)} />
							<input {...register(`nonusservices.VATFiling_list.${i}.value` as const)} />
						</div>
					))}
					<button type="button" onClick={() => append({ id: '2', value: 'Item 2' })}>
						append
					</button>
				</form>
			);
			...
			return (
				<form>
					...
					{keyValueFields.map((field, i) => (
						<input key={field.id} {...register(`test.0.keyValue.${i}.name` as const)} />
					))}
					<button type="button" onClick={() => append({ name: 'new' })}>
						nestAppend
					</button>
					<button type="button" onClick={() => remove(keyValueFields.length - 1)}>
						nestRemove
					</button>
				</form>
			);
		...
		const Component = () => {
			...
			return (
				<form>
					...
					<button
						type="button"
						onClick={() => setValue('items', [{ value: 'new' }], { shouldDirty: true })}
					>
						setItems
					</button>
				</form>
			...
			return (
				<div>
					<form onSubmit={handleSubmit(() => {})}>
						...
						<button
							type="button"
							onClick={() => setValue('firstName', 'temp', { shouldDirty: false })}
						>
							Set First Name
						</button>
