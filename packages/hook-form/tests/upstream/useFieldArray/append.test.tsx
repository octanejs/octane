// Ported from react-hook-form@7.81.0 src/__tests__/useFieldArray/append.test.tsx (jest → vitest, octane runtime).
...
describe('append', () => {
		...
		const Component = () => {
			...
			return (
				<form>
					...
					<button
						type={'button'}
						onClick={() => append([{ test: 'test-batch' }, { test: 'test-batch1' }])}
					>
						appendBatch
					</button>
				</form>
		...
		const Component = () => {
			...
			return (
				<form>
					...
					<button type="button" onClick={() => append({ value: '3' }, { shouldFocus: false })}>
						append
					</button>
				</form>
			);
	...
	it('should append nested field value without its reference', () => {
		...
		expect((screen.getAllByRole('textbox')[1] as HTMLInputElement).value).toEqual('');
	});
			...
			const App = () => {
				...
				return (
					<form>
						...
						<button
							type="button"
							onClick={() => append([{ value: '1' }, { value: '2' }, { value: '3' }])}
						>
							append
						</button>
					</form>
