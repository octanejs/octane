// Ported from react-hook-form@7.81.0 src/__tests__/useForm.test.tsx (jest → vitest, octane runtime).
...
describe('useForm', () => {
	...
	describe('when errors changes', () => {
		...
		it('should display the latest error message with errors prop', () => {
			const Form = () => {
				...
				const [errorsState, setErrorsState] = useState<FieldErrors<FormValues>>({
		...
		it('should use updated mode and reValidateMode inside of onChange handler', async () => {
			...
			const Form = () => {
				...
				const [reValidateMode, setReValidateMode] =
					useState<Exclude<Mode, 'onTouched' | 'all'>>('onBlur');
			...
			fireEvent.click(screen.getByRole('button', { name: /update validation mode/i }));
		...
		it('should have formState.isValid equals true with defined default values after executing resolver', async () => {
			const Toggle = () => {
				...
				const { register, formState } = useForm({
					...
					mode: 'onChange',
					resolver: async (values): Promise<ResolverResult<{ test: string }>> => {
						if (!values.test) {
							return {
								values: {},
								errors: {
	...
	it('should update isValidating form and field states correctly', async () => {
		...
		fireEvent.input(screen.getByPlaceholderText('async'), {
			target: { value: 'test' },
		});
		fireEvent.input(screen.getByPlaceholderText('required'), {
			target: { value: 'test' },
		});
	...
	it('should disable form inputs separately from its form', async () => {
		function App() {
			...
			return (
				<form>
					<input {...register('firstName', { disabled: true })} placeholder="firstName" />
					<input {...register('lastName', { disabled: false })} placeholder="lastName" />
				</form>
			);
		}
		...
		await waitFor(() => {
			expect((screen.getByPlaceholderText('firstName') as HTMLInputElement).disabled).toBeTruthy();
			expect((screen.getByPlaceholderText('lastName') as HTMLInputElement).disabled).toBeFalsy();
		});
	});
	...
	it('should allow submitting a form with disabled form fields', async () => {
		...
		await waitFor(() => {
			expect((screen.getByPlaceholderText('firstName') as HTMLInputElement).disabled).toBeTruthy();
			expect(screen.getByText('no error') as HTMLInputElement).toBeInTheDocument();
		});
	});
	...
	describe('when given formControl', () => {
		it('accepts default values', async () => {
			...
			const { register, handleSubmit, formControl } = createFormControl<FormValues>();
		...
		it('should reset useController value on remount with defaultValues', async () => {
			...
			const { register, handleSubmit, formControl, control } = createFormControl<FormValues>();
			const onSubmit = vi.fn();
			...
			function Form({ onFormSubmit }: { onFormSubmit: SubmitHandler<FormValues> }) {
				useForm({
					formControl,
					defaultValues: {
						firstName: '',
			...
			expect((screen.getByPlaceholderText('lastName') as HTMLInputElement).value).toBe('');
			fireEvent.click(screen.getByText('submit'));
		...
		it('should re-initialise when formControl prop reference changes (e.g. HMR/Fast Refresh)', async () => {
			...
			function Form({ ctrl, submit }: { ctrl: typeof formControl; submit: typeof handleSubmit }) {
			...
			const { rerender } = render(<Form ctrl={formControl} submit={handleSubmit} />);
			...
			await waitFor(() => {
				expect(onSubmit).toHaveBeenCalledWith({ firstName: 'Bill' }, expect.any(Object));
			});
		});
	});
	...
	describe('form level validation', () => {
		...
		it('should use error message from object return value', async () => {
			...
			await waitFor(() =>
				expect(screen.getByTestId('msg').textContent).toBe('first name is required'),
			);
		});
		...
		it('should preserve custom error type from object return value', async () => {
			...
			await waitFor(() => expect((capturedErrors as any).form?.firstName?.type).toBe('customType'));
		});
		...
		it('should clear object return errors when form becomes valid', async () => {
			...
			await waitFor(() => expect(screen.getByTestId('msg').textContent).toBe(''));
		});
	});
});
