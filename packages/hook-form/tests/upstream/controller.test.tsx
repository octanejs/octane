// Ported from react-hook-form@7.81.0 src/__tests__/controller.test.tsx (jest → vitest, octane runtime).
import { describe, expect, it, test, vi } from 'vitest';
import { useState, useEffect, memo } from 'octane';
import {
	act as actComponent,
	fireEvent,
	render,
...
import type { Control, ControllerRenderProps, FieldValues, ValidateResult } from '../../src/types';
...
function Input<TFieldValues extends FieldValues>({
	...
	return <input placeholder={placeholder} onInput={() => onChange(1)} onBlur={() => onBlur()} />;
}
...
describe('Controller', () => {
	...
	it('should show input is dirty.', async () => {
		...
		fireEvent.input(input, { target: { value: 'dirty' } });
	...
	it('should create error object when the value is Invalid Date during onChange event', async () => {
		...
		fireEvent.input(input, { target: { value: '2024-10-16' } });
	...
	it('should recover memoized Controller field after reset without rerender', async () => {
		...
		const MemoField = memo(({ control }: { control: Control<FormValues> }) => (
			<Controller
				control={control}
				name="test"
				render={({ field }) => <input value={field.value} onInput={field.onInput} />}
