// Ported from react-hook-form@7.81.0 src/__tests__/form.test.tsx (jest → vitest, octane runtime).
import { beforeEach, describe, expect, it, vi } from 'vitest';
import { useState } from 'octane';

import { fireEvent, render, screen, waitFor } from '@octanejs/testing-library';

import { Form } from '../../src/form.tsrx';
...
describe('Form', () => {
	...
	it('should handle error request callback', async () => {
		...
		const App = () => {
			...
			return (
				<Form encType={'application/json'} action={'/error'} onSubmit={onSubmit} control={control}>
					<button>Submit</button>
					<p>{isSubmitSuccessful ? 'submitSuccessful' : 'submitFailed'}</p>
	...
	it('should validate custom status code', async () => {
		vi.spyOn(global, 'fetch').mockResolvedValueOnce(new Response(null, { status: 201 }));
	...
	it('should include application/json header with encType supplied', async () => {
		...
		const App = () => {
			...
			return (
				<Form encType={'application/json'} action={'/json'} control={control} onSuccess={onSuccess}>
					<button>Submit</button>
					<p>{isSubmitSuccessful ? 'submitSuccessful' : 'submitFailed'}</p>
				</Form>
			);
