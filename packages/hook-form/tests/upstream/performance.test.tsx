// Ported from react-hook-form@7.81.0 src/__tests__/performance.test.tsx (jest → vitest, octane runtime).
...
function changeEmits(spy: vi.SpyInstance): Record<string, unknown>[] {
	return spy.mock.calls
		.map(([payload]) => payload)
		.filter((p): p is Record<string, unknown> => p != null && typeof p === 'object' && 'type' in p);
}
...
describe('onChange value-clone optimization', () => {
	it('emits without a values key when no value subscribers are active', () => {
		...
		fireEvent.input(screen.getByTestId('name'), { target: { value: 'b' } });
		fireEvent.input(screen.getByTestId('name'), { target: { value: 'c' } });
	...
	it('emits a values key when watch(fn) is active', async () => {
		...
		fireEvent.input(screen.getByTestId('name'), { target: { value: 'x' } });
	...
	it('handles one change per field on a 50-field form within budget', () => {
		...
		function Form() {
			const { register } = useForm({
				defaultValues: Object.fromEntries(names.map((n) => [n, ''])) as Record<string, string>,
			});
			return (
				<form>
					{names.map((n) => (
		...
		for (const n of names) {
			fireEvent.input(screen.getByTestId(n), { target: { value: 'x' } });
		}
		expect(performance.now() - t0).toBeLessThan(BUDGET_MS);
	});
...
describe('_getDirty optimization', () => {
	...
	it('tracks isDirty efficiently when multiple fields stay dirty (O(1) fast path)', () => {
		...
		for (let i = 1; i < names.length; i++) {
			fireEvent.input(screen.getByTestId(`f${i}`), {
				target: { value: 'dirty' },
			});
		}
		...
		const t0 = performance.now();
		for (let i = 0; i < 500; i++) {
			fireEvent.input(screen.getByTestId('f0'), {
	...
	it('surfaces untracked setValue fields in dirtyFields when the last tracked field goes pristine', async () => {
		...
		await act(async () => {
			fireEvent.input(screen.getByTestId('b'), { target: { value: 'x' } });
		});
		await act(async () => {
			fireEvent.input(screen.getByTestId('b'), { target: { value: '' } });
		});
...
describe('isDirty proxy guard', () => {
	...
	it('includes isDirty in state notification when subscribed', async () => {
		...
		const emits = nextSpy.mock.calls.map(([p]) => p);
		expect(emits.some((p: any) => 'isDirty' in p && p.isDirty === true)).toBe(true);
	});
});
		...
		function Form() {
			...
			return (
				<form>
					...
					{toggleValue && (
						<input {...register('required', { required: true })} data-testid="required" />
					)}
				</form>
			);
		}
...
describe('Re-render efficiency', () => {
	it('does not re-render the form parent when a child useWatch re-renders', () => {
		...
		const Child = ({ control }: { control: Control<{ a: string; b: string }> }) => {
		...
		fireEvent.input(screen.getByTestId('a'), { target: { value: 'changed' } });
	...
	it('does not re-render a useWatch component when an unwatched field changes', () => {
		...
		const Watcher = ({ control }: { control: Control<{ a: string; b: string }> }) => {
			useWatch({ control, name: 'a' });
			watcherRenders++;
			return <div />;
		};
...
describe('setValues emission batching', () => {
	it('emits exactly one state notification regardless of how many fields change', async () => {
		...
		await act(async () => {
			result.current.setValues(Object.fromEntries(names.map((n) => [n, 'x'])) as any);
		});
	...
	it('delivers correct values and dirty state after batched setValues', async () => {
		...
		function Form() {
			...
			return (
				<button onClick={() => setValues({ a: 'x', b: 'y', c: '' }, { shouldDirty: true })}>
					set
				</button>
			);
		}
...
describe('setValues _valuesSubscriberCount guard', () => {
	it('omits values from the broadcast when no watch(fn) subscriber is active', async () => {
		const { result } = renderHook(() => useForm({ defaultValues: { a: '', b: '' } }));
