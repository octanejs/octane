// Ported from react-hook-form@7.81.0 src/__tests__/nested-null.test.tsx (jest → vitest, octane runtime).
...
describe('nested null bug', () => {
	it('should not keep parent as null and allow nested value', async () => {
		...
		await waitFor(() => {
			expect(onSubmit).toHaveBeenCalledWith({ example: { inner: '' } }, expect.anything());
		});
	});
		...
		function TestComponent() {
			...
			return (
				<form>
					...
					<button type="button" onClick={() => unregister('example.nested.deep')}>
						Unregister
					</button>
				</form>
			);
