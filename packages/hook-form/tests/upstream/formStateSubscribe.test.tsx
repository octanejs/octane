// Ported from react-hook-form@7.81.0 src/__tests__/formStateSubscribe.test.tsx (jest → vitest, octane runtime).
...
describe('FormStateSubscribe', () => {
		...
		const Test1 = ({ control }: { control: Control }) => (
			<FormStateSubscribe
				...
				render={({ isDirty, dirtyFields }) => {
					...
					return (
						<>
							<div>{dirtyFields['test'] ? 'hasDirtyField' : 'notHasDirtyField'}</div>
		...
		const Component = () => {
			...
			return (
				<form>
					...
					<input {...register('age', { valueAsNumber: true, required: true })} type="number" />
				</form>
			);
		};
			...
			return (
				<div>
					...
					<FormStateSubscribe
						...
						control={control}
						exact={exact}
						render={({ touchedFields }) => <p>{touchedFields.testData && 'touched'}</p>}
					/>
	...
	it('should be able to access defaultValues', () => {
		...
		const Test = ({ control }: { control: Control<FormValues> }) => (
			<FormStateSubscribe
				control={control}
				render={(formState) => (
					<p>{deepEqual(formState.defaultValues, defaultValues) ? 'yes' : 'no'}</p>
				)}
			/>
		);
	...
	it('should conditionally update formState after mount', async () => {
		...
		function App() {
			const [showDirty, toggleShowDirty] = useReducer((p) => !p, false);
	...
	it('should return the latest values with async values', async () => {
		...
		function Input({ control }: { control: Control<FormValues> }) {
			return (
				<FormStateSubscribe control={control} render={({ isValid }) => <p>{String(isValid)}</p>} />
			);
		}
