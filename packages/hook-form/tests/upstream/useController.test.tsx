// Ported from react-hook-form@7.81.0 src/__tests__/useController.test.tsx (jest → vitest, octane runtime).
...
import type { Control, FieldPath, FieldValues, UseFormReturn } from '../../src/types';
...
describe('useController', () => {
	...
	// OCTANE DIVERGENCE[hook-form-native-input-handler][adapted:controller-on-input]: Octane exposes native onInput instead of React's synthetic onChange.
	...
	it("setting values doesn't cause fields to be unregistered", async () => {
		function App() {
			...
			return useMemo(() => values?.login && <LoginField form={form} />, [values, form]);
		}
		...
		function LoginField({ form }: { form: UseFormReturn<{ login: string }> }) {
			...
			return <input value={ctrl.field.value} onInput={ctrl.field.onInput} />;
		}
	...
	describe('checkbox', () => {
		it('should work for checkbox by spread the field object', async () => {
			...
			expect(watchResult).toEqual([{}, { test: '' }, { test: true }, { test: false }]);
		});
			...
			const Component = () => {
				...
				return (
					<input
						type="checkbox"
						checked={!!field.value}
						onInput={(e) => field.onInput(e.target.checked)}
					/>
				);
			};
		...
		it('should work for checkbox by assign value manually', async () => {
			...
			const Component = () => {
				...
				return (
					<input
						...
						type="checkbox"
						checked={!!field.value}
						onInput={(e) => field.onInput(e.target.checked ? e.target.value : false)}
					/>
				);
			};
			...
			expect(watchResult).toEqual([{}, { test: '' }, { test: 'on' }, { test: false }]);
		});
	});
	...
	it('should not change reference for onChange and onBlur on input value change', () => {
		...
		const App = () => {
			...
			useEffect(() => {
				...
				field.onInput;
				field.ref;
			}, [field.onInput, field.onBlur, field.ref]);
	...
	it('should return defaultValues when component is not yet mounted', async () => {
		...
		expect(await screen.findByText('{"test":{"deep":[{"test":"0","test1":"1"}]}}')).toBeVisible();
	});
	...
	it('should update isValid correctly with strict mode', async () => {
		const App = () => {
			...
			return (
				<FormProvider {...form}>
					<Controller
						render={({ field }) => <input value={field.value} onInput={field.onInput} />}
						name="name"
						rules={{
							required: true,
						}}
	...
	it('should subscribe to exact form state update', () => {
		...
		fireEvent.input(screen.getByRole('textbox', { name: 'test' }), {
			target: {
				value: '',
			},
		});
		...
		fireEvent.input(screen.getByRole('textbox', { name: 'test_with_suffix' }), {
			target: {
				value: '',
			},
		});
	...
	it('should prevent value leakage and preserve previous field value when name changes', () => {
		...
		const Component = () => {
			...
			return (
				<div>
					<select value={type} onInput={(e) => setValue('type', e.target.value as any)}>
		...
		expect(screen.getByTestId('personal-name-value').textContent).toBe('John Doe');
	});
	...
	it('should keep nested array values when useController and watch APIs switch dynamic names', async () => {
		...
		const VisibilityField = ({ name }: { name: FieldPath<FormValues> }) => {
			...
			return (
				<>
					...
					<button type="button" onClick={() => field.onInput('desktop')}>
						update-visibility
					</button>
				</>
			);
		...
		expect(latestValues?.content.children[0].data.setting.visibility).toBe('mobile');
		expect(latestValues?.content.children[1].data.setting.visibility).toBe('both');
		...
		expect(latestValues?.content.children[0].data.setting.visibility).toBe('mobile');
		expect(latestValues?.content.children[1].data.setting.visibility).toBe('desktop');
	...
	it('should update isValid when Controller with required rule re-mounts via checkbox toggle', async () => {
		...
		const InputField = ({ control, index }: { control: Control<FormValues>; index: number }) => (
			<Controller
				control={control}
	...
	it('should update the field value when a parent object is cleared with setValue', async () => {
		function App() {
			...
			return (
				<div>
					...
					<button type="button" onClick={() => setValue('data', { type: 'foo' })}>
						set
					</button>
		...
		await waitFor(() => expect(screen.getByText('controller:foo')).toBeVisible());
		...
		await waitFor(() => expect(screen.getByText('controller:undefined')).toBeVisible());
