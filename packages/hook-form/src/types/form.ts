// Vendored from react-hook-form@7.81.0 src/types/form.ts.
// octane: structural stand-ins for the React types upstream references.
// Events are NATIVE DOM events in octane (no synthetic layer): a native Event
// structurally satisfies BaseSyntheticEvent below (persist is React-only and
// optional — upstream itself guards `event.persist && event.persist()`).
// Renderables (upstream's ReactNode) are `unknown`.
...
// Upstream: React.FormHTMLAttributes<HTMLFormElement>. Octane has no attribute
// prop types — native <form> attributes pass through untyped.
type FormHTMLAttributes = Record<string, unknown>;
...
export type SubmitHandler<T> = (data: T, event?: BaseSyntheticEvent) => unknown | Promise<unknown>;
...
export type UseFormRegisterReturn<TFieldName extends InternalFieldName = InternalFieldName> = {
	// octane: upstream's `onChange` — native events make `input` the
	// per-keystroke event, so the handler is exposed under `onInput`.
 ...
 *
 * @returns onInput, onBlur, name, ref, and native HTML validation attributes if browser validation is enabled.
 *
 * @example
 * ```tsx
...
export type UseFormSetError<TFieldValues extends FieldValues> = (
	name: FieldPath<TFieldValues> | `root.${string}` | 'root' | 'form' | `form.${string}`,
	error: ErrorOption,
	options?: {
		shouldFocus: boolean;
	},
...
export type FormProps<TFieldValues extends FieldValues, TTransformedValues = TFieldValues> = Omit<
	FormHTMLAttributes,
	'onError' | 'onSubmit'
> &
	Partial<{
		...
		method: 'post' | 'put' | 'delete';
		children: unknown;
		render: (props: { submit: (e?: Event) => void }) => unknown;
		encType:
