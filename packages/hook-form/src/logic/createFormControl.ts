// Adapted from react-hook-form@7.88.0 src/logic/createFormControl.ts for Octane.
...
>(
	...
	const _setFieldArray: BatchFieldArrayUpdate = (
		...
		if (args && method && !_options.disabled) {
			...
			const fieldArrayErrors: (FieldError[] & { root?: FieldError }) | undefined = get(
				_formState.errors,
				name,
			);
		...
		if (!_options.disabled || shouldDirty === true) {
			if (!isBlurEvent || shouldDirty) {
				...
				if (isCurrentFieldPristine !== _formState.isDirty) {
					updateDirtyFields(
						...
						getDirtyFields(_defaultValues, _formValues, undefined, _fields) as Record<
							string,
							unknown
						>,
					);
				...
				shouldUpdateField =
					shouldUpdateField ||
					(_isTracked('dirtyFields') && isPreviousDirty !== !isCurrentFieldPristine);
			}
			...
			if (isBlurEvent) {
				...
				if (!isPreviousFieldTouched) {
					...
					shouldUpdateField =
						shouldUpdateField ||
						(_isTracked('touchedFields') && isPreviousFieldTouched !== isBlurEvent);
				}
			}
	...
	const shouldRenderByError = (
		...
		const shouldUpdateValid =
			_isTracked('isValid') && isBoolean(isValid) && _formState.isValid !== isValid;
		...
		(options.shouldDirty || options.shouldTouch) &&
			updateTouchAndDirty(
				...
				field &&
					field._f &&
					!field._f.disabled &&
					(field._f.valueAsNumber || field._f.valueAsDate || field._f.setValueAs)
	...
	const trigger: UseFormTrigger<TFieldValues> = async (
		...
		_subjects.state.next({
			...(!isString(name) || (_isTracked('isValid') && isValid !== _formState.isValid)
				? {}
				: { name }),
