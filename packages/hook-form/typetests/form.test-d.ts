...
import type { FieldError, FieldValues, Resolver } from '../src/types';
...
/** it should correctly infer the output type from a zod schema with a transform */ {
	/* eslint-disable react-hooks/rules-of-hooks */
	const form = useForm({
		resolver: mockZodResolver(z.object({ id: z.number().transform((val) => String(val)) })),
	});
...
export function mockZodResolver<Input extends FieldValues, Context, Output>(): Resolver<
	Input,
	Context,
	Output | Input
> {
