import type { DeepPartial, ExtractObjects, IsAny, IsNever } from '../src/types';
...
/** {@link ExtractObjects} */ {
	/** it should extract all objects from a union */ {
		...
		>;
		type _t = Expect<Equal<typeof actual, { x: string } | { y: number; z: { w: number } }>>;
	}
}
...
/** {@link DeepPartial} */ {
	/** it should make all nested properties optional */ {
		...
		type _t = Expect<Equal<typeof actual, { x?: string; y?: number; z?: { w?: boolean } }>>;
	}
	...
	/** it should be assignable for types containing unknown */ {
		...
		type _t = Expect<typeof actual extends DeepPartial<{ x: unknown }> ? true : false>;
	}
