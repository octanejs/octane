/**
 * Precompile Sonner's differential fixtures for React. The same `.tsrx`
 * source is loaded by Octane in the test project and rewritten to use the
 * published `sonner@2.0.7` package on the React side.
 */
...
function compileOne(sourcePath: string): void {
	...
	const rewritten = transformed.code
		.replace(/from\s+["']@octanejs\/sonner["']/g, 'from "sonner"')
...
export async function setup(): Promise<void> {
	...
	compileOne(join(FIXTURE_DIR, 'sonner-diff.tsrx'));
}
