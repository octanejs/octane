...
describe('@octanejs/aria/components — NumberField', () => {
	...
	it('does not increment a number field for Android IME composition keystrokes', async () => {
		const r = mount(NumberFieldScenario);
		await act(() => {});
		const input = r.container.querySelector('input.react-aria-Input') as HTMLInputElement;
		await act(() => {
		...
		expect(input.value).toBe('5');
		expect((r.container.querySelector('input[name="amount"]') as HTMLInputElement).value).toBe('5');
	...
	it('does not commit a draft number from an Android composition-confirmation Enter', async () => {
		const r = mount(NumberFieldScenario);
		await act(() => {});
		const input = r.container.querySelector('input.react-aria-Input') as HTMLInputElement;
		const hidden = r.container.querySelector('input[name="amount"]') as HTMLInputElement;
		...
		expect(hidden.value).toBe('10');
		r.unmount();
	});

	it('tolerates compositionend when the formatted field missed compositionstart', async () => {
		const r = mount(NumberFieldScenario);
		await act(() => {});
		const input = r.container.querySelector('input.react-aria-Input') as HTMLInputElement;
		const errors: string[] = [];
