// Ported from react-aria (source: .react-spectrum/packages/react-aria/src/selection/ListKeyboardDelegate.ts).
// Verbatim (no React surface); RefObject type from @react-types/shared. Two strict-TS
// adjustments with identical runtime behavior: the CardView layout-delegate escape hatch
// indexes through a structural type (upstream indexes LayoutDelegate, which doesn't declare
// getKeyLeftOf/getKeyRightOf), and its possibly-null key lands in a local instead of
// reassigning the non-null `key` parameter.
import type {
	Collection,
	Direction,
	DisabledBehavior,
...
// Optional spatial methods some layout delegates (e.g. CardView's) implement beyond the
// LayoutDelegate interface.
type DirectionalLayoutDelegate = LayoutDelegate & {
...
export class ListKeyboardDelegate<T> implements KeyboardDelegate {
	...
	private layoutDelegate: DirectionalLayoutDelegate;
	...
	getKeyRightOf?(key: Key, options?: { includeDisabled?: boolean }): Key | null {
		...
		let layoutDelegateMethod =
			this.direction === 'ltr' ? ('getKeyRightOf' as const) : ('getKeyLeftOf' as const);
		if (this.layoutDelegate[layoutDelegateMethod]) {
			let nextKey: Key | null = this.layoutDelegate[layoutDelegateMethod]!(key);
			return this.findNextNonDisabled(
				nextKey,
	...
	getKeyLeftOf?(key: Key, options?: { includeDisabled?: boolean }): Key | null {
		let layoutDelegateMethod =
			this.direction === 'ltr' ? ('getKeyLeftOf' as const) : ('getKeyRightOf' as const);
		if (this.layoutDelegate[layoutDelegateMethod]) {
			let nextKey: Key | null = this.layoutDelegate[layoutDelegateMethod]!(key);
			return this.findNextNonDisabled(
				nextKey,
