// Ported from react-aria (source: .react-spectrum/packages/react-aria/src/grid/GridKeyboardDelegate.ts).
// octane adaptations:
// - `IGridCollection`/`GridNode` come from the ported stately grid collection and
//   `getChildNodes`/`getFirstItem`/`getLastItem`/`getNthItem` from the ported stately
//   collections helpers; `DOMLayoutDelegate` from the ported selection area.
