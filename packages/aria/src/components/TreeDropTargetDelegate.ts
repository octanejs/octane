// Ported from react-aria-components (source: .react-spectrum/packages/react-aria-components/src/TreeDropTargetDelegate.ts).
// The geometry/target-resolution logic is verbatim. All referenced types
// (Direction/DropTarget/DropTargetDelegate/ItemDropTarget/RootDropTarget) are the
// real @react-types/shared types, exactly as upstream. PHASE-7: this delegate is
// only exercised through consumer-provided `dragAndDropHooks` (the dnd engine),
// which cannot be constructed until `useDragAndDrop` is ported — Tree wires it up
// in its inert dnd branch.
import type {
	Direction,
	DropTarget,
	DropTargetDelegate,
