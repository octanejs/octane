// Per packages/wouter/upstream/canonical/test/use-route.test.tsx
import { createElement as h, type ComponentBody, type OctaneNode } from 'octane';
...
function createRouterWrapper(
	options: Omit<RouterProps, 'children'>,
): ComponentBody<{ children: OctaneNode }> {
...
function assertRoute(
	...
	location: string,
	expected: false | Match | Record<string, string | undefined>,
) {
	...
	if (expected === false) {
		...
		expect(result.current).toStrictEqual([true, expected]);
	}
}
...
it('is case insensitive', () => {
	...
	assertRoute('/Users/:Name', '/users/alex', {
		0: 'alex',
		Name: 'alex',
	});
});
...
it('supports required segments', () => {
	...
	assertRoute('/root/payments/:id/refunds/:refId', '/root/payments/1/refunds/2', [
		true,
		{ 0: '1', 1: '2', id: '1', refId: '2' },
	]);
});
...
it('supports optional wildcards', () => {
	assertRoute('/app/*?', '/app/blog/mac', {
		0: 'blog/mac',
		'*': 'blog/mac',
	});
	assertRoute('/app/*?', '/app', { 0: undefined, '*': undefined });
	assertRoute('/app/*?/dashboard', '/app/v1/dashboard', {
		0: 'v1',
		'*': 'v1',
	});
...
it('supports other characters in segments', () => {
	assertRoute('/users/:name', '/users/1-alex', {
		0: '1-alex',
		name: '1-alex',
	});
...
it('reacts to pattern updates', () => {
	const { result, rerender } = renderHook(({ pattern }: { pattern: string }) => useRoute(pattern), {
		wrapper: createRouterWrapper({
			hook: memoryLocation({
				path: '/blog/products/40/read-all',
				static: true,
			}).hook,
		}),
	...
	expect(result.current).toStrictEqual([
		true,
		{
			0: 'products/40/read-all',
			'*': 'products/40/read-all',
		},
	]);
});
...
it('reacts to location updates', () => {
	...
	const { result } = renderHook(() => useRoute('/cities/:city?'), {
		wrapper: createRouterWrapper({ hook }),
	});
	...
	expect(result.current).toStrictEqual([false, null]);
	act(() => navigate('/cities'));
	expect(result.current).toStrictEqual([true, { 0: undefined, city: undefined }]);
});
