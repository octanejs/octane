 ...
 */
const _relativePath = (base: string, path: string) =>
...
export const absolutePath = (to: string, base?: string) =>
	to[0] === '~' ? to.slice(1) : baseDefaults(base) + to;

export const relativePath = (base = '', path: string) =>
...
export const sanitizeSearch = (search: string) => unescape(stripQm(search));
