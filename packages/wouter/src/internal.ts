...
export function subSlot(slot: symbol | undefined, tag: string): symbol {
	if (slot === undefined) {
		...
		if (bare === undefined) {
			bare = Symbol.for(`@octanejs/wouter:${tag}`);
	...
	if (child === undefined) {
		child = Symbol.for(`${slot.description ?? ''}:wouter:${tag}`);
		byTag.set(tag, child);
	}
	return child;
}
...
export function optionAndSlot<T extends object>(
	first: T | symbol | undefined,
	second: symbol | undefined,
): [T, symbol | undefined] {
	return typeof first === 'symbol' ? ([{}, first] as [T, symbol]) : [first ?? ({} as T), second];
}
