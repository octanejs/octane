/**
 * Vitest globalSetup for the `jotai` project — the jotai analogue of octane's
 * differential precompile. Runs ONCE in pure Node before any test loads,
 * compiles every `.tsrx` fixture under `packages/jotai/tests/_fixtures`
 * through `@tsrx/react` + esbuild, and writes the React-runtime JS into the
 * SHARED differential cache that octane's `_rig.ts` reads from — so the jotai
 ...
 * the REAL jotai React binding (the byte-for-byte oracle). The public API
 * matches 1:1 (atom/createStore + Provider/useAtom/useAtomValue/useSetAtom),
 * so a flat import rewrite is all it takes.
 */
...
function compileOne(srcPath: string): void {
	...
	let rewritten = transformed.code
		.replace(/from\s+["']@octanejs\/jotai(\/[^"']*)?["']/g, (_m, sub) => `from "jotai${sub || ''}"`)
