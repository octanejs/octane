...
export default function domToReact(
	...
	const reactElements: ElementDescriptor[] = [];
	...
	for (let index = 0; index < nodesLength; index++) {
		...
		if (hasReplace) {
			let replaceElement = options.replace?.(node, index) as ElementDescriptor;
			...
			if (isValidElement(replaceElement)) {
				...
				reactElements.push(transform(replaceElement, node, index) as ElementDescriptor);
		...
		if (node.type === 'text') {
			...
			reactElements.push(transform(node.data, node, index) as ElementDescriptor);
			continue;
		}
		...
		reactElements.push(
			transform(createElement(node.name, props, children), node, index) as ElementDescriptor,
		);
	}
