 ...
 */
function normalizeAcceptGroups(
	accept?: Accept | readonly AcceptGroup[],
): AcceptGroup[] | undefined {
	if (!isDefined(accept)) {
		return undefined;
	}
	if (Array.isArray(accept)) {
		return (accept as readonly AcceptGroup[]).filter(
			(group) => isDefined(group) && isDefined(group.accept),
		);
	}
...
export function acceptPropAsAcceptAttr(
	accept?: Accept,
	{
		omitWildcardMimeTypesWithExtensions = false,
	}: { omitWildcardMimeTypesWithExtensions?: boolean } = {},
): string | undefined {
	if (isDefined(accept)) {
		return (
