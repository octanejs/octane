...
describe('fileMatchSize()', () => {
	...
	let utils;
	beforeEach(async () => {
		utils = await import('../../../src/utils/index');
	});
...
describe('isEvtWithFiles()', () => {
	...
	it('should return false if items contain no file kind and types omit Files', () => {
		...
		expect(utils.isEvtWithFiles({ dataTransfer: { types: ['text/plain'], items: [] } })).toBe(
			false,
		);
	});
...
describe('getTooLargeRejectionErr()', () => {
	...
	it('formats larger sizes using human-readable units', () => {
		...
		expect(utils.getTooLargeRejectionErr(5 * 1024 * 1024).message).toEqual(
...
describe('getTooSmallRejectionErr()', () => {
	...
	it('formats larger sizes using human-readable units', () => {
		...
		expect(utils.getTooSmallRejectionErr(5 * 1024 * 1024).message).toEqual(
...
describe('getDragVerdict()', () => {
	...
	it("returns 'accept' when every file passes the drag-evaluable checks", () => {
		...
		expect(
			utils.getDragVerdict({ files: pdfs(), multiple: true, accept: 'application/pdf' }),
		).toEqual('accept');
	});
	...
	it("returns 'reject' when a non-empty MIME type doesn't match {accept}", () => {
		expect(utils.getDragVerdict({ files: pdfs(), multiple: true, accept: 'image/*' })).toEqual(
			'reject',
		);
	});
	...
	it("prefers a confident 'reject' over 'unknown' even with a validator", () => {
		...
		expect(
			utils.getDragVerdict({ files: pdfs(), multiple: true, accept: 'image/*', validator }),
		).toEqual('reject');
	...
	it("treats empty-type DataTransferItems as 'accept', or 'unknown' when a validator is set", () => {
		...
		expect(
			utils.getDragVerdict({ files: emptyTypeItems, multiple: true, accept: 'text/markdown,.md' }),
		).toEqual('accept');
		expect(
			utils.getDragVerdict({
				files: emptyTypeItems,
				multiple: true,
...
describe('pickerOptionsFromAccept()', () => {
	...
	it('falls back to the MIME types when a group has no extensions', () => {
		expect(
			utils.pickerOptionsFromAccept([{ accept: { 'image/jpeg': [], 'image/png': [] } }]),
		).toEqual([
	...
	it('accepts a bare string extension value, matching showOpenFilePicker', () => {
		...
		).toEqual([{ description: 'PDF', accept: { 'application/pdf': ['.pdf'] } }]);
	});
...
describe('acceptPropAsAcceptAttr()', () => {
	...
	describe('with {omitWildcardMimeTypesWithExtensions}', () => {
		...
		it('drops a wildcard MIME type when it is paired with extensions', () => {
			expect(utils.acceptPropAsAcceptAttr({ 'image/*': ['.jpeg', '.png'] }, opts)).toEqual(
				'.jpeg,.png',
			);
		});
