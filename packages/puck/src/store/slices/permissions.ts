...
export const createPermissionsSlice = (
	...
	const resolvePermissions: ResolvePermissions = async (params = {}, force) => {
		...
		const resolveDataForItem = async (item: ComponentData, force: boolean = false) => {
			...
			if (componentConfig.resolvePermissions) {
				...
				if (Object.values(changed).some((el) => el === true) || force) {
					...
					const resolvedPermissions = await componentConfig.resolvePermissions(item, {
						changed,
	...
	const refreshPermissions: RefreshPermissions = (params) => resolvePermissions(params, true);
	...
	return {
		...
		getPermissions: ({ item, type, root } = {}) => {
			...
			if (item) {
				...
				return (
					resolvedForItem ? { ...globalPermissions, ...resolvedForItem } : initialPermissions
				) as Permissions;
			} else if (type) {
