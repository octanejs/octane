...
export const useContentIdsWithPreview = (
	...
	const preview = useContextStore(ZoneStoreContext, (s) => s.previewIndex[zoneCompound]);
	...
	const updateContent = useRenderedCallback(
		(
			...
			if (preview) {
				...
				setContentIdsWithPreview(
					previewExists ? contentIds.filter((id) => id !== draggedItemId) : contentIds,
				);
			}
	...
	useEffect(() => {
		...
		updateContent(contentIds, preview, isDragging, draggedItemId, previewExists);
