 ...
 * ```
 */
export function fastComponentRef<P extends object, E extends Element, R extends React.OctaneNode>(
	fn: (props: React.PropsWithoutRef<P>, forwardedRef: React.Ref<E>) => R,
) {
	// OCTANE DIVERGENCE[ref-as-prop][ordinary:base-ui-utils-native-refs]: the ref travels with the other props.
	return fastComponent(function RefComponent(
		props: React.PropsWithoutRef<P> & React.RefAttributes<E>,
	) {
		const { ref = null, ...componentProps } = props;
		return fn(componentProps as React.PropsWithoutRef<P>, ref);
	});
}
