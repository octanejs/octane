/**
 * Precompile GSAP differential fixtures for React. The same `.tsrx` source is
 * loaded by Octane in the test project and rewritten to import the pinned
 * `@gsap/react` 2.1.2 oracle on the React side.
 */
...
function compileOne(sourcePath: string): void {
	...
	const rewritten = transformed.code
		.replace(/from\s+["']@octanejs\/gsap["']/g, 'from "@gsap/react"')
...
function walk(directory: string): string[] {
	...
	for (const name of readdirSync(directory)) {
		...
		} else if (fullPath.endsWith('.tsrx')) {
			files.push(fullPath);
		}
	}
	return files;
