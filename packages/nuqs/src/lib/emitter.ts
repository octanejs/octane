export type Emitter<Events extends Record<string, unknown>> = {
	on<Key extends keyof Events>(type: Key, handler: (event: Events[Key]) => any): () => void;
	off<Key extends keyof Events>(type: Key, handler?: (event: Events[Key]) => any): void;
...
export function createEmitter<Events extends Record<string, unknown>>(): Emitter<Events> {
	const all: Map<keyof Events, Array<(event: Events[keyof Events]) => any>> = new Map();
	return {
		on<Key extends keyof Events>(type: Key, handler: (event: Events[Key]) => any): () => void {
