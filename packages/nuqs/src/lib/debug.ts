...
export type DebugSink = (code: DebugCode, args: unknown[], isWarn?: boolean) => void;
...
export function debug<Code extends DebugCode>(code: Code, ...args: DebugArgs<Code>): void {
	sink?.(code, args);
}

export function warn<Code extends DebugCode>(code: Code, ...args: DebugArgs<Code>): void {
	sink?.(code, args, true);
}
