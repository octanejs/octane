// Ported from react's `TransitionStartFunction`. Octane's `useTransition`
// returns a function of this shape (a Promise-returning scope turns the
// transition into an async action). Declared locally so the framework-agnostic
// core carries no `react` type dependency — the octane binding never imports
// `@types/react`.
export type TransitionStartFunction = (callback: () => void | Promise<void>) => void;
