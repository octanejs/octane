...
export function isParserBijective<T>(
	...
	if (parser.type === 'multi' && Array.isArray(serialized)) {
		...
		throw new Error(`[nuqs] isParserBijective: mismatched parser type and serialized value type`);
	}
	// Test value equality
	...
	const parsed =
		parser.type === 'multi' && Array.isArray(input)
			...
			: parser.type !== 'multi' && typeof input === 'string'
				...
				: null;
	if (parsed === null) {
		throw new Error(`[nuqs] testParseThenSerialize: parsed value is null (when parsing ${input})`);
	}
