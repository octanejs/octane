	...
	return createParser({
		parse: (query) => {
			...
			return query
				.split(separator)
				.map((item, index) =>
					safeParse(itemParser.parse, item.replaceAll(encodedSeparator, separator), `[${index}]`),
				)
...
type inferParserRecordType<Map extends Record<string, GenericParserBuilder<any>>> = {
