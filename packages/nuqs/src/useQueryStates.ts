...
export function useQueryStates<KeyMap extends UseQueryStatesKeysMap>(
	...
	const resolvedUrlKeys = useMemo(
		() => Object.fromEntries(Object.keys(keyMap).map((key) => [key, urlKeys[key] ?? key])),
				...
				handlers[stateKey as keyof KeyMap] = ({ state, query }: CrossHookSyncPayload) => {
					setInternalState((currentState) => {
						...
						if (Object.is(currentValue, nextValue)) {
							debug(2, hookId, stateKeys, urlKey, state, defaultValue, stateRef.current);
						...
						debug(3, hookId, stateKeys, urlKey, state, defaultValue, stateRef.current);
		...
		(stateUpdater, callOptions = {}) => {
			...
			const newState: Partial<Nullable<KeyMap>> =
				typeof stateUpdater === 'function'
					? (stateUpdater(applyDefaultValues(stateRef.current, defaultValues)) ?? nullMap)
			...
			let doFlush = false;
			const debounceAborts: Array<(p: Promise<URLSearchParams>) => Promise<URLSearchParams>> = [];
			for (let [stateKey, value] of Object.entries(newState)) {
				...
				const update: UpdateQueuePushArgs = {
					...
					options: {
						...
						startTransition:
							callOptions.startTransition ?? parser.startTransition ?? startTransition,
					},
				};
				const resolvedLimitUrlUpdates =
					callOptions.limitUrlUpdates ?? parser.limitUrlUpdates ?? limitUrlUpdates;
	...
	const state = Object.entries(keyMap).reduce((out, [stateKey, parser]) => {
		...
		const query =
			queuedQuery === undefined
				? ((parser.type === 'multi' ? searchParams.getAll(urlKey) : searchParams.get(urlKey)) ??
					fallbackValue)
				: queuedQuery;
