...
import { mergeForm, useForm } from '@octanejs/tanstack-form';
...
describe('useForm', () => {
	...
	it("should set field errors from the field's validators", async () => {
		function Comp() {
			const form = useForm({
				...
				validators: {
					...
					onBlur: ({ value }) => {
						if (value.firstName === 'other') return 'Please enter a different value (onBlurError)';
						return undefined;
					},
				},
			});
						...
						children={(field) => (
							<div>
								<input
									...
									onInput={(e) => field.handleChange(e.target.value)}
								/>
								<p data-testid={'field-onchange'}>{field.state.meta.errorMap.onChange}</p>
							</div>
						)}
					/>
		...
		await user.type(input, 'other');
		expect(getByTestId('form-onchange')).toHaveTextContent('Something went wrong');
	...
	it("should set field errors from the form's onChangeAsync validator", async () => {
						...
						children={(field) => (
							<div>
								<input
									...
									onInput={(e) => field.handleChange(e.target.value)}
								/>
								<p data-testid="field-error">{field.state.meta.errorMap.onChange}</p>
							</div>
						)}
					/>
		...
		expect(firstNameErrorElement).toHaveTextContent('First name cannot be "other"');
	...
	it("should set errors on the fields from the form's onChange validator", async () => {
					...
					<form.Field
						...
						children={(field) => (
							<>
								...
								<p data-testid="field-error">{field.state.meta.errors.join('')}</p>
							</>
						)}
					/>
		...
		expect(queryByText('Please enter a different value')).not.toBeInTheDocument();
		expect(queryByText('Something went wrong')).not.toBeInTheDocument();

		await user.type(input, 'Tom');
		await waitFor(() => expect(fieldError.textContent).toBe('Please enter a different value'));
		await waitFor(() => expect(formError.textContent).toBe('Something went wrong'));
	});
		...
		function Comp() {
			...
			return (
				...
				>
					...
					/>
					<button type="button" data-testid="remove" onClick={() => form.deleteField('firstName')}>
						remove first name
					</button>
				</form>
			);
	...
	it('should handle stable transforms to update the base form on subsequent renders', async () => {
		function Comp() {
			...
			return (
				<div>
					<form.Field name="test" children={(field) => <p>{field.state.value}</p>} />
		...
		function Comp() {
			...
			return (
				<>
					...
					<form.Subscribe
						children={(state) => <span data-testid="state-value">{state.values.name}</span>}
					/>
				</>
			);
		}
