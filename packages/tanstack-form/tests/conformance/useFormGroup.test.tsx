...
describe('form.FormGroup', () => {
	...
	it('should expose group state value reactively', async () => {
				...
				<form.FormGroup name="step1">
					{(group) => (
						<>
							...
							/>
							<pre data-testid="group-value">{JSON.stringify(group.state.value)}</pre>
						</>
					)}
				</form.FormGroup>
			);
		...
		await waitFor(() => expect(getByTestId('group-value').textContent).toBe('{"name":"updated"}'));
	});
	...
	it('should call onGroupSubmitInvalid when group-level validation fails', async () => {
		...
		expect(onSubmit).not.toHaveBeenCalled();
		await waitFor(() => expect(getByTestId('group-error').textContent).toBe('Name is required'));
	});
