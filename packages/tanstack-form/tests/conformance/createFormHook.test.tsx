...
import { createFormHook, createFormHookContexts, useStore } from '@octanejs/tanstack-form';
...
function TextField({ label }: { label: string }) {
	...
	return (
		<label>
			<div>{label}</div>
			<input value={field.state.value} onInput={(e) => field.handleChange(e.target.value)} />
		</label>
	);
}
...
describe('createFormHook', () => {
	...
	it('should not lose focus on update with withFieldGroup', async () => {
		...
		const ChildForm = withFieldGroup({
			...
			render: function Render({ group }) {
				const firstName = useStore(group.store, (state) => state.values.firstName);
				return (
					<div>
						<p>{firstName}</p>
						<group.Field
	...
	it('should allow nesting withFieldGroup in other withFieldGroups', () => {
		...
		const LensNested = withFieldGroup({
			...
			render: function Render({ group }) {
				return <group.Field name="firstName">{(field) => <p>{field.name}</p>}</group.Field>;
			},
		});
		...
		const FieldGroupObject = withFieldGroup({
			...
			render: function Render({ group }) {
				return (
					...
					>
						{(field) => {
							expect(field.options.validators?.onChangeListenTo).toStrictEqual(['userPassword']);
							expect(field.options.validators?.onBlurListenTo).toStrictEqual([
								'userConfirmPassword',
							]);
							return <></>;
	...
	it('should allow using typed app form', () => {
		...
		function Child() {
			...
			return (
				<form.AppField name="firstName" children={(field) => <field.TextField label="Testing" />} />
			);
		}
	...
	describe('extendForm', () => {
			...
			function Comp() {
				...
				return (
					<>
						...
						<form.AppField
							name="lastName"
							children={(field) => <field.ExtendedTextField label="Last Name" />}
						/>
					</>
				);
			}
		...
		it('should include both parent and extended form components', () => {
			function ExtendedSubmit({ label }: { label: string }) {
				...
				return (
					<form.Subscribe selector={(state) => state.isSubmitting}>
						{(isSubmitting) => (
							<button data-testid="extended-submit" disabled={isSubmitting} type="button">
								{label}
							</button>
						)}
					</form.Subscribe>
			...
			expect(getByTestId('extended-submit')).toHaveTextContent('Extended Submit');
		});
		...
		it('should support chaining multiple extendForm calls', () => {
			...
			function Comp() {
				...
				return (
					<>
						<form.AppField name="a" children={(field) => <field.TextField label="A" />} />
						<form.AppField name="b" children={(field) => <field.FieldA label="B" />} />
						<form.AppField name="c" children={(field) => <field.FieldB label="C" />} />
					</>
				);
			}
		...
		it('should work with withForm after extendForm', () => {
			...
			const { useAppForm: useExtendedForm, withForm: withExtendedForm } = createFormHook({
				fieldComponents: { TextField },
				formComponents: { SubscribeButton },
				fieldContext,
				formContext,
