...
describe('createFormHook', () => {
	it('should not break with an infinite type on large schemas', () => {
		...
		const GeneratedTypes0Visibility_Names = ['Normal', 'Advanced', 'Hidden'] as const;
		type GeneratedTypes0Visibility = (typeof GeneratedTypes0Visibility_Names)[number];
	...
	it('component made from withForm should have its props properly typed', () => {
		...
		const CorrectComponent = <WithFormComponent form={appForm} prop1="test" prop2={10} />;
	...
	it('should infer subset values and props when calling withFieldGroup', () => {
		...
		const FormGroupComponent = withFieldGroup({
			...
			render: function Render({ group, children, ...props }) {
				...
				expectTypeOf(children).toEqualTypeOf<unknown>();
				expectTypeOf(props).toEqualTypeOf<{}>();
				return <group.Test />;
			},
		});
	...
	it('should allow spreading formOptions when calling withFieldGroup', () => {
		...
		const UnknownFormGroupComponent = withFieldGroup({
			...
			render: function Render({ group }) {
				...
				expectTypeOf(group.handleSubmit).parameters.toEqualTypeOf<[] | [{ foo: string }]>();
				return <group.Test />;
			},
		});
	});
	...
	it('should allow passing compatible forms to withFieldGroup', () => {
		...
		const CorrectComponent1 = (
			<FormGroup form={extendedAppForm} fields="person" prop1="Test" prop2={10} />
		);
	...
	it('should require strict equal submitMeta if it is set in withFieldGroup', () => {
		...
		const FormLensNoMeta = withFieldGroup({
			...
			render: function Render({ group }) {
				...
				expectTypeOf(group.handleSubmit).parameters.toEqualTypeOf<[] | [submitMeta: never]>();
		...
		const CorrectComponent1 = <FormLensNoMeta form={noMetaForm} fields="person" />;
		...
		const CorrectComponent3 = <FormGroupWithMeta form={metaForm} fields="person" />;
		...
		const CorrectComponent4 = <FormLensNoMeta form={diffMetaForm} fields="person" />;
	...
	it('should not allow withFieldGroups with different metas to be nested', () => {
		...
		it('should not allow fields mapping if the top level is an array', () => {
			...
			const FieldGroupRecord = withFieldGroup({
				defaultValues: { anything: defaultFields } as Record<string, typeof defaultFields>,
				render: function Render() {
					return <></>;
				},
			});
			...
			const CorrectComponent1 = <FieldGroupRecord form={form} fields="relativesRecord" />;
			const WrongComponent1 = (
				<FieldGroupRecord
					form={form}
			...
			const CorrectComponent3 = <FieldGroupArray form={form} fields="relatives" />;
			const WrongComponent2 = (
				<FieldGroupArray
					form={form}
	...
	it('should allow interfaces without index signatures to be assigned to `props` in withForm and withFormGroup', () => {
		...
		const Component2 = <WithFormComponent2 title="" something="else" form={appForm} />;

		const FieldGroupComponent1 = (
			<WithFieldGroupComponent1 title="" form={appForm} fields={{ name: 'name' }} />
		);
