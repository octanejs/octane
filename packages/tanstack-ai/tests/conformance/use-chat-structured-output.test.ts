...
describe('useChat({ outputSchema }) — runtime', () => {
	...
	it("preserves each turn's structured part across multi-turn history", async () => {
		...
		function streamFor(p: Person, runId: string, messageId: string): Array<StreamChunk> {
		...
		const adapter = {
			async *connect() {
				const chunks =
					call === 0 ? streamFor(personA, 'run-a', 'msg-a') : streamFor(personB, 'run-b', 'msg-b');
				call++;
				for (const chunk of chunks) yield chunk;
			},
		};
		...
		const partA = assistants[0]!.parts.find((p) => p.type === 'structured-output');
		const partB = assistants[1]!.parts.find((p) => p.type === 'structured-output');
	...
	it("invokes the user's onChunk callback alongside internal tracking", async () => {
		...
		const deltaCalls = onChunk.mock.calls.filter(([c]) => c.type === 'TEXT_MESSAGE_CONTENT');
...
describe('useChat() without outputSchema — runtime', () => {
	it('does not break or track structured state when no schema is supplied', async () => {
		...
		const runtimeView: { partial?: unknown; final?: unknown } = result.current as unknown as {
			partial?: unknown;
			final?: unknown;
		};
