...
describe('@octanejs/tanstack-router — event lifecycle', () => {
	it('emits onLoad → onBeforeRouteMount → onResolved → onRendered on navigation', async () => {
		...
		const events: string[] = [];
		const unsubs = ['onLoad', 'onBeforeRouteMount', 'onResolved', 'onRendered'].map((type) =>
		...
		expect(events.indexOf('onResolved')).toBeLessThan(events.indexOf('onRendered'));
...
describe('@octanejs/tanstack-router — route error boundaries', () => {
	...
	it('falls back to defaultErrorComponent when the route has none', async () => {
		setThrow(true);
		const router = makeLifecycleRouter('/boom', { routeError: false, defaultError: true });
		await router.load();
		const r = mount(RouterProvider as any, { router });
		await flush();
...
describe('@octanejs/tanstack-router — match status handling', () => {
...
describe('@octanejs/tanstack-router — scroll restoration restore', () => {
