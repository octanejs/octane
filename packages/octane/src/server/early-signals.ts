...
import { NATIVE_HYDRATION_CAPTURE_KEY } from '../hydration/native-intent.js';
import {
...
let INDEPENDENT_HYDRATION_FORM_BOOTSTRAP_JS: string | undefined;
...
export function streamedSignalBootstrapJs(
	...
	const independent =
		(formSubmissions
			? INDEPENDENT_HYDRATION_FORM_BOOTSTRAP_JS
			: INDEPENDENT_HYDRATION_BOOTSTRAP_JS) ??
		...
		'(function(d){var h=' +
			...
			(formSubmissions
				? 'var native=d[' +
					JSON.stringify(NATIVE_HYDRATION_CAPTURE_KEY) +
					'];if(native&&native.skip(e))return;'
				: '') +
			'var b=t.closest(is);' +
			...
			(formSubmissions ? 'v[9]=native.next();' : '') +
			'if(g&&p&&' +
			(formSubmissions ? 'v[9]===p[9]+1&&' : '') +
			'p[2]===b&&p[7]===g&&p[6].isConnected&&p[6].contains(p[1])&&p[6].type==="button"&&p[6].getAttribute("' +
			HYDRATE_SELECTION_ATTR +
	...
	if (formSubmissions) INDEPENDENT_HYDRATION_FORM_BOOTSTRAP_JS = independent;
	else INDEPENDENT_HYDRATION_BOOTSTRAP_JS = independent;
	return formSubmissions
...
function formSubmissionBootstrapJs(): string {
	return (FORM_SUBMISSION_BOOTSTRAP_JS ??=
		...
		'E=w.Element||Element,H=w.HTMLFormElement||HTMLFormElement,N=w.Node||Node,np=N.prototype,' +
		'od=Object.getOwnPropertyDescriptor(np,"ownerDocument").get,pe=Object.getOwnPropertyDescriptor(np,"parentElement").get,' +
		'ic=Object.getOwnPropertyDescriptor(np,"isConnected").get,nt=Object.getOwnPropertyDescriptor(np,"nodeType").get,' +
		...
		',nk=' +
		JSON.stringify(NATIVE_HYDRATION_CAPTURE_KEY) +
		',ls=new Map,rs=new WeakMap,seen=new WeakSet,dead=false,depth=0,flushing=false,sequence=0;' +
		'd[nk]={next:function(){return ++sequence;},skip:function(e){var t=e.target;if(e.type!=="click"||!t||nt.call(t)!==1)return false;var c=E.prototype.closest.call(t,"button,input"),f=c&&c.form;return !!(c&&(c.localName==="button"?c.type==="submit":c.type==="submit"||c.type==="image")&&f&&ga.call(f,a));}};' +
		'function attr(e,n){return ga.call(e,n);}function prop(f,n){return Object.getOwnPropertyDescriptor(fp,n).get.call(f);}' +
		'function current(r){return r.event.target===r.form&&od.call(r.form)===d&&ic.call(r.form)&&pe.call(r.form)===r.parent&&attr(r.form,a)===r.key&&' +
		...
		'function activate(r){if(u.activate){u.activate(r);return;}if(!current(r))return;var f=r.form,b=r.boundary;if(!b)return;var id=r.boundaryId,when=r.boundaryWhen,h=d[hk];if(id===null||(when!=="interaction"&&when!=="dynamic")||!h||h.q.length>=' +
		EARLY_HYDRATION_INTENTS_LIMIT +
		')return;var p=[r.event,f,b,id,when,r.boundaryEvents,undefined,undefined,true,r.sequence];if(h.capture)h.capture(p);else h.q.push(p);}' +
		...
		'var l=ls.get(f);rs.set(f,l?l.key:attr(f,a));clear(f);clearActivations(f);for(var i=u.q.length-1;i>=0;i--)if(u.q[i].form===f){u.q[i].discarded=true;u.q[i].snapshot=undefined;u.q.splice(i,1);}}' +
		'var u=d[k]={version:1,q:[],captures:function(f){var key=attr(f,a);return !dead&&od.call(f)===d&&ic.call(f)&&!!key&&rs.get(f)!==key;},' +
		'has:function(e){return seen.has(e);},release:release,flush:function(){if(dead||depth||flushing||!u.receive)return;flushing=true;' +
		'try{for(var i=0;i<u.q.length;){if(!u.receive)break;var r=u.q[i];if(!r.snapshot){i++;continue;}if(!current(r)){r.discarded=true;r.snapshot=undefined;u.q.splice(i,1);clearActivations(r.form,r.event);prune(r.form);continue;}' +
		'var accepted;try{accepted=u.receive(r);}catch(error){release(r.form);throw error;}' +
		'if(accepted){r.snapshot=undefined;if(r.discarded)clearActivations(r.form,r.event);var at=u.q.indexOf(r);if(at!==-1)u.q.splice(at,1);prune(r.form);}else if(u.q[i]===r)i++;}}finally{flushing=false;}},' +
		...
		'ls.forEach(function(l){clearTimeout(l.timer);});ls.clear();clearActivations();u.q.forEach(function(r){r.discarded=true;r.snapshot=undefined;});u.q.length=0;u.receive=undefined;u.activate=undefined;}};' +
		...
		'if(l&&l.key!==key){clear(f);clearActivations(f);for(var j=u.q.length-1;j>=0;j--)if(u.q[j].form===f){u.q[j].discarded=true;u.q[j].snapshot=undefined;u.q.splice(j,1);}l=null;}' +
		'if(!l){l={key:key,timer:setTimeout(function(){release(f);},' +
		EARLY_FORM_SUBMISSIONS_TIMEOUT_MS +
		')};ls.set(f,l);}var b=E.prototype.closest.call(f,is),r={event:e,form:f,key:key,parent:pe.call(f),boundary:b,boundaryId:b?attr(b,ia):null,boundaryWhen:b?attr(b,"' +
		HYDRATE_WHEN_ATTR +
		'"):null,boundaryEvents:b?attr(b,"' +
		HYDRATE_INTERACTION_EVENTS_ATTR +
		'"):null,sequence:d[nk].next(),snapshot:undefined};' +
