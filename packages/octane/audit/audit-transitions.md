...
Citations to specific upstream React tests: **5** (#7, #8, #9, #10, #11) — all in
the "multiple-suspend edge cases" block. These were explicitly ported from React.

Tests without an exact upstream line (canonical-but-uncited): **6** (#1–#6) —
...
1. **`isStale` returned via fixture-level identity check** (test #5). The pattern
   ...
   test in `ReactDeferredValue-test.js`. The exact ordering — prior value in the
   SAME render that sees the new prop (no intermediate transient render) — is a
...
2. **`isPending` set in the SAME commit as the transition setter's state change**
   ...
   pending"; the SAME-COMMIT timing — i.e. that `n` and `isPending` both become
   visible to the DOM in one synchronous batch — is an Ripple implementation
...
3. **Urgent-supersede with pre-tagged fulfilled thenable** (test #10). The test
   ...
   that an urgent render commits **synchronously** with C visible — without
   waiting for a microtask — and that the transition's outstanding counter then
...
5. **Entangled transition: partial resolution leaves OLD sibling DOM intact**
   ...
   closer look — React's `ReactTransition-test.js:190` asserts isPending stays
...
6. **No coverage for `useTransition` inside a Suspense boundary that itself has a
   pending fallback** — i.e. no test for the interaction between an outer
...
10. **Test #11's assertion is synchronous** (`expect(#deferred=Deferred: 2)`
    ...
    useDeferredValue inside the same transition pass, this is fine — but it's a
...
- Error path: a transition whose `use()` thenable REJECTS → assert `isPending`
...
- Multiple independent `useTransition` hooks where ONLY ONE is started (sibling
  pending isolation — confirm `useTransition` doesn't leak pending across hooks
  for unrelated transitions; the inverse of test #9).
- `useTransition` `start()` called during render (should throw or be ignored —
  React throws).
