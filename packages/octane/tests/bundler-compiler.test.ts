		...
		try {
			...
			writeFileSync(
				...
				JSON.stringify({
					name: 'raw-octane',
					main: 'index.js',
					dependencies: { 'identity-sensitive-core': '1.0.0' },
					peerDependencies: { octane: '*' },
					optionalDependencies: { 'missing-child': '1.0.0' },
					octane: {
						vite: {
							optimizeDeps: {
								exclude: ['identity-sensitive-core', 'identity-sensitive-core', '', ' padded ', 42],
							},
						},
					},
				}),
			...
			expect(discovered.viteOptimizeDepsExclusions).toEqual(['identity-sensitive-core']);
