...
// Local stand-ins for lexical/src/__tests__/utils (not shipped in the npm
// package): a non-inline decorator whose text content is 'Hello world' (11
// chars, matching upstream's TestDecoratorNode), a node-type assertion, and a
// per-test editor in place of initializeUnitTest's testEnv.
...
function $assertNodeType<T extends LexicalNode>(
	node: LexicalNode | null | undefined,
	predicate: (n: unknown) => n is T,
): T {
...
function createTestEditor(): LexicalEditor {
	const editor = createEditor({
		namespace: '',
		nodes: [OverflowNode, TestDecoratorNode],
		onError: (e: unknown) => {
			throw e;
		},
		theme: {},
...
// Ported from @lexical/react/src/__tests__/unit/useLexicalCharacterLimit.test.ts.
describe('LexicalNodeHelpers tests', () => {
	describe('merge', () => {
		function $initializeEditorWithLeftRightOverflowNodes(): [NodeKey, NodeKey] {
			const root = $getRoot();
		...
		it('merges an overflow node (left overflow selected)', async () => {
			...
			await editor.update(() => {
				[overflowLeftKey, overflowRightKey] = $initializeEditorWithLeftRightOverflowNodes();

				const overflowLeft = $assertNodeType($getNodeByKey(overflowLeftKey), $isOverflowNode);
				const overflowRight = $assertNodeType($getNodeByKey(overflowRightKey), $isOverflowNode);
			...
			await editor.update(() => {
				const paragraph = $assertNodeType($getRoot().getFirstChild(), $isParagraphNode);
