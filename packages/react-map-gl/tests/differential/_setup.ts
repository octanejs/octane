/**
 * Precompile the Mapbox differential fixtures for React. The same `.tsrx`
 * source is loaded by Octane in the test project and rewritten to import the
 * pinned upstream binding (packages/react-map-gl/upstream/src) on the React
 * side, so the oracle is @vis.gl/react-mapbox 8.1.2 itself, not a description
 * of it.
 */
...
function compileOne(sourcePath: string): void {
	...
	const rewritten = transformed.code
		.replace(/from\s+["']@octanejs\/react-map-gl["']/g, 'from "@vis.gl/react-mapbox"')
