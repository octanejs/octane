import * as A from '@octanejs/alien-signals';
import type { Assert, Equal } from '../../../../scripts/react-port/type-assertions';

// Concrete contracts from the authenticated 0.4.0 declarations. The named
// dependency list and direct functional setter remain compatibility extensions.
type ExpectedWritable = { (value: number | ((previous: number) => number)): void; (): number };
...
type Contract1 = Assert<
	Equal<
		...
		{ (value: number | ((previous: number) => number)): void; (): number }
	>
>;
type Contract2 = Assert<Equal<A.DependencyList, readonly unknown[]>>;
