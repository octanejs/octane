...
import { setupResizeJsDom } from '../upstream/spec/dom.ts';
import {
	CustomCell,
	CustomContainer,
	CustomItem,
...
import type { VGridHandle, VListHandle, WindowVirtualizerHandle } from '../src/index.ts';
...
describe('Octane VList', () => {
	...
	it('keeps composed hook state isolated across sibling instances and remounts', async () => {
		...
		const virtualContainers = rendered.container.querySelectorAll('main > div > div');
...
describe('Octane advanced virtualizers', () => {
	it('supports custom container and item components', async () => {
		const { container } = render(VirtualizerFixture, {
			props: { items: items.slice(0, 5), as: CustomContainer, item: CustomItem },
		});
		await settleResize();

		expect(container.querySelector('[data-custom-container]')).not.toBeNull();
		expect(container.querySelectorAll('[data-custom-item]')).toHaveLength(5);
	});
	...
	it('renders the experimental grid and exposes both axis handles', async () => {
		const gridRef: { current: VGridHandle | null } = { current: null };
		const { container } = render(GridFixture, {
			props: { row: 20, col: 20, gridRef, item: CustomCell },
		});
		await settleResize();

		expect(container.querySelector('[data-cell="0-0"]')?.textContent).toBe('0:0');
		expect(container.querySelector('[data-custom-cell]')).not.toBeNull();
		expect(gridRef.current?.getRowSize(0)).toBe(ITEM_HEIGHT);
		expect(gridRef.current?.getColSize(0)).toBe(100);
		expect(gridRef.current?.getRowOffset(2)).toBe(ITEM_HEIGHT * 2);
		expect(gridRef.current?.getColOffset(2)).toBe(200);
	});
});
