...
import { GridFixture, ListFixture, WindowFixture } from '../_fixtures/components.tsrx';
...
describe('Octane SSR', () => {
	...
	it.each([renderToString, renderToStaticMarkup])(
		'renders the requested experimental_VGrid window',
		(render) => {
			const { html } = render(GridFixture, {
				row: 100,
				col: 100,
				ssrRowCount: 3,
				ssrColCount: 4,
			...
			expect(html).toContain('id="octane-grid"');
		},
	);
});
