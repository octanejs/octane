...
/**
 * Verifies every vendored LiveStore upstream byte against upstream/SHA256SUMS
 * and confirms the required React test artifacts are present.
 */
export function verifyLivestoreUpstream(root = packageRoot) {
...
if (isMain) {
	const result = verifyLivestoreUpstream();
	console.log(`LiveStore upstream evidence is current (${result.files} byte-exact files).`);
}
