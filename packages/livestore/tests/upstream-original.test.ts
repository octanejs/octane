...
import { runPristineUpstreamSuite } from '../../../scripts/react-parity/livestore-pristine-runtime.mjs';

const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), '../../..');

type Identity = { file: string; fullName: string; status?: string };

// @parity-case pristine:livestore-original-suite
it('runs the pinned @livestore/react 0.4.0 suite unchanged', function runsPinnedLivestoreSuite() {
	const expected = JSON.parse(
		readFileSync(resolve(repoRoot, 'packages/livestore/audit/pristine-runtime.json'), 'utf8'),
	).tests as Identity[];
