...
/**
 * Verifies every vendored @tiptap/react byte against upstream/SHA256SUMS and
 * confirms the required React runtime suite artifacts are present.
 */
export function verifyTiptapUpstream(root = packageRoot) {
	...
	if (JSON.stringify(actualFiles) !== JSON.stringify(expectedFiles)) {
		throw new Error('Vendored TipTap file inventory differs from upstream/SHA256SUMS');
	}
	...
	for (const required of [
		...
		'src/use-client.spec.ts',
		'src/menus/BubbleMenu.spec.ts',
		'src/menus/FloatingMenu.spec.ts',
		'LICENSE.md',
	]) {
...
if (isMain) {
	const result = verifyTiptapUpstream();
	console.log(`TipTap upstream evidence is current (${result.files} byte-exact files).`);
}
