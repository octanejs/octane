/**
 * Adapted divergence: menu handlers receive native MouseEvent instances, not
 * synthetic events. Dedicated file so remaining menu conformance stays in
 * ordinary shards.
 */
...
import { MenuHarness } from '../_fixtures/menus.tsrx';
...
describe('@octanejs/tiptap menus', function () {
	...
	it('registers real menu plugins and syncs the detached menu element', function () {
		...
		const stoppedActionClick = vi.fn(function (event: Event) {
		...
		const result = mount(MenuHarness as any, {
			...
			onMenuClick: function (event: Event) {
				menuClicks.push({
					event,
			...
			bubbleRef: function (element: HTMLDivElement | null) {
				bubbleRefs.push(element);
			},
			floatingRef: function (element: HTMLDivElement | null) {
				floatingRefs.push(element);
			},
		});
		settle();
