...
export const RESOLVED_ADAPTER_BROWSER_STUB_ID = '\0vyre:adapter-browser-stub';
...
export function serve() {
  throw new Error('[vyre] Server adapters cannot run in the browser.');
}
export function nodeRequestToWebRequest() {
  throw new Error('[vyre] Node request helpers cannot run in the browser.');
}
export function webResponseToNodeResponse() {
  throw new Error('[vyre] Node response helpers cannot run in the browser.');
}
`;
}
...
/**
 * Generate the client hydration entry (served at virtual:vyre-hydrate).
 *
 ...
 *
 * vyre specifics:
 *   - `import { hydrate } from 'vyre'` (NO `mount`).
 *   - `hydrate(body, container, props)` signature — no `{ target, props }` wrapper.
 *   - The layout `children` is a ComponentBody `(s) => Page(s, { params })`,
 *     NOT a 0-arg thunk: vyre's `childSlot` invokes a bare function child
 *     as a ComponentBody (block first arg, `{}` props), so page data rides the
 *     closure — mirroring the server `createLayoutWrapper`.
 *   - hydrate() itself locates/consumes the <script data-vyre-suspense>
...
export function create_client_entry_source(_options) {
	return `// Auto-generated by vite-plugin-vyre.
...
(async () => {
  try {
    ...
    if (!el || !target) {
      console.error('[vyre] Unable to hydrate: missing #__ripple_data or #root.');
      return;
    }
    const data = JSON.parse(el.textContent || '{}'); // { entry, exportName, layout, params }
    if (!data.entry) {
      console.error('[vyre] Unable to hydrate: no route entry in #__ripple_data.');
      return;
    }
    ...
    if (!Component) {
      console.error('[vyre] Unable to hydrate: no component export for', data.entry);
      return;
    }
    ...
    if (data.layout) {
      ...
      if (Layout) {
        // children is a ComponentBody closing over params; vyre's childSlot
        // calls it with the block + {} props (the closure supplies the real
        // props), mirroring the server createLayoutWrapper so markers line up.
        const children = (s) => Component(s, { params });
        hydrate(Layout, target, { params, children });
        return;
      }
    }

    hydrate(Component, target, { params });
  } catch (error) {
    console.error('[vyre] Failed to bootstrap client hydration.', error);
  }
})();
`;
}
