...
// @parity-case pristine:tanstack-store-original-suite
it('runs the pinned @tanstack/react-store 0.11.0 suite unchanged', function runsPinnedTanstackStoreSuite() {
	const expected = JSON.parse(
		readFileSync(resolve(repoRoot, 'packages/tanstack-store/audit/pristine-runtime.json'), 'utf8'),
	).tests as Identity[];
