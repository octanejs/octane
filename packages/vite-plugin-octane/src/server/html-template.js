...
// A real (hidden) element survives Vite's HTML module-script rewrite, unlike
...
export function validateSsrTemplate(html) {
	...
	if (headMarkerCount !== 1) {
		throw new Error(
			`[@octanejs/vite-plugin] index.html must contain exactly one ${HEAD_MARKER}; found ${headMarkerCount}.`,
		);
	}
...
function validateSsrBodyContract(html) {
	...
	if (markerCount !== 1) {
		throw new Error(
			`[@octanejs/vite-plugin] index.html must contain exactly one ${BODY_MARKER}; found ${markerCount}.`,
		);
	}
	const bodyCloseCount = html.match(/<\/body\s*>/gi)?.length ?? 0;
	if (bodyCloseCount !== 1) {
		throw new Error(
			`[@octanejs/vite-plugin] index.html must contain exactly one closing </body> tag for hydration injection; found ${bodyCloseCount}.`,
		);
	}
}
...
/**
 * Add the request nonce to the production hydrate script Vite built. The
...
export function applyHydrationNonce(html, nonce) {
	...
	if (hydrationTags.length === 0 && markerCount === 1) {
		// Vite coalesces/hoists HTML module entries into the head, so the hydrate
	...
	if (invalidMarkerContract) {
		throw new Error(
			`[@octanejs/vite-plugin] Built SSR template must identify its hydration module script; found ${hydrationTags.length}.`,
		);
	}
...
export function getContextNonce(context) {
	...
	if (typeof value !== 'string' || value.length === 0) {
		throw new Error(
			`[@octanejs/vite-plugin] Context.state.get('${OCTANE_NONCE_STATE_KEY}') must be a non-empty string.`,
		);
	}
	return value;
}
