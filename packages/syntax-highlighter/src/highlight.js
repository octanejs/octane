...
function flattenCodeTree(tree, className = [], newTree = []) {
	...
	for (let i = 0; i < tree.length; i++) {
		...
		if (node.type === "text") {
			...
			flattenCodeTree(node.children, classNames).forEach((i2) =>
				newTree.push(i2),
			);
		}
	}
...
function stdin_default(defaultAstGenerator, defaultStyle) {
	return function SyntaxHighlighter({
		...
		const allLineNumbers = showLineNumbers
			? /* @__PURE__ */ React.createElement(AllLineNumbers, {
		...
		if (!astGenerator) {
			return /* @__PURE__ */ React.createElement(
				...
				{ ...preProps },
				allLineNumbers,
				/* @__PURE__ */ React.createElement(CodeTag, { ...codeTagProps }, code),
			);
		}
