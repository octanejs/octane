...
type InputProps<T extends React.ElementType> =
	WithBaseUIEvent<PropsOf<T>> | ((otherProps: PropsOf<T>) => PropsOf<T>) | undefined;

const EMPTY_PROPS = {};

/* eslint-disable id-denylist */
/**
 * Merges multiple sets of Octane props. It follows the Object.assign pattern where the rightmost object's fields overwrite
 ...
 * by calling `event.preventBaseUIHandler()`. For custom callbacks with primitive/object values,
...
/**
 * Merges an arbitrary number of Octane props using the same logic as {@link mergeProps}.
...
function copyInitialProps<T extends ElementType>(
	inputProps: WithBaseUIEvent<PropsOf<T>> | undefined,
) {
...
function mutablyMergeInto<T extends ElementType>(
	...
	externalProps: WithBaseUIEvent<PropsOf<T>> | undefined,
) {
	if (!externalProps) {
		return mergedProps;
	}
...
function isNativeEvent(event: unknown): event is Event {
	// Duck typing also recognizes events created by an iframe's Window.
	return (
		event != null &&
		typeof event === 'object' &&
