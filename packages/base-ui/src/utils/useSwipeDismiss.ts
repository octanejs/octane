...
import { clamp } from '@octanejs/base-ui-utils/clamp';
...
function nativeSwipeEvent(event: SwipeDismissMoveEvent): PointerEvent | TouchEvent {
...
export function useSwipeDismiss(options: UseSwipeDismissOptions): UseSwipeDismissReturnValue {
	...
	function startSwipeAtPosition(
		...
		const target = getTargetAtPoint(position, nativeSwipeEvent(event));
	...
	function handleMoveCore(
		...
		const target = getTarget(nativeSwipeEvent(event)) as HTMLElement | null;
			...
			if (allowedToStart) {
				...
				if (isTouchLikeEvent(event)) {
					...
					if (pendingStartPos && element) {
						const target = getTargetAtPoint(currentPos, nativeSwipeEvent(event));
