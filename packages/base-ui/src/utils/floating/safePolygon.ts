// Ported verbatim from .base-ui/packages/react/src/floating-ui-react/safePolygon.ts (v1.6.0).
// Builds the "safe" triangle between the cursor's exit point and the popup so the user can move
// diagonally toward the popup without it closing under the pointer. Pure geometry plus a mousemove
// handler; no React/octane API is involved, so this is a straight transcription.
...
function isInsideRect(pointX: number, pointY: number, rect: Rect): boolean {
	return (
		pointX >= rect.x &&
...
export function safePolygon(options: SafePolygonOptions = {}): HandleClose {
	...
	const fn: HandleClose = ({ x, y, placement, elements, onClose, nodeId, tree }) => {
		...
		return function onMouseMove(event: MouseEvent): undefined {
			timeout.clear();
			...
			// Prevent an overlapping floating element from getting stuck in an open-close loop.
			...
			// where the floating element remains open, but should be ignored. A constant of 1
			...
			// Since the triangle is created from the cursor point, which can start beyond the ref
			// element's edge, traversing back and forth from the ref to the floating element can
			// cause it to close. This ensures it always remains open in that case.
