...
import { mergeCleanups } from '@octanejs/base-ui-utils/mergeCleanups';
...
import { useFloatingTree } from '../components/FloatingTree.tsrx';
		...
		return {
			...
			onMouseMove(event) {
				...
				function handleMouseMove() {
					...
					// The active trigger can change while the rest delay is pending, including
					// when the popup opens programmatically. Recheck it when the timer fires.
					const isStillOverInactive = isOverInactiveTrigger(
					...
					if (
						...
						(!latestOpen || isStillOverInactive) &&
						checkShouldOpen()
					) {
						store.setOpen(
							true,
