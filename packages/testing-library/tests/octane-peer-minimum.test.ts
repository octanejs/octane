...
const nextReleaseOctane = resolve(packageDirectory, '../octane/src/index.ts');
...
describe('@octanejs/testing-library Octane peer minimum', function () {
	...
	it('bundles the pure entry against next-release Octane source that exports isInActScope', async function () {
		const code = await bundleTestingLibraryAgainst(nextReleaseOctane);
	...
	it('resolves isInActScope from next-release Octane source', async function () {
		const octane = await import(nextReleaseOctane);
