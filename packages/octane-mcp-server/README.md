...
`application-composition`, `scaling-curves`, `behavior-root-events`, `activity`,
`streaming-ssr`, `streaming-backpressure`, `compiler-throughput`, `codegen-size`,
`hook-memo`, `bundle-size`, `bundle-reachability`, `three-renderer`,
`three-bundle-size`, …)
