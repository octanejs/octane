...
function domProps(props: any, isValidProp?: MotionConfigValue['isValidProp']): Record<string, any> {
	const out: Record<string, any> = {};
	for (const k in props) {
		if (MOTION_PROPS.has(k)) continue;
		// Refs and visual styles are host integration; native events bypass the
		// ordinary-prop predicate, as they do in Motion's DOM renderer.
		if (k !== 'ref' && k !== 'style' && !k.startsWith('on') && isValidProp?.(k) === false) continue;
...
function createMotionComponent(tag: string, preloadedFeatures: boolean): MotionComponent {
	return function MotionComponent(props: any, scope: any): void {
		...
		const node = hostComponent(
			scope,
			0,
			tag,
			domProps(props, config.isValidProp),
			props.children,
		) as HTMLElement;
...
// renders children.
export interface MotionConfigProps extends Omit<MotionConfigValue, 'shouldReduceMotion'> {
	children?: OctaneNode;
}

export function MotionConfig(props: MotionConfigProps, scope: Scope): void {
	...
	provideContext(scope, MotionConfigContext, {
		...
		isValidProp: 'isValidProp' in props ? props.isValidProp : parent.isValidProp,
		reducedMotion,
