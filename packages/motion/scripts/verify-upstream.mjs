...
/**
 * Verifies every vendored Motion suite artifact against upstream/SHA256SUMS and
 * confirms the curated React runtime cases from the 12.42.2 pin are present.
 */
export function verifyMotionUpstream(root = packageRoot) {
	...
	if (JSON.stringify(actualFiles) !== JSON.stringify(expectedFiles)) {
		throw new Error('Vendored Motion file inventory differs from upstream/SHA256SUMS');
	}
	...
	for (const required of [
		'src/value/__tests__/use-motion-value.test.tsx',
		'src/jest.setup.upstream.tsx',
		'src/gestures/__tests__/utils.ts',
		'LICENSE.md',
	]) {
...
if (isMain) {
	const result = verifyMotionUpstream();
	console.log(`Motion upstream evidence is current (${result.files} byte-exact files).`);
}
