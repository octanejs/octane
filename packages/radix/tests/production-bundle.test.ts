...
function unrelatedComponents(modules: string[]): string[] {
	return modules.filter((module) =>
		/(?:^|\/)packages\/radix\/src\/(?:Dialog|Popover)\.ts$/.test(module),
	);
}

describe('@octanejs/radix public production imports', () => {
	it('renders the selected Separator without retaining unrelated primitive modules', async () => {
	...
	it('preserves Separator behavior and primitive pruning in a Vite production build', async () => {
