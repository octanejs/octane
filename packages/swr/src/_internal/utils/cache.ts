import { defaultConfigOptions } from './web-preset.js';
import { IS_SERVER } from './env.js';
import { UNDEFINED, mergeObjects, noop } from './shared.js';
import { internalMutate } from './mutate.js';
import { SWRGlobalState } from './global-state.js';
...
export const initCache = <Data = any>(
	...
	if (!SWRGlobalState.has(provider)) {
		...
		const subscriptions: Record<string, ((current: any, prev: any) => void)[]> =
			Object.create(null);
		const subscribe = (key: string, callback: (current: any, prev: any) => void) => {
			...
			if (!SWRGlobalState.has(provider)) {
				...
				if (!IS_SERVER) {
					...
					const releaseFocus = opts.initFocus(
						setTimeout.bind(
							UNDEFINED,
							revalidateAllKeys.bind(UNDEFINED, EVENT_REVALIDATORS, revalidateEvents.FOCUS_EVENT),
						),
					);
