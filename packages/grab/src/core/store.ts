import { createStore, produce } from '../reactive.js';
import { batch, createSignal } from '../reactive.js';
...
const createGrabStore = (input: GrabStoreInput) => {
	...
	const actions: GrabActions = {
		...
		removeGrabbedBox: (boxId: string) => {
			setStore('grabbedBoxes', (boxes: GrabbedBox[]) => boxes.filter((box) => box.id !== boxId));
		},
