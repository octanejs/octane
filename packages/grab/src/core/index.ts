...
export const init = (rawOptions?: Options): ReactGrabAPI => {
	...
	return createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) {
			// The renderer is dynamically imported so the Octane mount — and the
			// document access it implies — never runs during SSR.
			void import('../components/renderer.tsrx')
				.then(({ ReactGrabRenderer }) => {
					if (disposed) return;
					// Props are accessor functions: upstream relied on Solid's getter
					// props for liveness; `useProp` inside each component subscribes to
					// the same signals through these accessors.
					const rendererMount = createOctaneRoot(rendererRoot);
					rendererMount.render(ReactGrabRenderer, {
						selectionVisible,
						selectionBounds,
						selectionBoundsMultiple,
						selectionShouldSnap: accessor(
							() => store.frozenElements.length > 0 || dragPreviewBounds().length > 0,
						),
						selectionElementsCount: accessor(() => store.frozenElements.length),
						frozenLabelEntryAccessors: visibleFrozenLabelEntryAccessors,
						pendingShiftPreviewEntry: accessor(() => pendingShiftPreviewEntry() ?? undefined),
						selectionFilePath: accessor(() => store.selectionFilePath ?? undefined),
						selectionTagName,
						selectionComponentName: resolvedComponentName,
						selectionLabelVisible,
						selectionLabelStatus: 'idle' as const,
						hierarchyState,
						...
						isFrozen: accessor(() => isFrozenPhase() || isActivated() || isToolbarSelectHovered()),
						inputValue: accessor(() => store.inputText),
						isPromptMode,
						...
						toolbarVisible: accessor(() => pluginRegistry.store.theme.toolbar.enabled),
						...
						onToolbarRef: (element: HTMLDivElement | undefined) => {
						...
						contextMenuHasFilePath: accessor(() => Boolean(contextMenuFilePath()?.filePath)),
						actions: accessor(() => pluginRegistry.store.actions),
						...
						toolbarMenuPosition,
						toolbarMenuActions: accessor(() =>
							pluginRegistry.store.actions.filter((action) => action.showInToolbarMenu === true),
						),
						defaultActionId: accessor(
					...
					disposeRenderer = () => rendererMount.unmount();
				})
				.catch((error) => {
...
export { isInstrumentationActive } from '../bippy/index.js';
