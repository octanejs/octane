...
export const getGlobalApi = (): ReactGrabAPI | null => {
	...
	return window.__OCTANE_GRAB__ ?? window.__REACT_GRAB__ ?? globalApi ?? null;
};
...
export const clearGlobalApi = (api: ReactGrabAPI): void => {
	...
	if (typeof window !== 'undefined') {
		if (window.__OCTANE_GRAB__ === api) delete window.__OCTANE_GRAB__;
		if (window.__REACT_GRAB__ === api) delete window.__REACT_GRAB__;
	}
};
