// Ported from @tanstack/react-hotkeys@0.10.0 packages/react-hotkeys/tests/useHotkeySequences.test.tsx
...
import { useHotkeySequences } from '@octanejs/tanstack-hotkeys';
import type { UseHotkeySequenceDefinition } from '@octanejs/tanstack-hotkeys';
...
describe('useHotkeySequences', () => {
	...
	// @parity-case adapted:upstream-usehotkeysequences:should-register-multiple-sequence-handlers
	...
	// @parity-case adapted:upstream-usehotkeysequences:should-call-the-correct-callback-for-each-sequence
	...
	// @parity-case adapted:upstream-usehotkeysequences:should-unregister-all-sequences-on-unmount
	...
	// @parity-case adapted:upstream-usehotkeysequences:should-handle-an-empty-array-as-a-no-op
	it('should handle an empty array as a no-op', () => {
		renderHook(() => useHotkeySequences([]));
		expect(SequenceManager.getInstance().getRegistrationCount()).toBe(0);
	});

	// @parity-case adapted:upstream-usehotkeysequences:should-skip-definitions-with-an-empty-sequence
	...
	// @parity-case adapted:upstream-usehotkeysequences:should-register-disabled-sequences-and-keep-them-in-the-manager
	...
	// @parity-case adapted:upstream-usehotkeysequences:should-handle-dynamic-array-changes-add-sequence-
	it('should handle dynamic array changes (add sequence)', () => {
		...
		const { rerender } = renderHook(
			({ defs }: { defs: Array<UseHotkeySequenceDefinition> }) => useHotkeySequences(defs),
			{
				initialProps: {
					defs: [
						{ sequence: ['G', 'G'], callback: gg },
	...
	// @parity-case adapted:upstream-usehotkeysequences:should-move-a-sequence-registration-when-only-the-target-changes
	it('should move a sequence registration when only the target changes', () => {
		...
		const { rerender } = renderHook(
			({ target }: { target: HTMLElement }) =>
				useHotkeySequences([{ sequence: ['G', 'G'], callback, options: { target } }]),
		...
		targetB.dispatchEvent(new KeyboardEvent('keydown', { key: 'g', bubbles: true }));
		targetB.dispatchEvent(new KeyboardEvent('keydown', { key: 'g', bubbles: true }));
		expect(callback).toHaveBeenCalledTimes(2);
	});

	describe('stale closure prevention', () => {
		// @parity-case adapted:upstream-usehotkeysequences:should-have-access-to-latest-state-values-in-callbacks
		...
		// @parity-case adapted:upstream-usehotkeysequences:should-sync-enabled-option-on-every-render
		it('should sync enabled option on every render', () => {
			const callback = vi.fn();

			const { rerender } = renderHook(
				({ enabled }: { enabled: boolean }) =>
					useHotkeySequences([{ sequence: ['G', 'G'], callback, options: { enabled } }]),
		...
		// @parity-case adapted:upstream-usehotkeysequences:should-preserve-registration-id-when-toggling-enabled
