 ...
 */
type LazyPath<TValue, TPathToCheck extends Path, TValidPath extends Path = readonly []> =
 ...
 */
type LazyArrayPath<TValue, TPathToCheck extends Path, TValidPath extends Path = readonly []> =
	...
	: TValue extends Record<PropertyKey, unknown>
		? {
				[TKey in ExactKeysOf<TValue>]:
					readonly [TKey] | DeepDirtyPath<NonNullable<PropertiesOf<TValue>[TKey]>, TKey, TDepth>;
			}[ExactKeysOf<TValue>]
		: never;
...
type DeepFieldPath<TChild, TKey extends PathKey, TDepth extends 0[]> = TChild extends
	readonly unknown[] | Record<PropertyKey, unknown>
...
export type FieldPath<TValue, TDepth extends 0[] = []> = TDepth['length'] extends 5
	...
	: TValue extends readonly unknown[] | Record<PropertyKey, unknown>
		? {
				[TKey in ExactKeysOf<TValue>]:
					readonly [TKey] | DeepFieldPath<NonNullable<PropertiesOf<TValue>[TKey]>, TKey, TDepth>;
			}[ExactKeysOf<TValue>]
		: never;
