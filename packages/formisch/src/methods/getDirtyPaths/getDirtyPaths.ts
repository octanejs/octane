...
export function getDirtyPaths(
	form: BaseFormStore,
	config?: GetFormDirtyPathsConfig | GetFieldDirtyPathsConfig<FormSchema, RequiredPath>,
): RequiredPath[] {
...
// @__NO_SIDE_EFFECTS__
function collectDirtyPaths(internalFieldStore: InternalFieldStore, paths: RequiredPath[]): void {
	...
	if (internalFieldStore.kind === 'object' && internalFieldStore.input.value) {
		...
		if (internalFieldStore.isDirty.value && internalFieldStore.path.length > 0) {
			paths.push([...internalFieldStore.path] as unknown as RequiredPath);
		}

		// Otherwise, field is atomic (array or cleared object) — emit its path
		// if any dirty content exists
	} else if (getFieldBool(internalFieldStore, 'isDirty') && internalFieldStore.path.length > 0) {
		paths.push([...internalFieldStore.path] as unknown as RequiredPath);
	}
}
