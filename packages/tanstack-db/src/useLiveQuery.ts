// Browser consumers do not need Node ambient types for the guarded warning checks.
...
import { useOptionalDbClient } from './DbProvider.tsrx';
...
export type UseLiveQueryConfig<TContext extends Context> = UseLiveQueryConfigOptions<TContext> &
...
export type ConditionalUseLiveQueryConfig<TContext extends Context> =
	UseLiveQueryConfigOptions<TContext> & {
		query:
			...
			| ((q: InitialQueryBuilder) => ConfiguredQueryBuilder<TContext> | undefined | null);
	};
...
function getWarningCallsite(stackIndex: number): string {
	...
	// Manual hooks add invokeManualHook, a public wrapper, and withSlot frames.
	// Remove those bridges so upstream's depth still identifies the consumer.
	const frames = stack.split(`\n`);
	const callerFrames = frames.filter((frame, index) => {
		return (
			!/^\s*at (?:invokeManualHook|withSlot)\s*\(/.test(frame) &&
			!/^\s*at invokeManualHook\s*\(/.test(frames[index - 1] ?? ``)
		);
	});
	return callerFrames[stackIndex]?.trim() ?? stack;
}

function warnDerivedIdentityHotPath(profiler: DerivedIdentityProfiler, durationMs: number): void {
...
export function warnUnhashableDerivedIdentity(error: UnhashableQueryIRError): void {
...
export function useLiveQuery(configOrQueryOrCollection: any, ...rest: Array<unknown>) {
	...
	const collectionRef = useRef<Collection<object, string | number, {}> | null>(
		null,
		subSlot(slot, 'collectionRef'),
	);
	const depsRef = useRef<Array<unknown> | null>(null, subSlot(slot, 'depsRef'));
	const configRef = useRef<unknown>(null, subSlot(slot, 'configRef'));
	const clientRef = useRef(dbClient, subSlot(slot, 'clientRef'));
	const legacyUnhashableIdentityRef = useRef<Array<unknown>>(
		[`legacy-unhashable`],
		subSlot(slot, 'legacyUnhashableIdentityRef'),
	);
	...
	const derivedIdentityProfilerRef = useRef<DerivedIdentityProfiler>(
		...
		subSlot(slot, 'derivedIdentityProfilerRef'),
	);
	const deferredCollectionsRef = useRef(
		new Set<CollectionImpl<any, string | number, any, any, any>>(),
		subSlot(slot, 'deferredCollectionsRef'),
	);
	const observerRef = useRef<LiveQueryObserver<object, string | number> | null>(
		null,
		subSlot(slot, 'observerRef'),
	);
	const queryHashRef = useRef<string | undefined>(undefined, subSlot(slot, 'queryHashRef'));
	const identityErrorRef = useRef<UnhashableQueryIRError | undefined>(
		undefined,
		subSlot(slot, 'identityErrorRef'),
	);

	const queryKey = !inputIsCollection ? getExplicitQueryKey(configOrQueryOrCollection) : undefined;
	let preparedQueryValue: unknown | typeof unpreparedQueryValue = unpreparedQueryValue;
	...
	if (queryKey) {
		...
		try {
			...
			streamIdentity = [`deps`, resolvedDeps, getPreparedLiveQueryIdentity(preparedQueryValue)];
		} catch (error) {
	...
	if (needsNewCollection) {
		if (inputIsCollection) {
			...
			collectionRef.current = createCollectionFromPreparedQuery(preparedQueryValue) as Collection<
				object,
				string | number,
				{}
			>;
