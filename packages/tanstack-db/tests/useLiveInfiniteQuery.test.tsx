...
import { createFilterFunctionFromExpression } from './db-fixtures/db-internal/create-filter';
			...
			sync: ({ markReady, begin, write, commit }) => {
				...
				return {
					loadSubset: (subsetOpts: LoadSubsetOptions) => {
						...
						let filtered = [...allPosts].sort((a, b) => b.createdAt - a.createdAt);

						if (subsetOpts.cursor) {
							const whereFromFn = createFilterFunctionFromExpression(subsetOpts.cursor.whereFrom);
...
describe(`useLiveInfiniteQuery`, () => {
	...
	it(`should handle deletion from partial page with descending order`, async () => {
		...
		expect(result.current.data.find((p) => p.id === firstItemId)).toBeUndefined();
	...
	it(`should handle deletion from partial page with ascending order`, async () => {
		...
		const { result } = renderHook(() => {
			return useLiveInfiniteQuery(
				(q) => q.from({ posts: collection }).orderBy(({ posts: p }) => p.createdAt, `asc`), // ascending order
				{
					pageSize: 20,
					getNextPageParam: (lastPage) => (lastPage.length === 20 ? lastPage.length : undefined),
				},
	...
	it(`should request limit+1 (peek-ahead) from loadSubset for hasNextPage detection`, async () => {
		...
		const callWithLimit = loadSubsetCalls.find((call) => call.limit !== undefined);
					...
					return {
						loadSubset: (opts: LoadSubsetOptions) => {
							...
							if (opts.cursor) {
								...
								try {
									const whereFromFn = createFilterFunctionFromExpression(whereFrom);
									const fromData = filtered.filter(whereFromFn);

									const whereCurrentFn = createFilterFunctionFromExpression(whereCurrent);
									...
									const limitedFromData = opts.limit ? fromData.slice(0, opts.limit) : fromData;
