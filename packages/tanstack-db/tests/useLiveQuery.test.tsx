...
import { mockSyncCollectionOptions, stripVirtualProps } from './db-fixtures/utils';
...
describe(`Query Collections`, () => {
	...
	describe(`aggregates nested inside expressions`, () => {
		...
		it(`subquery with coalesce(count(...)) can be left-joined as a source`, async () => {
			...
			const { result } = renderHook(() => {
				return useLiveQuery((q) => {
					...
					return q
						...
						.leftJoin({ ic: issueCountSubquery }, ({ persons, ic }) => eq(persons.id, ic.userId))
						.select(({ persons, ic }) => ({
							name: persons.name,
			...
			const results = result.current.data;
			expect(stripVirtualProps(results.find((r) => r.name === `John Doe`))).toEqual({
				name: `John Doe`,
				issueCount: 2, // Issues 1 and 3
			});
			expect(stripVirtualProps(results.find((r) => r.name === `Jane Doe`))).toEqual({
				name: `Jane Doe`,
	...
	describe(`includes subqueries`, () => {
		...
		it(`should render includes results and reactively update child collections`, async () => {
			...
			const alphaProject = parentResult.current.data.find((p: any) => p.id === `p1`)!;
			...
			const { result: childResult } = renderHook(() => useLiveQuery((alphaProject as any).issues));
