...
export const stripVirtualProps = <T extends Record<string, any> | undefined>(value: T) => {
...
export function createIndexUsageTracker(collection: any): {
	...
	collection.entries = function* () {
		...
		if (stack.includes(`createFilterFunction`) || stack.includes(`currentStateAsChanges`)) {
			stats.fullScanCalls++;
			stats.queriesExecuted.push({
				type: `fullScan`,
			});
...
export function mockSyncCollectionOptions<T extends object = Record<string, unknown>>(
	...
	const options: CollectionConfig<T, string | number, never> & {
		...
		defaultIndexType:
			config.defaultIndexType ?? (config.autoIndex === `eager` ? BTreeIndex : undefined),
	};
...
export function mockSyncCollectionOptionsNoInitialState<T extends object = Record<string, unknown>>(
		...
		Promise.resolve(testFn())
			.then(async (value) => {
				...
				if (!expectedRejectionCaught) {
					reject(new Error(`Expected rejection with message "${expectedMessage}" was not caught`));
					return;
				}
