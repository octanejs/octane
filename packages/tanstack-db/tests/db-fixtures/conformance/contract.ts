 ...
 */
export interface DeferredSourceHandle<T extends { id: string } = Row> extends SourceHandle<T> {
...
export interface LiveQueryDriver {
	...
	makeSource: <T extends { id: string }>(initialData: ReadonlyArray<T>) => SourceHandle<T>;
	 ...
	 */
	mountControllable: <P>(build: (q: any, param: P) => any, initial: P) => ControllableHandle<P>;
