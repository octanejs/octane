...
export function runSuite(rawDriver: LiveQueryDriver) {
	...
	const driver: LiveQueryDriver = {
		...
		mountControllable: (build, initial) => track(rawDriver.mountControllable(build, initial)),
	...
	const scenario = (key: string, name: string, fn: () => Promise<void> | void) => {
	...
	describe(`live-query conformance :: ${driver.name}`, () => {
		...
		scenario(`basic-select`, `from + where + select returns matching rows`, async () => {
		...
		scenario(`live-insert`, `a sync insert appears in the result`, async () => {
			const source = driver.makeSource(SEED);
			const h = driver.mount((q) =>
				q.from({ items: source.collection }).select(({ items }: any) => ({ id: items.id })),
			);
			await h.flush();
		...
		scenario(`findone-cardinality`, `findOne returns a single row, not an array`, async () => {
			const source = driver.makeSource(SEED);
			const h = driver.mount((q) =>
				q
					.from({ items: source.collection })
		...
		scenario(`no-updates-after-unmount`, `no result mutation after unmount`, async () => {
			const source = driver.makeSource(SEED);
			const h = driver.mount((q) =>
				q.from({ items: source.collection }).select(({ items }: any) => ({ id: items.id })),
			);
		...
		scenario(`groupby-aggregate`, `groupBy + count aggregates per group`, async () => {
			const people = driver.makeSource(SEED);
			const h = driver.mount((q) =>
				q
			...
			const byTeam = new Map(h.current().data.map((r: any) => [r.team, r.count]));
		...
		scenario(`nested-aggregates`, `coalesce(count(...), 0) in a joined subquery`, async () => {
			const people = driver.makeSource(SEED);
			const issues = driver.makeSource(ISSUES);
			const h = driver.mount((q) => {
				const issueCounts = q
				...
				return q
					...
					.leftJoin({ ic: issueCounts }, ({ persons, ic }: any) => ops.eq(persons.id, ic.userId))
			...
			const byName = new Map(h.current().data.map((r: any) => [r.name, r.issueCount]));
		...
		scenario(`live-update`, `a sync update is reflected in place`, async () => {
			...
			expect(h.current().data.find((r: any) => r.id === `1`).name).toBe(`Johnny Doe`);
			h.unmount();
		});
		...
		scenario(`param-recompile`, `changing a query parameter recompiles the result`, async () => {
			const source = driver.makeSource(SEED);
		...
		scenario(`config-object-input`, `accepts the { query } config-object input form`, async () => {
			const source = driver.makeSource(SEED);
		...
		scenario(`error-status`, `a failing source surfaces an error (flag or boundary)`, async () => {
		...
		scenario(`order-only-move`, `an order-only move republishes the ordered result`, async () => {
			const source = driver.makeSource(SEED);
