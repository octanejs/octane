...
// Adapted from react-intersection-observer@10.1.0 src/__tests__/useOnInView.test.tsx
import { beforeEach, expect, test, vi } from 'vitest';
import { render } from '@octanejs/testing-library';
import { useCallback, useEffect, useState } from 'octane';
import type { IntersectionEffectOptions } from '../../src/index';
import {
	intersectionMockInstance,
	mockAllIsIntersecting,
	setupIntersectionMocking,
} from '../../src/test-utils';
import { useOnInView } from '../../src/useOnInView.tsrx';
...
const LazyOnInViewChangedComponent = ({ options }: { options?: IntersectionEffectOptions }) => {
...
const ThresholdTriggerComponent = ({ options }: { options?: IntersectionEffectOptions }) => {
...
const MergeRefsComponent = ({ options }: { options?: IntersectionEffectOptions }) => {
	const [inView, setInView] = useState(false);
	...
	const setRef = useCallback((node: Element | null) => inViewRef(node), [inViewRef]);
...
const MultipleCallbacksComponent = ({ options }: { options?: IntersectionEffectOptions }) => {
...
test('should track which threshold triggered the visibility change', () => {
	...
	const triggeredThresholds = JSON.parse(element.getAttribute('data-triggered-thresholds') || '[]');
