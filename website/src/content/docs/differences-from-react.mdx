...
Octane also removes avoidable `use()` waterfalls by default. Independent requests can
start together; a request that needs an earlier result remains sequential:
...
Set `parallelUse: false` when you specifically need React's serial start timing.
