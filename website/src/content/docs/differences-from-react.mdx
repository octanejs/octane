...
custom-hook declarations are resolved by lexical binding to a transitive fixed
point, so aliases and cycles retain context, state, suspension, and effect
lifecycle behavior. Projection guards witness a method/callable, its receiver,
and its explicit arguments; derived receivers are witnessed through the
