...
text inputs use native `onInput`, for example. Check
...
<aside className="doc-callout tip">
	...
	<p>
		{
			'Both dialects use the same hooks, context, portals, Suspense, transitions, native events, SSR, and hydration. The octane() Vite plugin builds both, while octane/compiler/volar adds editor support for .tsrx.'
		}
	</p>
</aside>
...
keyed `.map()`. Continue with [Core APIs](/docs/core-apis) for the APIs shared by both
styles.
