...
Scoped `<style>` blocks belong to both dialects: a block styles the template scope it
sits in, and an assigned block (`const theme = <style>…</style>`) is a theme with a
`$class` you can pass around or `apply` elsewhere. TSRX adds scopes of its own —
each `@{ ... }` body and every directive arm can carry a block that reaches only that
scope. See [Styling](/docs/styling).
...
<aside className="doc-callout tip">
	...
	<p>
		{
			'Both dialects use the same hooks, context, portals, Suspense, transitions, native events, scoped <style> blocks, SSR, and hydration. The Vite, Rspack, and Rsbuild integrations compile both, while octane/compiler/volar adds editor support for .tsrx.'
		}
	</p>
</aside>
...
- [Styling](/docs/styling) — scoped `<style>` blocks, class maps, and `apply`.
