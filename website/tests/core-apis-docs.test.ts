...
// Mount the real docs route once, then verify its learning structure and every
// embedded demo through that single integration boundary.
describe('Core APIs documentation', () => {
	it('presents a concept-first guide with complete navigation and interactive examples', async () => {
...

		{
			...
			const restore = Array.from(demo.querySelectorAll<HTMLButtonElement>('button')).find(
				(button) => button.textContent?.includes('Restore list'),
			)!;
			...
			expect(status.textContent).toContain('1 of 3 packed');
			expect(
				(packing.getByRole('checkbox', { name: 'Passport' }) as HTMLInputElement).checked,
			).toBe(false);
...

		{
			...
			const loading = await waitFor(() => {
				const pending = demo.querySelector('.data-loading');
