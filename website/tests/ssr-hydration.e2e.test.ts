	...
	it.concurrent(
		...
		async () => {
			...
			try {
				...
				// the router's location without changing matches. The hydrated tree must
				...
				const survived = await page.evaluate(async () => {
					...
					const before = router.stores.location.get();
					...
					while (router.stores.location.get() === before && Date.now() < deadline) {
						await new Promise((resolve) => setTimeout(resolve, 25));
					}
					await new Promise((resolve) => setTimeout(resolve, 100));
					return {
						processed: router.stores.location.get() !== before,
