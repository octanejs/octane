...
function regenerateAdaptedTree(lock, packageDirectory, pristineDirectory, execFile, check = false) {
	...
	try {
		for (const { sourcePath, targetPath } of planned) {
			...
			if (!bytes.includes(0)) {
				bytes = Buffer.from(
					applyAdaptedRewrites(bytes.toString('utf8'), lock.adaptedRewrites, targetPath),
				);
			}
			...
			if (check) {
				const actual = path.join(packageDirectory, targetPath);
				if (!existsSync(actual)) missing.push(targetPath);
				else {
					if (realpathSync(actual) !== path.join(realpathSync(packageDirectory), targetPath)) {
						throw new Error(`Adapted evidence must not use symlinks: ${targetPath}`);
					}
					if (!readFileSync(actual).equals(bytes)) mismatched.push(targetPath);
				}
			} else writeTreeFile(packageDirectory, targetPath, bytes);
	...
	if (check) {
		...
		for (const relativeRoot of targetRoots) {
			const directory = path.join(packageDirectory, relativeRoot);
			if (!existsSync(directory)) continue;
			for (const entry of readdirSync(directory, { recursive: true, withFileTypes: true })) {
				if (entry.isDirectory()) continue;
				const file = path
					.relative(packageDirectory, path.join(entry.parentPath, entry.name))
					.split(path.sep)
					.join('/');
				if (!expected.has(file)) unexpected.push(file);
			}
		}
...
/** Verify regenerated bytes without changing either evidence tree. */
export function checkAdaptedTree(lock, packageDirectory) {
	const drift = regenerateAdaptedTree(
		lock,
		packageDirectory,
		path.join(packageDirectory, PRISTINE_RELATIVE_PATH),
		execFileSync,
		true,
	);
	if (Object.values(drift).some((files) => files.length > 0)) {
		throw new Error(`Adapted tree drifted from its lock and patches: ${JSON.stringify(drift)}`);
	}
	return planAdaptedFiles(lock)
		.map(({ targetPath }) => targetPath)
		.filter((targetPath) => !existsSync(patchArtifactPaths(packageDirectory, targetPath).skipPath));
}
...
async function commandRun(options) {
	...
	if (options.check) {
		...
		const pristineDrift = Object.values(verification).some((files) => files.length > 0);
		const adapted = pristineDrift
			? null
			: regenerateAdaptedTree(
					lock,
		...
		const adaptedDrift = adapted && Object.values(adapted).some((files) => files.length > 0);
		return {
			command: 'run',
			mode: 'check',
			status: pristineDrift || adaptedDrift ? 'failed' : 'passed',
			...verification,
			adapted,
		};
	}
	...
	// A materialized tree may still match after only adaptation rules changed.
	// Its git blob hashes, not the old adaptation fingerprint, prove whether
	// the current pristine boundary can be reused without the network.
	if (existsSync(statePath)) {
		const marker = JSON.parse(readFileSync(statePath, 'utf8'));
		if (marker.schemaVersion === 1 && typeof marker.lockFingerprint === 'string') {
	...
	for (const { sourcePath, targetPath } of planned) {
		...
		if (!pristineBytes.includes(0)) {
			pristineBytes = Buffer.from(
				applyAdaptedRewrites(pristineBytes.toString('utf8'), lock.adaptedRewrites, targetPath),
			);
		}
