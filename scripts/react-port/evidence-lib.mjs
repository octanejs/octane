...
import { expectedOctanePeerRange } from '../workspace-packages.mjs';
...
export function inspectBindingPackage(
	...
	if (manifest) {
		...
		const expectedOctanePeer = expectedOctanePeerRange(manifest.name);
		if (manifest.peerDependencies?.octane !== expectedOctanePeer) {
			issues.push(`octane peer must be ${expectedOctanePeer}`);
		}
