...
async function fixture() {
	const root = await mkdtemp(join(tmpdir(), 'solana-kit-types-'));
	const upstreamRoot = join(root, 'upstream');
	const adaptedRoot = join(root, 'adapted');
	await cp(join(REPO, 'packages/solana-kit/upstream/src'), upstreamRoot, {
		recursive: true,
	});
	await cp(join(REPO, 'packages/solana-kit/typetests'), adaptedRoot, {
		recursive: true,
	});
	const config = readTypeParityConfig(REPO, 'packages/solana-kit/audit/type-parity.json');
	return {
		root,
		upstreamRoot,
		adaptedRoot,
			...
			fileDispositions: config.fileDispositions.map(function remapEvidence(entry) {
				...
				return {
					...
					adaptedEvidence: entry.adaptedEvidence.map(function toFixture(path) {
						if (path.startsWith('packages/solana-kit/typetests/')) {
							return path.replace('packages/solana-kit/typetests/', 'adapted/');
						}
						return path;
					}),
				};
