...
import { inventoryFromIdentities } from './solana-kit-pristine-runtime.mjs';

const REPO = resolve(dirname(fileURLToPath(import.meta.url)), '../..');
const PACKAGE = join(REPO, 'packages/solana-kit');
...
function listedUpstreamArtifacts(kind) {
	const markdown = read('packages/solana-kit/UPSTREAM.md');
...
test('UPSTREAM.md dispositions cover every vendored runtime test file', () => {
	...
	const files = JSON.parse(read('packages/solana-kit/audit/upstream.lock.json'))
...
test('pristine vitest include matches pristine-run runtime dispositions', () => {
	const config = read('packages/solana-kit/tests/upstream-vitest.config.ts');
...
test('omitting an inventoried pristine identity fails exact-match validation', () => {
	const inventory = JSON.parse(read('packages/solana-kit/audit/pristine-runtime.json'));
...
test('adapted inventory preserves every selected pristine identity one-for-one', () => {
	const pristine = JSON.parse(read('packages/solana-kit/audit/pristine-runtime.json'));
	const adapted = JSON.parse(read('packages/solana-kit/audit/adapted-runtime.json'));
