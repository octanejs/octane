...
function assertionGroups(source, fileName) {
	...
	for (const match of source.matchAll(/\/\/\s*@ts-expect-error([^\n]*)\n\s*([^\n]+)/g)) {
		groups.push(
			`expect-error:${match[1].trim()}:${normalizeRefType(match[2].replace(/\s+/g, ' ').trim())}`,
		);
	}
	function visit(node) {
...
function normalizeRefType(value) {
	return value
		.replace(/\bReact\.Ref<HTMLTextAreaElement>/g, 'PublicTextareaRef')
		.replace(/\bTextareaAutosizeProps\['ref'\]/g, 'PublicTextareaRef');
}
