...
const CONFIG = 'packages/livestore/audit/test-classifications.json';
const MANIFEST = 'packages/livestore/audit/react-parity.json';
...
export function verifyLivestoreTestClassifications(root) {
	const testsRoot = resolve(root, 'packages/livestore/tests');
	const discovered = readdirSync(testsRoot, { recursive: true, withFileTypes: true })
		...
		.filter(function excludeUpstreamCopy(path) {
			return !path.includes('/tests/upstream/') && !path.includes('/.pristine-upstream-');
		})
		.sort();
	...
	if (JSON.stringify(discovered) !== JSON.stringify(declared)) {
		throw new Error('every port-authored livestore test must have exactly one classification');
	}
