...
import { renderTypeInventories, verifyTiptapTypes } from './tiptap-types-lib.mjs';
...
if (write) {
	...
	console.log(
		`Wrote tiptap type inventories (${inventory.upstream.length} files, ${inventory.upstream.reduce(
			function sum(a, f) {
...
if (pristine.status !== 0) {
	...
	throw new Error('tiptap pristine type lane failed');
}
...
if (adapted.status !== 0) {
	...
	throw new Error('tiptap adapted type lane failed');
}

console.log('tiptap type parity lanes passed.');
