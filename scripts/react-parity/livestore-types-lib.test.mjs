...
import { buildTypeInventory } from './livestore-types-lib.mjs';

async function fixture() {
	const root = await mkdtemp(join(tmpdir(), 'livestore-types-'));
	const upstreamRoot = join(root, 'upstream');
	const adaptedRoot = join(root, 'adapted');
	await cp(new URL('../../packages/livestore/audit/type-probes', import.meta.url), upstreamRoot, {
		recursive: true,
	});
	await cp(new URL('../../packages/livestore/typetests', import.meta.url), adaptedRoot, {
		recursive: true,
	});
...
test('rejects retargeting an adapted public import', async function rejectsRetargetedImport(t) {
	...
	const file = join(value.adaptedRoot, 'public-api.test-d.ts');
	const source = await readFile(file, 'utf8');
	await writeFile(file, source.replace("from '../src/mod.ts'", "from '../src/useStore.ts'"));
	assert.throws(function run() {
		buildTypeInventory(value.root, value.config);
