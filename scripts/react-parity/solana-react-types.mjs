...
if (write) {
	...
	console.log(
		`Wrote solana-react type inventories (${inventory.upstream.length} files, ${inventory.upstream.reduce(
			function sum(a, f) {
...
if (pristine.status !== 0) {
	...
	throw new Error('solana-react pristine type lane failed');
}
...
if (adapted.status !== 0) {
	...
	throw new Error('solana-react adapted type lane failed');
}

console.log('solana-react type parity lanes passed.');
