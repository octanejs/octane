...
const samples = integer('samples', 25);
const updates = integer('updates', 10000);
const warmups = 8;
...
const roots = [
	{
		label: 'current',
		root: options.has('current-root')
			? fs.realpathSync(path.resolve(options.get('current-root')))
			: REPO,
	},
];
...
assert.equal(quick, false, 'Focused comparisons use explicit full settings');
assert.ok(options.has('baseline-root') && options.has('baseline-ref'));
if (options.has('current-root')) assert.ok(options.has('current-ref'));
const referenceFile = path.join(
	REPO,
	'benchmarks/scoped-signals/results/2026-08-27/consolidation-native-costs.json',
);
const reference = JSON.parse(fs.readFileSync(referenceFile, 'utf8'));
const referenceLabelByRef = new Map([
	['cd9ed33754bfc3eeb144ba256dc9b437614e3e92', 'baseline'],
	['422c2c937784af835c64d6fae5a39162143c7a44', 'current'],
]);
const sourceRefs = {
	baseline: payload.environment.baselineRef,
	current: options.has('current-ref')
		? git('rev-parse', '--verify', options.get('current-ref') + '^{commit}').trim()
		: null,
};
const gcEvents = [];
const observer = new PerformanceObserver((list) => {
	for (const event of list.getEntries())
		gcEvents.push({
			...
			flags: event.detail?.flags,
		});
});
observer.observe({ entryTypes: ['gc'] });
let randomState = 0x422c2c93;
const nextRandom = () => {
...
Object.assign(payload, {
	suite: 'scoped-native-reads-focused-props',
	mode: process.execArgv.some((arg) => arg.startsWith('--cpu-prof'))
		? 'profile-diagnostic-not-primary-timing'
		: 'focused-paired-timing',
	limits: [
		'Synchronous production source renderer updates in happy-dom; no layout, paint, browser frame, mount, SSR, hydration, or async timing.',
		'Compile and bundle times, source hashing, public semantic checks, CPU counter calls, GC event delivery, and teardown verification are outside measured update intervals.',
		'The selected supplemental source toolchain is unchanged; this is not a locked workspace or CI gate.',
		'All measured wall samples, including samples overlapping GC or host scheduling delays, remain in the primary results.',
		'A wide paired uncertainty interval is inconclusive; no wall-clock pass threshold or outlier filter is introduced.',
	],
	method:
		'Use the unchanged production buildCase/dataFor/checkElement helpers and authored TSRX fixture; keep one live root per case/version, preallocate props, warm each version eight full update blocks, then time seeded ABBA/BAAB blocks with randomized case order. Public output/identity and native signal updates are verified outside each timed block. All timing samples are retained.',
	configuration: {
		pairs: samples,
		updatesPerBlock: updates,
		warmupBlocksPerVersion: warmups,
		seed: '0x422c2c93',
		cases: [
			'unread-block/native-off',
			'unread-block/native-on',
			'single-block/native-on',
			'repeated-block/native-on',
			'distinct-block/native-on',
		],
		pairOrder: 'Seeded ABBA or BAAB; both versions run twice per pair',
		lifecycle:
			'Roots live across the focused update measurement; mounting and SSR remain in the separate unchanged full harness.',
	},
	sourceRefs,
	sourceRoots: roots.map(({ label, root }) => ({ label, root, immutableRef: sourceRefs[label] })),
	diagnosticsPolicy:
		'Process CPU, main-thread CPU and GC overlaps are separate diagnostics and never replace primary wall timing or justify removing samples.',
	invocationParity:
		'Every version uses the same measure function, same loop body, one flushSync callback per update, preallocated props arrays of equal length and identical public output/identity checks. No per-version wrapper or special-case timed path.',
	referenceEvidence: {
		file: referenceFile,
		sha256: hashFile(referenceFile),
		role: 'Require exact bundle bytes when rebuilding archived cd9/422c; final candidate records hashes without claiming equality.',
	},
	preservedOriginalRunnerSha256: '93e81cbecb25551c5afa02db25a81f3af73d4ae872e2886755f8b3f71e2374c0',
	scratch,
	warmupRecords: [],
	pairs: [],
	summaries: [],
	outlierPolicy:
		'Retain every sample and GC event. Report arithmetic absolute timings, paired ratio distribution, geometric mean ratio and Student-t95% interval on log ratios. Never drop outliers or infer parity from a wide interval.',
	hostBefore: {
...
const totalCpu = (value) => value.user + value.system;
function prepare(entry) {
	const data = dataFor(entry);
	const container = document.createElement('div');
	document.body.appendChild(container);
	const root = entry.api.createRoot(container);
	root.render(entry.api.Component, data.props);
	const host = checkElement(container, data.value(), 0);
	const props = Array.from({ length: updates }, (_, tick) => ({ ...data.props, tick: tick + 1 }));
	return { entry, data, container, root, host, props };
}
function measure(prepared) {
	const { entry, root, props } = prepared;
	const cpuBefore = process.cpuUsage();
	const threadBefore =
		typeof process.threadCpuUsage === 'function' ? process.threadCpuUsage() : null;
	const start = performance.now();
	for (const value of props) entry.api.flushSync(() => root.render(entry.api.Component, value));
	const end = performance.now();
	const cpu = process.cpuUsage(cpuBefore);
	const thread = threadBefore === null ? null : process.threadCpuUsage(threadBefore);
	checkElement(prepared.container, prepared.data.value(), updates, prepared.host);
	return {
		target: entry.target,
		start,
		end,
		wall_us: ((end - start) * 1000) / updates,
		cpu_us: totalCpu(cpu) / updates,
		main_thread_cpu_us: thread === null ? null : totalCpu(thread) / updates,
	};
}
function verifySignal(prepared, value) {
	if (prepared.data.update === null) return;
	prepared.entry.api.flushSync(() => prepared.data.update(value));
	checkElement(prepared.container, prepared.data.value(), updates, prepared.host);
}
function ratioSummary(values) {
	const sorted = [...values].sort((a, b) => a - b);
	const logs = values.map(Math.log);
	const stats = summarizeSamples(logs, { scoreMode: 'mean' });
	const q = (p) => sorted[Math.min(sorted.length - 1, Math.floor(sorted.length * p))];
	return {
		samples: values.length,
		arithmeticMean: values.reduce((a, b) => a + b, 0) / values.length,
		geometricMean: Math.exp(stats.mean),
		geometricCi95: [Math.exp(stats.mean - stats.moe), Math.exp(stats.mean + stats.moe)],
		min: sorted[0],
		p10: q(0.1),
		p25: q(0.25),
		median: q(0.5),
...
try {
	for (const target of roots) {
		const refs = sourceRefs[target.label];
		target.compilerInputs = compilerSnapshot(target.root);
		if (refs !== null)
			for (const input of target.compilerInputs)
				assert.equal(
					input.sha256,
					hash(git('show', refs + ':' + input.path)),
					'Archived compiler differs from Git',
				);
	}
	...
	for (const scenario of CASES.filter((item) => item.name.endsWith('-block'))) {
		for (const enabled of scenario.reads ? [true] : [false, true]) {
			const pair = [];
			for (const target of roots) {
				const entry = await buildCase(target, scenario, enabled, 'client');
				const record = compiled.at(-1);
				const expectedLabel = referenceLabelByRef.get(sourceRefs[target.label]);
				if (expectedLabel !== undefined) {
					const expectedName = expectedLabel + record.name.slice(record.name.indexOf(':'));
					const old = reference.compiled.find((item) => item.name === expectedName);
					assert.ok(old);
					assert.equal(
						record.bundleSha256,
						old.bundleSha256,
						'Frozen source must produce the exact existing measured bundle',
					);
					record.matchesExistingBundle = true;
				}
				pair.push(prepare(entry));
			}
			assert.deepEqual(
				pair.map((entry) => entry.entry.target),
				['baseline', 'current'],
			);
			const name = scenario.name + (enabled ? ':native' : ':ordinary');
			cases.push({ name, pair });
			samplesByCase.set(name, []);
		}
	}
	for (let warmup = 0; warmup < warmups; warmup++) {
		for (const scenario of shuffled(cases)) {
			const order = warmup % 2 === 0 ? [0, 1] : [1, 0];
			const records = order.map((side) => measure(scenario.pair[side]));
			payload.warmupRecords.push({ warmup, case: scenario.name, records });
		}
		await nextTurn();
		console.log(
			'Focused TSRX warmup ' +
				...
				'/' +
				warmups +
				' complete; output and identity controls passed.',
		);
	}
	for (let round = 0; round < samples; round++) {
		for (const scenario of shuffled(cases)) {
			const order = nextRandom() % 2 === 0 ? [0, 1, 1, 0] : [1, 0, 0, 1];
			const records = order.map((side) => measure(scenario.pair[side]));
			const byTarget = (target, metric) =>
				records.filter((row) => row.target === target).reduce((sum, row) => sum + row[metric], 0) /
				2;
			const row = {
				index: pairIndex++,
				round,
				case: scenario.name,
				order: order.map((side) => (side === 0 ? 'baseline' : 'current')),
				records,
				ratio: {},
			};
			for (const metric of ['wall_us', 'cpu_us', 'main_thread_cpu_us']) {
				if (records.some((record) => record[metric] === null)) continue;
				row.ratio[metric] = byTarget('current', metric) / byTarget('baseline', metric);
			}
			payload.pairs.push(row);
			samplesByCase.get(scenario.name).push(row);
			for (const prepared of scenario.pair) {
				verifySignal(prepared, round + 8);
				verifySignal(prepared, 7);
			}
			await nextTurn();
		}
		if ((round + 1) % 5 === 0 || round === samples - 1)
			console.log(
				'Focused TSRX pair round ' +
					(round + 1) +
					'/' +
					samples +
					' complete; native signal controls passed.',
			);
	}
	for (const scenario of cases) {
		const rows = samplesByCase.get(scenario.name);
		const summary = { case: scenario.name, pairs: rows.length, ratios: {}, absolute: {} };
		for (const metric of ['wall_us', 'cpu_us', 'main_thread_cpu_us']) {
			if (rows.some((row) => row.ratio[metric] === undefined)) continue;
			summary.ratios[metric] = ratioSummary(rows.map((row) => row.ratio[metric]));
			summary.absolute[metric] = {};
			for (const target of ['baseline', 'current'])
				summary.absolute[metric][target] = timingStatForJson(
					summarizeSamples(
						rows.flatMap((row) =>
							row.records
	...
	for (const scenario of cases)
		for (const prepared of scenario.pair) {
			...
			assert.equal(prepared.container.textContent, '');
			prepared.data.update?.(999);
			assert.equal(
				prepared.container.textContent,
				'',
				'A live producer must not revive a retired root',
			);
	...
	for (const row of payload.pairs)
		for (const record of row.records) {
			const events = gcEvents.filter(
				(event) => event.startTime < record.end && event.startTime + event.duration > record.start,
			);
			record.gcEvents = events.length;
			record.gcWallMs = events.reduce(
				(sum, event) =>
					sum +
					Math.min(record.end, event.startTime + event.duration) -
					Math.max(record.start, event.startTime),
				0,
			);
		}
	assert.equal(
		hashFile(fixtureFile),
		hash(fixture),
		'Authored fixture changed during focused measurement',
	);
	for (const [file, contents] of snapshotInputs)
		assert.equal(
			hashFile(file),
			hash(contents),
			'Source changed during focused measurement: ' + file,
		);
	for (const target of roots) {
		const ref = sourceRefs[target.label];
		...
		for (const [file, contents] of snapshotInputs) {
			...
			assert.equal(
				hash(contents),
				hash(git('show', ref + ':' + relative)),
				'Archived package bytes differ from Git',
			);
		}
	}
	...
	assert.ok(process.env.BENCH_JSON, 'Focused evidence requires a fresh BENCH_JSON path');
	assert.equal(fs.existsSync(process.env.BENCH_JSON), false, 'Never overwrite focused evidence');
