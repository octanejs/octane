...
// Native Inferno portal-swarm twin. Same three sections as the octane app:
//   A        — createPortal written inline at JSX child position.
...
//              (reference-identical portal VNodes).
// For Inferno, A and B are the SAME mechanism (both produce a portal VNode
...
class SectionA extends Component {
	constructor(props) {
		super(props);
		this.state = { open: false, tick: 0, distinct: false };
		bindA(
			(open) => this.setState({ open }),
			(tick) => this.setState({ tick }),
			(distinct) => this.setState({ distinct }),
		);
	}

	render() {
		const { open, tick, distinct } = this.state;
		return (
