...
// React 19 svg-dashboard — the canonical VDOM baseline. Same shared state
// snapshots and identical rendered DOM as the sibling fixtures. Rows and
// domain sections are memo'd (idiomatic optimized React): ops that leave row
// objects reference-identical bail out per row, so the measured work is
// React's keyed diff + re-render of the rows an op actually touched.

const EdgePath = memo(function EdgePath({ e }) {
	return <path className={e.cls} d={e.d} style={e.style} {...e.attrs} />;
});
...
export default function App() {
	...
	bindSetters({ setTopo, setCharts, setUi });
	...
	return (
		<svg className="dash" width="1200" height="800" viewBox={ui.viewBox}>
			...
			<g className="root" transform={ui.rootTransform}>
				...
				<g className="overlay" ref={overlayRef} />
