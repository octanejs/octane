...
export function PartnersRail({
	...
	const rowsByTier = getPartnerTierGroupsForPlacement(partners, placementContext);
	...
	return (
		<div className="group/rail flex flex-col border-l border-gray-500/20 rounded-bl-lg overflow-hidden w-full">
			<div className="w-full flex gap-2 justify-between items-center border-b border-gray-500/20 px-3 py-2">
				<Link className="font-medium opacity-60 hover:opacity-100 text-xs" to={titleTo}>
					{title}
				</Link>
				<a
			...
			{rowsByTier.map((row) => {
				...
				return (
					<div key={row.tier} className="relative flex flex-wrap items-stretch w-full">
