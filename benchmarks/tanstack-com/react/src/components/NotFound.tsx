...
function scoreSectionDestination(destination: NotFoundDestination, pathname: string) {
	const tokens = getPathTokens(pathname);
...
function DestinationCard({ destination }: { destination: NotFoundDestination }) {
...
export function NotFound({ children }: { children?: React.ReactNode }) {
	...
	const suggestions = React.useMemo(() => getSuggestedDestinations(pathname), [pathname]);
				...
				<section className="space-y-7 lg:self-start">
					...
					<div className="space-y-4">
						...
						<p className="max-w-xl text-base leading-7 text-gray-600 dark:text-gray-400 md:text-lg">
							The address might have moved, been renamed, or picked up an extra segment. I checked
							the path and found a few places that are more likely to help.
						</p>
					</div>
					...
					<div>
						...
						<div className="mt-3 grid gap-3">
							{suggestions.map((suggestion) => (
								<DestinationCard key={suggestion.key} destination={suggestion} />
							))}
						</div>
					</div>
				</section>
