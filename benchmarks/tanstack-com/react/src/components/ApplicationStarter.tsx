...
import { ArrowRight, Check, ChevronDown, Copy, Download, Loader2, Rocket } from 'lucide-react';
...
function getHostingDeployPartnerId(partnerId: string): HostingDeployPartnerId | undefined {
	switch (partnerId) {
		case 'cloudflare':
		case 'lovable':
		case 'netlify':
...
export function ApplicationStarter({
	...
	const [transientAction, setTransientAction] = React.useState<StarterTransientAction | null>(null);
	const [hasFocusedPromptInput, setHasFocusedPromptInput] = React.useState(false);
	const [isPromptFocused, setIsPromptFocused] = React.useState(false);
	const [isMacShortcutPlatform, setIsMacShortcutPlatform] = React.useState(false);
	const [showPackageManagerOptions, setShowPackageManagerOptions] = React.useState(false);
	const [showToolchainOptions, setShowToolchainOptions] = React.useState(false);
	const canRevealOptions = hasInput && !hasMigrationRepositoryUrlError && !isGenerating;
	const canUseFinalActions =
		hasRevealedOptions && hasInput && !hasMigrationRepositoryUrlError && !isGenerating;
	const transientActionTimerRef = React.useRef<ReturnType<typeof setTimeout> | null>(null);
	const showTransientActionFeedback = React.useCallback((action: StarterTransientAction) => {
	...
	const isDeployFeedbackActive = isSelectedHostingDeployPending || transientAction === 'deploy';
	...
	const selectedHostingDeployHref = React.useMemo(
		() =>
			selectedPromptDeployProvider && result?.prompt
				? buildStarterPromptDeployUrl(selectedPromptDeployProvider, result.prompt)
				: undefined,
		[result?.prompt, selectedPromptDeployProvider],
	);
	const netlifyStartHref = React.useMemo(
		() => (resultPrompt ? buildStarterPromptDeployUrl('netlify', resultPrompt) : undefined),
		[resultPrompt],
	);
	...
	const trackSelectedHostingDeployLink = React.useCallback(() => {
		...
		trackActivation({
			action: selectedHostingDeployPartner === 'netlify' ? 'netlify_start' : 'deploy',
	...
	const renderCopyPromptButton = () => (
		...
		>
			...
			{isGeneratingPrompt ? loadingPhrase : isPromptCopied ? 'Copied' : primaryActionLabel}
		</Button>
	);
	const renderCopyCliCommandButton = () => (
		...
		>
			...
			{isGeneratingPrompt ? 'Preparing...' : isCommandCopied ? 'Copied' : 'Copy CLI Command'}
		</Button>
	);
	const renderActionAnchor = ({
		...
		return (
			<Button
				...
				className={twMerge(className, disabled && 'pointer-events-none opacity-50')}
			>
				{transientAction === action || waitingForHref ? (
					<Loader2 className={twMerge('animate-spin', size === 'xs' ? 'h-3 w-3' : 'h-4 w-4')} />
				) : (
					icon
				)}
				{transientAction === action ? 'Opening...' : waitingForHref ? 'Preparing...' : label}
			</Button>
		);
	};
	const renderSelectedHostingDeployButton = () => {
		...
		if (selectedPromptDeployProvider) {
			const disabled =
				!canUseFinalActions || !selectedHostingDeployHref || transientAction === 'deploy';
			...
			return (
				...
				>
					...
					{isDeployFeedbackActive ? 'Opening...' : waitingForHref ? 'Preparing...' : 'Deploy'}
				</Button>
			);
		}
									...
									<div className="border-b border-gray-200 px-3 py-2 dark:border-gray-800">
										...
										<input
											...
											className={twMerge(
												...
												palette.ring,
												hasMigrationRepositoryUrlError && 'border-red-300 dark:border-red-800',
											)}
										/>
										{hasMigrationRepositoryUrlError ? (
								...
								{!showOptionsSection ? (
									<div className="border-t border-gray-200 px-3 py-2 dark:border-gray-800">
										...
										>
											...
											Next
											{enableHotkeys ? <SubmitShortcutHint isMac={isMacShortcutPlatform} /> : null}
										</Button>
									</div>
								) : null}
							</div>
													...
													<div className="mt-2 flex flex-wrap gap-2">
														{starterPackageManagers.map((packageManager) => (
															<StarterChipButton
																...
																palette={palette}
																selected={selectedPackageManager === packageManager}
															>
																{packageManager}
															</StarterChipButton>
														))}
											...
											<div className="bg-gray-50/70 px-5 py-4 dark:bg-gray-900/50">
												<div className="flex flex-col gap-4">
													...
													<div className="flex flex-wrap items-center gap-3">
														...
														{showCliExportActions ? renderCopyCliCommandButton() : null}
													</div>
													...
													{showCliExportActions && showMoreActions ? (
														<div className="flex flex-wrap items-center gap-2">
															...
															<Button
																...
																disabled={!canUseFinalActions || transientAction === 'clone'}
															...
															>
																...
																{transientAction === 'clone' ? 'Opening...' : 'Clone to GitHub'}
															</Button>
		...
		<Collapsible open={open} onOpenChange={onOpenChange}>
			<div className={twMerge(compact ? 'pt-1' : 'pt-2')}>
				...
				>
					{title}
					<ChevronDown
						className={twMerge('h-3 w-3 transition-transform duration-200', open && 'rotate-180')}
					/>
				</CollapsibleTrigger>
