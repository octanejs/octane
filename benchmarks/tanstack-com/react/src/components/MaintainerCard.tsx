...
import { getRoleInLibrary, Maintainer, getPersonsMaintainerOf } from '~/libraries/maintainers';
...
const MaintainerSocialIcons: Record<keyof Maintainer['social'], React.ReactNode> = {
	twitter: (
	...
	bluesky: (
		<svg viewBox="0 0 24 21" className="w-5 h-5" fill="currentColor" aria-hidden="true">
...
export function CompactMaintainerCard({ maintainer }: CompactMaintainerCardProps) {
	return (
		<a
		...
		>
			...
			<div className="absolute inset-0 p-3 flex flex-col justify-end">
				<div className="text-white">
					<div className="text-sm font-bold leading-tight">{maintainer.name}</div>
				</div>
			</div>
...
export function MaintainerRowCard({ maintainer, libraryId }: MaintainerRowCardProps) {
				...
				<div className="flex-1 min-w-0 overflow-hidden">
					<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
						<div className="flex items-center gap-3">
							...
							{libraryId && <RoleBadge maintainer={maintainer} libraryId={libraryId} />}
						</div>
						...
						<div className="flex flex-wrap gap-2 mt-2 max-w-full">
							...
							{!libraryId &&
								...
								libraries
									...
									.map((library) => <LibraryBadge key={library.id} library={library} />)}
		...
		>
			...
			<div className="p-3 space-y-2">
				<div className="flex items-center justify-between">
					<span className="text-base font-bold" id={`maintainer-name-${maintainer.github}`}>
						{maintainer.name}
					</span>
					<div className="flex items-center gap-2">
						{libraryId && <RoleBadge maintainer={maintainer} libraryId={libraryId} />}
				...
				{!hideLibraries && !libraryId && libraries.length > 0 && (
					<div className="flex flex-wrap gap-1.5 pt-1">
						{libraries.slice(0, showAllLibraries ? undefined : 2).map((library) => (
