...
// Native keyed Inferno implementation — the Inferno column for the same
// create/replace/update/select/swap/remove/runlots/clear ops the octane apps are
// driven through by run.mjs.
//
// operation contract. Inferno class setState commits synchronously in event
// handlers, so the harness captures the renderer work inside each timed click.
...
// ── Deterministic shuffle machinery (identical across all nine targets,
...
// renderer retries cannot skew the sequence. Identical clicks therefore
// produce identical permutations across all nine targets.
...
class Row extends Component {
	shouldComponentUpdate(nextProps) {
		return nextProps.selected !== this.props.selected || nextProps.item !== this.props.item;
	}

	render() {
		const { selected, item, dispatch } = this.props;
		return (
...
Jumbotron.defaultHooks = { onComponentShouldUpdate: () => false };

class Main extends Component {
	constructor(props) {
		super(props);
		this.state = initialState;
		this.dispatch = (action) => this.setState((state) => listReducer(state, action));
	}
