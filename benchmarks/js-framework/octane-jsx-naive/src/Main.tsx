...
import { actions, bindActions } from './actions.js';
import Row from './Row.tsx';

// NAIVE-authoring twin of ../../octane-jsx/src/Main.tsx — the same app, same
// button ids and rendered rows, but written the way a React dev naturally
// writes it: JS control flow only (`items.map`, ternaries) and a cross-module
// <Row/> component per row (see ./Row.tsx for the full list of tuned-path
// opt-outs). The DOM/button contract is identical, so ../../run.mjs and
// ../../run-reorder.mjs drive this fixture unchanged via the TARGETS env; the
// headline metric is the intra-octane naive/tuned ratio (../../README-naive.md).
//
...
export default function Main() {
	...
	// Rows dispatch through the imported `actions` object (member-callee — the
	// naive handler shape); bind the real implementations each render.
	bindActions(select, remove);
