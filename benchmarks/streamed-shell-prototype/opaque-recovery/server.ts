...
const server = createServer(async (request, response) => {
	...
	if (url.pathname === '/') {
		...
		// Cases run serially; the same module-scope query site is compiled for SSR
		// and for the client. The client deliberately has no loader installed.
		(
			globalThis as typeof globalThis & { __opaqueSessionLoad?: () => Promise<string> }
		).__opaqueSessionLoad = () => {
			session.loads++;
			return pending;
		};
		...
		const stream = renderToPipeableStream(
			...
			{},
			{
				streamedSignals: { buildId: 'opaque-build', documentId: name },
				onError(error) {
