...
export const TEST7_CHUNKS = [encode('Hello, '), encode('World! '), encode('This is text.')];
...
export const TEST11_CHUNKS = [new Uint8Array([0x00, 0x01, 0x02, 0xff, 0xfe, 0xfd])];
...
export const singleRawStreamFn = createServerFn().handler(async () => {
	const stream = createDelayedStream([encode('chunk1'), encode('chunk2'), encode('chunk3')], 50);
	return {
...
export const multipleRawStreamsFn = createServerFn().handler(async () => {
	const stream1 = createDelayedStream([encode('stream1-a'), encode('stream1-b')], 30);
	const stream2 = createDelayedStream([encode('stream2-a'), encode('stream2-b')], 50);
	return {
...
export const mixedStreamingFn = createServerFn().handler(async () => {
	const rawStream = createDelayedStream([encode('mixed-raw-1'), encode('mixed-raw-2')], 50);

	return {
		immediate: 'immediate-value',
		deferred: new Promise<string>((resolve) => setTimeout(() => resolve('deferred-value'), 100)),
...
export const TEST12_STREAM_A_EXPECTED = concatBytes(TEST12_STREAM_A_CHUNKS.map(([chunk]) => chunk));
export const TEST12_STREAM_B_EXPECTED = concatBytes(TEST12_STREAM_B_CHUNKS.map(([chunk]) => chunk));
...
export const TEST13_EXPECTED = concatBytes(TEST13_CHUNKS.map(([chunk]) => chunk));
...
export const TEST14_STREAM_A_EXPECTED = concatBytes(TEST14_STREAM_A_CHUNKS.map(([chunk]) => chunk));
export const TEST14_STREAM_B_EXPECTED = concatBytes(TEST14_STREAM_B_CHUNKS.map(([chunk]) => chunk));
export const TEST14_STREAM_C_EXPECTED = concatBytes(TEST14_STREAM_C_CHUNKS.map(([chunk]) => chunk));
...
export const LATE_STREAM_CHUNKS = [encode('late-1'), encode('late-2'), encode('late-3')];
...
export const IMMEDIATE_STREAM_CHUNKS = [encode('immediate-1'), encode('immediate-2')];
...
export function createStreamConsumer() {
	...
	return async function consumeStream(
		...
		const actualStream =
			stream instanceof RawStream ? stream.stream : (stream as ReadableStream<Uint8Array>);
