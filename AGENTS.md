...
the `@octanejs/devtools` in-page developer tools, the private
...
- `packages/octane-devtools/` (npm: `@octanejs/devtools`) — the in-page
  devtools panel (live component tree, hook state, profiler-backed
  performance, agent prompt export) over the `octane/devtools` runtime
  bridge; enabled per dev server with `octane({ devtools: true })`, always
  compiled away from builds.
...
- `packages/{zustand,jotai,i18next,tanstack-ai,tanstack-query,tanstack-store,tanstack-form,apollo-client,motion,dnd-kit,stylex,tanstack-router,remix-router,nuqs,tanstack-table,tanstack-virtual,lexical,tiptap,floating-ui,radix,hook-form,base-ui,sonner,recharts,visx,three,lucide,redux,redux-toolkit,testing-library,mdx}/`
  (npm: `@octanejs/*`) — framework bindings, each an octane port of a React
  library (state, AI, data-fetching, animation, styling, routing, URL
  search-params state, editor,
