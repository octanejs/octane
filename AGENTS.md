...
pnpm typecheck
pnpm sync          # refresh shared generated state before pushing
...
Before pushing commits to `origin`, `upstream`, or any other remote, run
`pnpm sync`. Review and commit any changes it produces so the generated state is
included in the pushed commits.
