...
`@octanejs/vite-plugin` metaframework (plus its Vercel and Cloudflare deploy
adapters), the `@octanejs/mcp-server` MCP server, the private
...
- `packages/{zustand,jotai,i18next,tanstack-ai,tanstack-query,tanstack-store,tanstack-form,apollo-client,motion,dnd-kit,stylex,tanstack-router,remix-router,nuqs,tanstack-table,tanstack-virtual,lexical,tiptap,floating-ui,radix,hook-form,base-ui,sonner,cmdk,recharts,visx,three,lucide,redux,redux-toolkit,testing-library,mdx}/`
  ...
  positioning, UI primitives, forms and form state, toast notifications, command menus, charts and visualization primitives, web 3D, icons,
