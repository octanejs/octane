...
New package: the `octane` command line.
...
Octane is a compiler framework, so its misconfigurations mostly fail *quietly*.
...
fill in *missing* input and only in a real terminal; in a pipe or under `CI` a
