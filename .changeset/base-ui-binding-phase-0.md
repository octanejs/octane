...
`/toggle`, `/toggle-group`, `/avatar`, `/switch`, `/use-render`, `/merge-props`).
...
Phase 2 (in progress): the Field/Form context infrastructure + `Switch` — with the octane
uncontrolled-input adaptation (initial-checked attribute + imperative `.checked` property via
the native setter + native change dispatch), matching React's controlled input byte-for-byte.
See `docs/base-ui-migration-plan.md` for the phased plan and running progress.
