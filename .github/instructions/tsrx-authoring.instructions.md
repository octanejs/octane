---
description: >-
  Full .tsrx authoring reference: components, text holes, events, control flow,
  refs, scoped styles and themes ($class, apply)
applyTo: '**/*.tsrx'
---
# Authoring `.tsrx`
...
Every assigned block is a theme and keeps every selector, including element and
descendant rules. A class-map entry carries the hash too, so those rules can
match elements that use an entry such as `theme.dark`.
