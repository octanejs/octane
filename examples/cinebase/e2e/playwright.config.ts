...
const address = await resolveExampleServerAddress({
	baseURLEnv: 'CINEBASE_E2E_BASE_URL',
	portEnv: 'CINEBASE_E2E_PORT',
...
export default defineConfig({
	...
	webServer: address.external
		? undefined
		: {
				command: `NODE_ENV=production CINEBASE_DIST=1 PORT=${address.port} HOST=${address.host} node server.mjs`,
