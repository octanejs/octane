...
import { useState, useRef, useEffect, type OctaneNode } from 'octane';
import { OwnerStackCases } from './owner-stack-cases.tsx';
...
const NestedCard = ({ title, children }: { title: string; children: OctaneNode }) => {
	return (
			...
			<div className="space-y-4">
				...
				<div>
					...
					<textarea
						id="test-textarea"
						value={textareaValue}
						onInput={(event) => setTextareaValue((event.target as HTMLTextAreaElement).value)}
...
function FixtureStub(props: { name: string; testId: string }) {
	return (
		<section className="border rounded-lg p-4 bg-amber-50" data-testid={props.testId}>
			<h2 className="text-lg font-bold mb-2">{props.name}</h2>
			<p className="text-sm text-amber-800">
				Stubbed in the Octane port — upstream fixture depends on React-only / blocked binding paths
				(R3F, Pierre, iframe host, shadow-dom edge, perf grids).
			</p>
		</section>
	);
}

function PerfGrid(_props: { rowCount: number; columnCount: number }) {
	return <FixtureStub name="PerfGrid" testId="perf-grid-stub" />;
}
function HeavyPage(_props: { view: unknown }) {
	return <FixtureStub name="HeavyPage" testId="heavy-page-stub" />;
}
function PierreDiffPreview() {
	return <FixtureStub name="PierreDiffPreview" testId="pierre-diff-preview-stub" />;
}
function PierreDiffFixture() {
	return <FixtureStub name="PierreDiffFixture" testId="pierre-diff-fixture" />;
}
function ShadowDomEdgeFixture() {
	return <FixtureStub name="ShadowDomEdgeFixture" testId="shadow-dom-edge-fixture" />;
}
function ThreeFiberFixture() {
	return <FixtureStub name="ThreeFiberFixture" testId="three-fiber-fixture" />;
}
function ThreeJsFixture() {
	return <FixtureStub name="ThreeJsFixture" testId="three-js-fixture" />;
}
function IframeFixture() {
	return <FixtureStub name="IframeFixture" testId="iframe-fixture" />;
}
type HeavyView = unknown;
function parseHeavyView(_raw: string | null): HeavyView | null {
	return null;
}
          ...
          Next fixture exposes the same data-testid so one spec runs on both. */}
			...
			<header className="mb-4">
				<h1 className="text-2xl font-bold" data-testid="main-title">
					Octane Grab E2E Test Page (ported from react-grab e2e-app-vite)
				</h1>
				<p className="text-gray-600" data-testid="main-description">
					1:1 port of apps/e2e-app-vite for validating @octanejs/grab
				</p>
			</header>
