...
const address = await resolveExampleServerAddress({
	baseURLEnv: 'WAYFINDER_EXAMPLE_BASE_URL',
	portEnv: 'WAYFINDER_EXAMPLE_PORT',
	...
	webServer: address.external
		...
		: {
				command: productionPreview
					? `HOST=${address.host} PORT=${address.port} node dist/server/entry.js`
					: `./node_modules/.bin/vite --host ${address.host} --port ${address.port} --strictPort`,
				url: address.baseURL,
				cwd: exampleRoot,
