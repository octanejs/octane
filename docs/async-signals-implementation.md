...
The final native-presentation benchmark uses the actual StyleX compiler and canonical native reader. For 5,000 progress updates, targeted subscriptions remove 5,000 whole-source snapshots, 25,000 projected reads, and 5,000 StyleX merges while retaining the same terminal DOM. Seven-sample median synchronous update time is 11.55 ms versus 20.24 ms for whole-source projection; unrelated updates are 3.96 ms versus 12.35 ms. Mean relative uncertainty across these lanes is approximately 5–17%. These happy-dom measurements establish work avoided, not browser paint, input latency, or production speedup. An actual style variant change still performs its required merge.
