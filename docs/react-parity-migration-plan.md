...
| `ReactDOMEventPropagation-test.js` (89) | Bubble inner→outer for every event; **bubbling through portals** (✅ confirmed working incl. nested portals — see §8); mouseenter/leave + pointerenter/leave **synthesis** (non-bubbling, per-element `:1444,:1517`). NB: Octane has **no capture-phase JSX handlers** (`onClickCapture`) — that is a separate design question, not assumed in scope. | High |
