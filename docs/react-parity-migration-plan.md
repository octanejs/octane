...
| Omitted dependency arrays | **Intentional divergence** — for effect-family hooks, `useMemo`, `useCallback`, `useImperativeHandle`, and locally declared transparent custom wrappers in full-compiled `.tsrx`/`.tsx` modules, omission asks the compiler to infer dependencies from lexical captures. Explicit arrays retain React semantics; `null` is the explicit every-render/recompute form. |
| Causal authored-state model | **Intentional opt-in divergence.** `compiler.stateModel.default: 'causal'` rejects state writes during render and hook purity phases before updater/reducer evaluation or same-value bailout. The rollout default remains `'permissive'`, preserving the existing bounded render-replay behavior used by parity fixtures and approved dependency packages. Effect setup/cleanup findings remain report-only until the lifecycle, replacement-primitive, and Callback Provenance phases of `strict-state-plan.md`. |
...
> `onInput` is the per-edit handler for text controls. The compiler reports
> `OCTANE_NATIVE_TEXT_ONCHANGE` for statically known text hosts that appear to carry
> React-style `onChange`; a development fallback covers final ambiguous uncontrolled
> props. Deliberate native commit behavior opts out with the non-serialized
> `suppressNativeChangeWarning` host hint. The 2026-06-24 reverted test slice was NOT
> recoverable from git — the new
