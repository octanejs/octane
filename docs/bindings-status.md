...
[`packages/shadcn`](../packages/shadcn) `0.0.1` — ports `shadcn-ui/ui (radix base)@4baadbc6517070ae8f8feb2c97037adc2b305544 + shadcn@4.14.1`. Status data: [`packages/shadcn/status.json`](../packages/shadcn/status.json).

Tiers 1 and 2 complete plus the first Tier-3 composites — 40 component families (~185 exported components) of the pinned Radix base: the 16 primitive-free families (Alert, Badge, Breadcrumb, Button, Card, Empty, Input, Item, Kbd, Label, NativeSelect, Pagination, Separator, Skeleton, Spinner, Table, Textarea), the overlay family (Dialog, AlertDialog, Sheet, Tooltip, Popover, HoverCard), the menu family (DropdownMenu, ContextMenu, Menubar, NavigationMenu incl. navigationMenuTriggerStyle), the input controls (Checkbox, RadioGroup, Switch, Slider, Select, Tabs, Toggle, ToggleGroup), the structure family (Accordion, Collapsible, Avatar, AspectRatio, Progress, ScrollArea), and the Sidebar (all 24 parts + useSidebar + useIsMobile) and Field families — plus `cn()`, the default neutral theme tokens (`@octanejs/shadcn/theme.css`), and a generated shadcn-CLI-compatible registry (packages/shadcn/registry, `registry:build`/`registry:check`). Every cva variant map, class string, and `data-slot`/`data-variant`/`data-size`/`data-state` attribute is preserved byte-for-byte from the pinned upstream sources. Remaining Tier-3 composites (command — blocked on @octanejs/cmdk publishing — sonner, chart, data-table) and registry hosting/CLI e2e are planned; see docs/shadcn-port-plan.md.
...
- Variant visuals require a shadcn style stylesheet (e.g. style-nova): components carry semantic `cn-*` hooks plus structural utilities, matching upstream's multi-style system; the style CSS itself is Tailwind-@apply source compiled by the consumer's Tailwind v4 build.
