...
| [`@octanejs/three`](#octanejsthree) | `@react-three/fiber@9.6.1 (2a528745)` | Technical-preview Milestones 0, 2, and 3 surface: renderer configuration and DOM Canvas boundary, compiler ABI and renderer-local Three intrinsic types, catalogue and both extend forms, primitive/args construction, Three prop application, attachment, ordered placement/recreation, retained visibility, lifecycle/ref delivery, ownership-aware disposal, promise-returning programmatic roots, callable root state, scene/camera/raycaster and resize/DPR/viewport configuration, shadows/colors, one shared frame loop, global effects, useStore/useThree/useFrame/useGraph and managed-instance helpers, plus a deterministic testing harness. Pointer events and later asset/portal surfaces remain deferred. | Octane owns component execution, hooks, context, scheduling, Suspense, refs, and effects instead of embedding React Reconciler; The programmatic root renders an Octane component plus props rather than a React element descriptor; The upstream callable store selector remains order-based because dynamic function calls cannot receive compiler slots; compiler-visible useStore(selector) and useThree(selector) preserve Octane's conditional-hook semantics; Pointer event configuration rejects clearly until the Milestone 4 event system lands; buildGraph omits unnamed and array-valued material entries instead of publishing empty or undefined material keys; Removing a pierced prop resets its original nested target; R3F 9.6.1 mistakenly writes that default to the leaf key on the root object | Three scene modules are classified client-only and Canvas.children is configured as an omitted server region. Package-level DOM-shell server output and hydration adoption remain Milestone 7 work. | 2026-07-16 |
...
Technical-preview Milestones 0, 2, and 3 surface: renderer configuration and DOM Canvas boundary, compiler ABI and renderer-local Three intrinsic types, catalogue and both extend forms, primitive/args construction, Three prop application, attachment, ordered placement/recreation, retained visibility, lifecycle/ref delivery, ownership-aware disposal, promise-returning programmatic roots, callable root state, scene/camera/raycaster and resize/DPR/viewport configuration, shadows/colors, one shared frame loop, global effects, useStore/useThree/useFrame/useGraph and managed-instance helpers, plus a deterministic testing harness. Pointer events and later asset/portal surfaces remain deferred.
...
- Pointer event configuration rejects clearly until the Milestone 4 event system lands.
- buildGraph omits unnamed and array-valued material entries instead of publishing empty or undefined material keys.
...
- The checked-in crosswalk classifies 90 upstream public exports and 157 executable upstream tests with zero unclassified cases; Milestone 3 adds public root/configuration, Canvas, frame-loop, hook, selector, testing-helper, compiler-preset, bundler, and real-browser evidence to the existing host-driver and same-source R3F differential suite.
- Ray and pointer events begin in Milestone 4; asset loading, portals, full Canvas SSR/hydration, and XR/HMR hardening follow in later milestones.
