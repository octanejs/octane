...
Process-split Electron bindings: ./main registers ipcMain handlers, ./preload exposes Electron IPC and desktop helpers via contextBridge, and the renderer entry provides Octane hooks (useInvoke, useInvokeState, useIpcEvent, useNativeTheme, useWindowState) plus promise helpers for app/window/dialog/shell/clipboard/screen. Main-only modules (BrowserWindow construction, Menu, Tray, session, protocol) stay in electron/main.
