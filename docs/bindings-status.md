...
- Not ported yet (phased — docs/remix-router-port-plan.md): useBlocker/usePrompt/ScrollRestoration/useBeforeUnload/useViewTransitionState (E), static SSR + framework stubs + full export parity (F). Never: framework mode (needs @react-router/dev), RSC. tests/conformance/parity.test.ts pins the exact per-phase boundary.
