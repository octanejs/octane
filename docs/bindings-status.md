...
The central status table for the 50 `@octanejs/*` framework bindings.
...
| [`@octanejs/blocknote`](#octanejsblocknote) | `@blocknote/react@0.53.0` | Milestone 1 — useCreateBlockNote, BlockNoteViewRaw, useBlockNoteEditor, BlockNoteContext/useBlockNoteContext (+ full mechanical port scaffold). | Upstream exports BlockNoteViewRaw; BlockNoteView alias not yet added; TipTap/react-dom boundary files flagged CHECKPOINT in port-upstream.mjs | Not yet evaluated. | 2026-08-17 |
...
| [`@octanejs/interior`](#octanejsinterior) | `interior.dev@github.com/ddoemonn/interior` | 54 interior.dev micro-interaction components for Octane. | useReducedMotion provided by @octanejs/interior (src/hooks/reduced-motion.ts) rather than @octanejs/motion; Headless hooks extracted for CopyButton; bulk extraction script (scripts/extract-hooks.mjs) ready for remaining 51 components | Not yet evaluated. | 2026-08-17 |
...
| [`@octanejs/puck`](#octanejspuck) | `@measured/puck@0.20.2` | Full @measured/puck@0.20.2 core port — Puck editor, Render, DropZone, hooks, and config types. | Native onChange on controlled text inputs (ExternalInput search field) — upstream React synthetic onChange; Octane uses onInput for per-edit; use-debounce replaced with @octanejs/tanstack-pacer useDebouncer in DragDropContext | Not yet evaluated. | 2026-08-17 |
...
Milestone 1 — useCreateBlockNote, BlockNoteViewRaw, useBlockNoteEditor, BlockNoteContext/useBlockNoteContext (+ full mechanical port scaffold).

Known divergences:

- Upstream exports BlockNoteViewRaw; BlockNoteView alias not yet added.
- TipTap/react-dom boundary files flagged CHECKPOINT in port-upstream.mjs.

SSR / hydration: Not yet evaluated.

Scope/evidence last checked: 2026-08-17.

- Full mechanical port (~156 files) via scripts/port-upstream.mjs (2026-08-07).
- M1 export conformance test wired; editor mount blocked on @octanejs/tiptap EditorContent checkpoint.
- Dependencies: @blocknote/core (reuse), @octanejs/tiptap, @octanejs/floating-ui, @octanejs/tanstack-store.
...
[`packages/interior`](../packages/interior) `0.1.0` — ports `interior.dev@github.com/ddoemonn/interior`. Status data: [`packages/interior/status.json`](../packages/interior/status.json).

54 interior.dev micro-interaction components for Octane.

Known divergences:

- useReducedMotion provided by @octanejs/interior (src/hooks/reduced-motion.ts) rather than @octanejs/motion.
- Headless hooks extracted for CopyButton; bulk extraction script (scripts/extract-hooks.mjs) ready for remaining 51 components.

SSR / hydration: Not yet evaluated.

Scope/evidence last checked: 2026-08-17.

- CopyButton + useCopyToClipboard fully extracted with manual slot forwarding.
- scripts/extract-hooks.mjs automates hook extraction; multiline/lazy-init hook calls need manual slot review.
- scripts/port-upstream.mjs preserves src/hooks/ and copy-button.tsrx on re-port.
...
[`packages/puck`](../packages/puck) `0.1.0` — ports `@measured/puck@0.20.2`. Status data: [`packages/puck/status.json`](../packages/puck/status.json).

Full @measured/puck@0.20.2 core port — Puck editor, Render, DropZone, hooks, and config types.

Known divergences:

- Native onChange on controlled text inputs (ExternalInput search field) — upstream React synthetic onChange; Octane uses onInput for per-edit.
- use-debounce replaced with @octanejs/tanstack-pacer useDebouncer in DragDropContext.

SSR / hydration: Not yet evaluated.

Scope/evidence last checked: 2026-08-17.

- Source ported from puckeditor/puck packages/core via scripts/port-upstream.mjs.
- Render and full Puck editor shell mount pass conformance tests (2026-08-07).
- @dnd-kit/react and lucide-react rewired to @octanejs/dnd-kit and @octanejs/lucide; @dnd-kit/* upgraded to catalog 0.5.0.
- Manual hook slots declared for entire src/ tree.
...
[`packages/tauri`](../packages/tauri) `0.0.1` — ports `@tauri-apps/api@2.11.1`. Status data: [`packages/tauri/status.json`](../packages/tauri/status.json).
...
[`packages/valtio`](../packages/valtio) `0.1.1` — ports `valtio@2.3.2`. Status data: [`packages/valtio/status.json`](../packages/valtio/status.json).
