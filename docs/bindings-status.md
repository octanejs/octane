...
The central status table for the 35 `@octanejs/*` framework bindings.
...
| [`@octanejs/scan`](#octanejsscan) | `react-scan@0.5.7` | Phases 2–5 of 6 (docs/octane-scan-port-plan.md): the programmatic core — `scan`, `setOptions`, `getOptions`, `getReport`, `resetReport`, `onRender(Component, cb)`, the `log`/`onCommitStart`/`onCommitFinish` options — plus a faithful port of react-scan's `new-outlines` overlay (indigo `rgb(115,97,230)` 1px crisp stroke + faint interior fill, 45-frame linear fade, lerp easing toward re-measured rects, `getLabelText` grouping; pointer-transparent, verified in real Chromium). Plus a faithful port of react-scan's toolbar widget: the collapsed bar (inspect toggle, notifications bell, the `Outline Re-renders` power switch driving `enabled`, and a color-graded FPS meter) and the bell-expanded interaction inspector (react-scan's notifications panel) — a client-side interaction profiler that times and names each interaction (`Clicked X — Nms processing time`), lists a History, and ranks the components that rendered during it (Ranked/Overview/Prompts tabs + an Alerts audio chime). All plain shadow-DOM the profiler cannot see, upstream renders its UI in Preact rather than React for the same self-instrumentation reason. Plus `useScan` (slot-forwarding hook variant), the click-to-inspect inspector (hover outline with the component name + a locked panel showing identity, report counts, and the profiler’s actual schedule causes; Escape exits), a draggable toolbar that snaps to any of the four corners, and Fix/Explanation/Data LLM-prompt sub-tabs. Naming resolves through a shared element→component registry: because instances only register while the profiler is active, the package starts the profiler at import (before hydration) so every mounted component is nameable, seeding the registry from the profiler's event buffer for instances that mounted before the sink was listening. Live props/state viewing and the script-tag auto entry are not yet ported; lite/hookless scopes over-approximate their DOM range (see the plan’s follow-ups). | Contract port, not code port: a layered, multi-engine architecture (InspectionSource → Pipeline → Services → Plugins). react-scan's fiber-welded engine is replaced by a framework-agnostic `InspectionEvent` contract with the Octane profiler as one adapter; the overlay/toolbar/inspector are first-party plugins on public services; No fibers: callbacks receive an `InspectionEvent` (component identity, phase, schedule causes, timings, and a lazy `domNodes()` resolver) instead of a React `Fiber`; Enablement is a build property, not a runtime flag: the app must compile with `octane({ profile: true })` — octane's production compile strips the inspection channel entirely, so react-scan's `dangerouslyForceRunInProduction` has no equivalent; `getReport()` returns per-component aggregates keyed by compile-time component identity rather than fiber-keyed maps | Client-only by nature; every export is a safe no-op under SSR because the server produces no profiler events. | 2026-07-19 |
...
[`packages/apollo-client`](../packages/apollo-client) `0.1.6` — ports `@apollo/client@4.2.6`. Status data: [`packages/apollo-client/status.json`](../packages/apollo-client/status.json).
...
[`packages/aria`](../packages/aria) `0.0.5` — ports `react-aria@3.50.0`. Status data: [`packages/aria/status.json`](../packages/aria/status.json).
...
[`packages/base-ui`](../packages/base-ui) `0.1.9` — ports `@base-ui/react@1.6.0`. Status data: [`packages/base-ui/status.json`](../packages/base-ui/status.json).
...
[`packages/dexie`](../packages/dexie) `0.1.4` — ports `dexie-react-hooks@4.4.0`. Status data: [`packages/dexie/status.json`](../packages/dexie/status.json).
...
[`packages/dnd-kit`](../packages/dnd-kit) `0.1.6` — ports `@dnd-kit/react@0.5.0`. Status data: [`packages/dnd-kit/status.json`](../packages/dnd-kit/status.json).
...
[`packages/floating-ui`](../packages/floating-ui) `0.1.10` — ports `@floating-ui/react@0.27.19`. Status data: [`packages/floating-ui/status.json`](../packages/floating-ui/status.json).
...
[`packages/hook-form`](../packages/hook-form) `0.1.8` — ports `react-hook-form@7.81.0`. Status data: [`packages/hook-form/status.json`](../packages/hook-form/status.json).
...
[`packages/i18next`](../packages/i18next) `0.1.6` — ports `react-i18next@17.0.9`. Status data: [`packages/i18next/status.json`](../packages/i18next/status.json).
...
[`packages/jotai`](../packages/jotai) `0.1.8` — ports `jotai@2.20.1`. Status data: [`packages/jotai/status.json`](../packages/jotai/status.json).
...
[`packages/lexical`](../packages/lexical) `0.1.10` — ports `@lexical/react@0.46.0`. Status data: [`packages/lexical/status.json`](../packages/lexical/status.json).
...
[`packages/lucide`](../packages/lucide) `0.1.6` — ports `lucide-react@1.24.0`. Status data: [`packages/lucide/status.json`](../packages/lucide/status.json).
...
[`packages/mdx`](../packages/mdx) `0.1.8` — ports `@mdx-js/mdx@3.1.1`. Status data: [`packages/mdx/status.json`](../packages/mdx/status.json).
...
[`packages/motion`](../packages/motion) `0.1.10` — ports `motion@12.40.0`. Status data: [`packages/motion/status.json`](../packages/motion/status.json).
...
[`packages/radix`](../packages/radix) `0.1.10` — ports `radix-ui@1.6.1`. Status data: [`packages/radix/status.json`](../packages/radix/status.json).
...
[`packages/recharts`](../packages/recharts) `0.1.8` — ports `recharts@3.9.2`. Status data: [`packages/recharts/status.json`](../packages/recharts/status.json).
...
[`packages/redux`](../packages/redux) `0.1.8` — ports `react-redux@9.3.0`. Status data: [`packages/redux/status.json`](../packages/redux/status.json).
...
[`packages/redux-toolkit`](../packages/redux-toolkit) `0.1.6` — ports `@reduxjs/toolkit@2.12.0`. Status data: [`packages/redux-toolkit/status.json`](../packages/redux-toolkit/status.json).
...
[`packages/remix-router`](../packages/remix-router) `0.1.7` — ports `react-router@8.2.0`. Status data: [`packages/remix-router/status.json`](../packages/remix-router/status.json).
...
[`packages/scan`](../packages/scan) `0.1.0` — ports `react-scan@0.5.7`. Status data: [`packages/scan/status.json`](../packages/scan/status.json).

Phases 2–5 of 6 (docs/octane-scan-port-plan.md): the programmatic core — `scan`, `setOptions`, `getOptions`, `getReport`, `resetReport`, `onRender(Component, cb)`, the `log`/`onCommitStart`/`onCommitFinish` options — plus a faithful port of react-scan's `new-outlines` overlay (indigo `rgb(115,97,230)` 1px crisp stroke + faint interior fill, 45-frame linear fade, lerp easing toward re-measured rects, `getLabelText` grouping; pointer-transparent, verified in real Chromium). Plus a faithful port of react-scan's toolbar widget: the collapsed bar (inspect toggle, notifications bell, the `Outline Re-renders` power switch driving `enabled`, and a color-graded FPS meter) and the bell-expanded interaction inspector (react-scan's notifications panel) — a client-side interaction profiler that times and names each interaction (`Clicked X — Nms processing time`), lists a History, and ranks the components that rendered during it (Ranked/Overview/Prompts tabs + an Alerts audio chime). All plain shadow-DOM the profiler cannot see, upstream renders its UI in Preact rather than React for the same self-instrumentation reason. Plus `useScan` (slot-forwarding hook variant), the click-to-inspect inspector (hover outline with the component name + a locked panel showing identity, report counts, and the profiler’s actual schedule causes; Escape exits), a draggable toolbar that snaps to any of the four corners, and Fix/Explanation/Data LLM-prompt sub-tabs. Naming resolves through a shared element→component registry: because instances only register while the profiler is active, the package starts the profiler at import (before hydration) so every mounted component is nameable, seeding the registry from the profiler's event buffer for instances that mounted before the sink was listening. Live props/state viewing and the script-tag auto entry are not yet ported; lite/hookless scopes over-approximate their DOM range (see the plan’s follow-ups).

Known divergences:

- Contract port, not code port: a layered, multi-engine architecture (InspectionSource → Pipeline → Services → Plugins). react-scan's fiber-welded engine is replaced by a framework-agnostic `InspectionEvent` contract with the Octane profiler as one adapter; the overlay/toolbar/inspector are first-party plugins on public services.
- No fibers: callbacks receive an `InspectionEvent` (component identity, phase, schedule causes, timings, and a lazy `domNodes()` resolver) instead of a React `Fiber`.
- Enablement is a build property, not a runtime flag: the app must compile with `octane({ profile: true })` — octane's production compile strips the inspection channel entirely, so react-scan's `dangerouslyForceRunInProduction` has no equivalent.
- `getReport()` returns per-component aggregates keyed by compile-time component identity rather than fiber-keyed maps.

SSR / hydration: Client-only by nature; every export is a safe no-op under SSR because the server produces no profiler events.

Scope/evidence last checked: 2026-07-19.

## @octanejs/sonner

[`packages/sonner`](../packages/sonner) `0.1.6` — ports `sonner@2.0.7`. Status data: [`packages/sonner/status.json`](../packages/sonner/status.json).
...
[`packages/styled-components`](../packages/styled-components) `0.1.3` — ports `styled-components@6.4.3`. Status data: [`packages/styled-components/status.json`](../packages/styled-components/status.json).
...
[`packages/stylex`](../packages/stylex) `0.1.10` — ports `@stylexjs/stylex@0.19.0`. Status data: [`packages/stylex/status.json`](../packages/stylex/status.json).
...
[`packages/tanstack-ai`](../packages/tanstack-ai) `0.0.5` — ports `@tanstack/ai-react@0.17.0`. Status data: [`packages/tanstack-ai/status.json`](../packages/tanstack-ai/status.json).
...
[`packages/tanstack-devtools`](../packages/tanstack-devtools) `0.0.5` — ports `@tanstack/react-devtools@0.10.7`. Status data: [`packages/tanstack-devtools/status.json`](../packages/tanstack-devtools/status.json).
...
[`packages/tanstack-form`](../packages/tanstack-form) `0.0.5` — ports `@tanstack/react-form@1.33.2`. Status data: [`packages/tanstack-form/status.json`](../packages/tanstack-form/status.json).
...
[`packages/tanstack-query`](../packages/tanstack-query) `0.1.10` — ports `@tanstack/react-query@5.101.0`. Status data: [`packages/tanstack-query/status.json`](../packages/tanstack-query/status.json).
...
[`packages/tanstack-router`](../packages/tanstack-router) `0.1.10` — ports `@tanstack/react-router@1.170.16`. Status data: [`packages/tanstack-router/status.json`](../packages/tanstack-router/status.json).
...
[`packages/tanstack-store`](../packages/tanstack-store) `0.0.5` — ports `@tanstack/react-store@0.11.0`. Status data: [`packages/tanstack-store/status.json`](../packages/tanstack-store/status.json).
...
[`packages/tanstack-table`](../packages/tanstack-table) `0.1.8` — ports `@tanstack/react-table@8.21.3`. Status data: [`packages/tanstack-table/status.json`](../packages/tanstack-table/status.json).
...
[`packages/tanstack-virtual`](../packages/tanstack-virtual) `0.1.8` — ports `@tanstack/react-virtual@3.14.5`. Status data: [`packages/tanstack-virtual/status.json`](../packages/tanstack-virtual/status.json).
...
[`packages/testing-library`](../packages/testing-library) `0.1.8` — ports `@testing-library/react` (unpinned). Status data: [`packages/testing-library/status.json`](../packages/testing-library/status.json).
...
[`packages/three`](../packages/three) `0.1.4` — ports `@react-three/fiber@9.6.1 (2a528745)`. Status data: [`packages/three/status.json`](../packages/three/status.json).
...
[`packages/tiptap`](../packages/tiptap) `0.0.5` — ports `@tiptap/react@3.28.0`. Status data: [`packages/tiptap/status.json`](../packages/tiptap/status.json).
...
[`packages/visx`](../packages/visx) `0.1.5` — ports `@visx/visx@4.0.0 + master@485c035`. Status data: [`packages/visx/status.json`](../packages/visx/status.json).
...
[`packages/zustand`](../packages/zustand) `0.1.10` — ports `zustand@5.0.14`. Status data: [`packages/zustand/status.json`](../packages/zustand/status.json).
