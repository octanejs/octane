...
| [`@octanejs/grab`](#octanejsgrab) | `react-grab@0.2.0` | react-grab 0.2.0 element-grab overlay ported to Octane: init lifecycle, shadow-DOM overlay, selection/copy flow, context menu, plugins, global API, and the complete option/state/theme type surface. | Component ancestry resolves through Octane scopes instead of React fibers; src/bippy/ is a clean-room reimplementation over Octane instrumentation; options.freezeReactUpdates freezes Octane renderer updates; the option name is retained for API parity | Server rendering is covered by tests/ssr/isomorphic-import.test.ts: the package imports isomorphically and init stays inert without a DOM. | 2026-09-16 |
...
react-grab 0.2.0 element-grab overlay ported to Octane: init lifecycle, shadow-DOM overlay, selection/copy flow, context menu, plugins, global API, and the complete option/state/theme type surface.

Known divergences:

- Component ancestry resolves through Octane scopes instead of React fibers; src/bippy/ is a clean-room reimplementation over Octane instrumentation.
- options.freezeReactUpdates freezes Octane renderer updates; the option name is retained for API parity.

SSR / hydration: Server rendering is covered by tests/ssr/isomorphic-import.test.ts: the package imports isomorphically and init stays inert without a DOM.

Scope/evidence last checked: 2026-09-16.
...
[`packages/recharts`](../packages/recharts) `0.1.52` — ports `recharts@3.9.2`. Status data: [`packages/recharts/status.json`](../packages/recharts/status.json).
...
[`packages/tanstack-router`](../packages/tanstack-router) `0.1.54` — ports `@tanstack/react-router@1.170.18`. Status data: [`packages/tanstack-router/status.json`](../packages/tanstack-router/status.json).
