...
- Calendar’s day-picker `components` overrides leave the inline object literal (three module-level, one memoized on `locale`), so a re-render updates the existing day buttons instead of replacing 35 component types; the markup is byte-identical to upstream. CalendarDayButton also attaches the ref upstream’s base flavor creates and leaves unattached, without which keyboard focus never moves once the override replaces day-picker’s own DayButton.
