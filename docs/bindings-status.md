...
| --- | --- | --- | --- | --- | --- |
| [`@octanejs/alien-signals`](#octanejsalien-signals) | `react-alien-signals@0.4.0` | Complete react-alien-signals 0.4.0 hook and helper surface over the reused alien-signals 3.2.1 core, including batching, manual triggers, selectors, deferred reads and phase effects. | Direct signal setters retain functional updater support; hook setters store function-valued results without invoking them twice; The scope stop controller permanently remembers cancellation, including before commit. Upstream 0.4.0 also starts scopes after commit but does not retain pre-commit cancellation; createEffect retains compatibility with callbacks returning incidental non-function values; only function returns own cleanup | Supported and hydration-tested. Reads use the same server snapshot; effects and scopes begin only after client commit and stop on unmount. | 2026-09-13 |
...
| [`@octanejs/colorful`](#octanejscolorful) | `react-colorful@5.8.1` | Complete against the published react-colorful 5.8.1 root runtime and type surface: all 14 picker variants, HexColorInput, setNonce, six public color types, controlled updates, mouse/touch/keyboard input, commit callbacks, ARIA state, and automatic closest-root styling. | React synthetic event attribute types are represented by Octane native DOM event attributes; observable DOM events and callback values retain the upstream contract | All pickers and HexColorInput render deterministically without browser globals, hydrate by adopting existing nodes, install styles only on the client, and become interactive after hydration. | 2026-09-13 |
...
| [`@octanejs/hook-form`](#octanejshook-form) | `react-hook-form@7.88.0` | Source port of react-hook-form 7.88.0 at 28334faa6105b0475060592c6bf27dca87e3941e. Includes ErrorMessage, FieldArray, FormState and its FormStateSubscribe compatibility alias, getErrors, function form actions, native File/FormData, validation, schema resolvers, subscription and reconnect behavior. Retains all upstream runtime and type registrations with 1,364 pristine Jest executions, 1,355 adapted DOM/server cases, and an explicit 90-case browser crosswalk (87 native cases; three React MUI integrations inapplicable). | `register()` returns `onInput` (octane's native per-keystroke event) instead of React's synthetic `onChange`; mode names and `register` option keys keep the upstream spelling; The structured parity ledger records native no-op input delivery, microtask batching, duplicate resolver notification, async act flush, reset render-count, and eager `Object.is` bailout differences with executable case identities, consumer impact, and migration guidance; the adapted DOM/server suite contains no skipped or expected-failure cases; Three original browser registrations mount React MUI components and assert their portal DOM. No Octane MUI binding exists; those external integrations remain inapplicable and visible in the full case crosswalk | Supported: adapted server markup checks plus hydration node adoption, server-effect exclusion, client values, and teardown. | 2026-09-14 |
...
| [`@octanejs/intersection-observer`](#octanejsintersection-observer) | `react-intersection-observer@11.0.1` | Complete react-intersection-observer 11.0.1 public hooks, component, observer and test-utils surface. Ref cleanup, generic effect targets and all named public types are preserved, with strict types and unchanged React unit/SSR/browser oracles. | Public node and prop types use Octane structural types and do not import React; InView is implemented as a function component while preserving the upstream render-prop and wrapper behavior; In TSRX, an InView render function must be supplied through the children prop; nested children compile to an opaque render block; Unsupported false fallback (`fallbackInView={false}` / `defaultFallbackInView(false)`) does not emit `onChange(false)` on mount under Octane (`intersection-observer-initial-false-onchange`): treat the default hidden state as authoritative rather than waiting for that callback; Unsupported IntersectionObserver without a fallback surfaces as a ref-commit `console.error` under Octane rather than a synchronous mount throw (`intersection-observer-unsupported-mount-error-surface`); wrap with an error boundary/`tryBlock` or supply `fallbackInView`/`defaultFallbackInView` instead of try/catch around render; `@octanejs/intersection-observer/test-utils` does not auto-register Vitest/Jest beforeEach/afterEach (`intersection-observer-test-utils-manual-setup`); call `setupIntersectionMocking`/`resetIntersectionMocking` in the test setup file; Ref callbacks return cleanup functions on every supported Octane version; the private React-version detection table is inapplicable; React Profiler is not an Octane API. Adapted performance cases retain their component render and observer-construction counts | Supported. initialInView controls deterministic server output; observation starts when the client ref is committed. Hydration adopts server nodes and unmount disconnects observation. | 2026-09-13 |
...
| [`@octanejs/mobx`](#octanejsmobx) | `mobx-react-lite@5.0.3` | MobX 7 core re-exports plus observer, Observer, useLocalObservable, static-rendering controls, finalization utilities, and the retained useObserver/useStaticRendering aliases. Generic props, typed Maps, component statics, and native refs are preserved. | React class components and the legacy mobx-react Provider/inject APIs are not included; forwardRef compatibility options are omitted because Octane uses refs as props; React-specific batching, prop-types validation, React DevTools integration, and useDebugValue output are omitted | enableStaticRendering(true) renders observed components without creating a Reaction or retaining observable subscriptions. | 2026-09-14 |
...
| [`@octanejs/motion`](#octanejsmotion) | `motion@13.2.0` | Core surface: `motion.<tag>` (animate, gestures, variants with propagation/stagger, drag, layout basics), `AnimatePresence`, `MotionConfig`, live `useReducedMotion`, reduced-motion enforcement, `LayoutGroup` layoutId namespaces, `LazyMotion` with `domAnimation`/`domMax`, the `m` proxy and complete `./react-m` named host entry, plus the motion-value hooks (`useMotionValue`, `useScroll`, `useTransform`, `useSpring`, `useAnimate`, `useMotionValueEvent`); motion-dom's animation engine and gesture primitives are reused verbatim. | Exit animations run via cleanup-before-detach instead of React's deferred-deletion machinery; `layout`/`layoutId` use single-element FLIP, not the full projection tree; An `initial` target without an `animate` target does not materialize inline initial styles; set the starting style explicitly or provide an animation target | Dedicated Node-mode tests verify deterministic hook initialization without DOM effects; Chromium verifies hydration adopts that output and unmounts cleanly. Motion host-component SSR remains unsupported. | 2026-09-13 |
...
| [`@octanejs/resizable-panels`](#octanejsresizable-panels) | `react-resizable-panels@4.12.4` | Group, Panel, Separator, persistence hooks, refs, imperative APIs, layout constraints, pointer and keyboard interaction, ARIA, cursor handling, and ResizeObserver behavior are implemented against react-resizable-panels 4.12.4. | useId(undefined) fallback cannot reproduce React's mocked ':r123:' seam; adapted upstream case asserts a non-empty Octane id and is recorded as react-resizable-panels-useId-fallback; The upstream React Server Components demo scenario is inapplicable because Octane does not support RSC. Client startup and SSR hydration persistence have native conformance coverage | Deterministic server rendering and live hydration adoption are covered by dedicated executable projects. | 2026-09-13 |
...
| [`@octanejs/thinking-orbs`](#octanejsthinking-orbs) | `thinking-orbs@0.3.1` | ThinkingOrb component, resolvePreset, MODE_DRAWS, and public types — nine animation states, two tuned size presets, auto/dark/light theme, reduced-motion static frame. | none known | Canvas is client-only; server render emits the canvas element without animation. | 2026-09-13 |
...
[`packages/alien-signals`](../packages/alien-signals) `0.0.18` — ports `react-alien-signals@0.4.0`. Status data: [`packages/alien-signals/status.json`](../packages/alien-signals/status.json).

Complete react-alien-signals 0.4.0 hook and helper surface over the reused alien-signals 3.2.1 core, including batching, manual triggers, selectors, deferred reads and phase effects.

Known divergences:

- Direct signal setters retain functional updater support; hook setters store function-valued results without invoking them twice.
- The scope stop controller permanently remembers cancellation, including before commit. Upstream 0.4.0 also starts scopes after commit but does not retain pre-commit cancellation.
- createEffect retains compatibility with callbacks returning incidental non-function values; only function returns own cleanup.
...
- Reuses the inspected framework-neutral alien-signals 3.2.1 package. React and its declarations are test-only.
- All 36 original upstream cases and all 90 accepted typecheck API call occurrences are retained with one-for-one native adaptations.
- Phase hooks require a referentially stable signal list, matching upstream.
...
[`packages/colorful`](../packages/colorful) `0.0.18` — ports `react-colorful@5.8.1`. Status data: [`packages/colorful/status.json`](../packages/colorful/status.json).

Complete against the published react-colorful 5.8.1 root runtime and type surface: all 14 picker variants, HexColorInput, setNonce, six public color types, controlled updates, mouse/touch/keyboard input, commit callbacks, ARIA state, and automatic closest-root styling.
...
[`packages/electron`](../packages/electron) `0.0.30` — ports `electron@44.3.0`. Status data: [`packages/electron/status.json`](../packages/electron/status.json).
...
[`packages/hook-form`](../packages/hook-form) `0.1.49` — ports `react-hook-form@7.88.0`. Status data: [`packages/hook-form/status.json`](../packages/hook-form/status.json).

Source port of react-hook-form 7.88.0 at 28334faa6105b0475060592c6bf27dca87e3941e. Includes ErrorMessage, FieldArray, FormState and its FormStateSubscribe compatibility alias, getErrors, function form actions, native File/FormData, validation, schema resolvers, subscription and reconnect behavior. Retains all upstream runtime and type registrations with 1,364 pristine Jest executions, 1,355 adapted DOM/server cases, and an explicit 90-case browser crosswalk (87 native cases; three React MUI integrations inapplicable).
...
- The structured parity ledger records native no-op input delivery, microtask batching, duplicate resolver notification, async act flush, reset render-count, and eager `Object.is` bailout differences with executable case identities, consumer impact, and migration guidance; the adapted DOM/server suite contains no skipped or expected-failure cases.
- Three original browser registrations mount React MUI components and assert their portal DOM. No Octane MUI binding exists; those external integrations remain inapplicable and visible in the full case crosswalk.

SSR / hydration: Supported: adapted server markup checks plus hydration node adoption, server-effect exclusion, client values, and teardown.

Scope/evidence last checked: 2026-09-14.
...
[`packages/i18next`](../packages/i18next) `0.1.47` — ports `react-i18next@17.0.14`. Status data: [`packages/i18next/status.json`](../packages/i18next/status.json).
...
[`packages/intersection-observer`](../packages/intersection-observer) `0.0.18` — ports `react-intersection-observer@11.0.1`. Status data: [`packages/intersection-observer/status.json`](../packages/intersection-observer/status.json).

Complete react-intersection-observer 11.0.1 public hooks, component, observer and test-utils surface. Ref cleanup, generic effect targets and all named public types are preserved, with strict types and unchanged React unit/SSR/browser oracles.
...
- Unsupported IntersectionObserver without a fallback surfaces as a ref-commit `console.error` under Octane rather than a synchronous mount throw (`intersection-observer-unsupported-mount-error-surface`); wrap with an error boundary/`tryBlock` or supply `fallbackInView`/`defaultFallbackInView` instead of try/catch around render.
...
- Ref callbacks return cleanup functions on every supported Octane version; the private React-version detection table is inapplicable.
- React Profiler is not an Octane API. Adapted performance cases retain their component render and observer-construction counts.

SSR / hydration: Supported. initialInView controls deterministic server output; observation starts when the client ref is committed. Hydration adopts server nodes and unmount disconnects observation.
...
[`packages/mobx`](../packages/mobx) `0.1.33` — ports `mobx-react-lite@5.0.3`. Status data: [`packages/mobx/status.json`](../packages/mobx/status.json).

MobX 7 core re-exports plus observer, Observer, useLocalObservable, static-rendering controls, finalization utilities, and the retained useObserver/useStaticRendering aliases. Generic props, typed Maps, component statics, and native refs are preserved.
...
- Complete immutable and adapted runtime suites: 90 active cases and two pre-existing upstream skips.
- Strict original/adapted source tests and public declarations, native callback/retention regressions, Chromium events and focus, SSR/hydration, and unmount cleanup.
- The dependency now uses MobX 7. Applications importing mobx directly should use a compatible MobX 7 version.
...
[`packages/motion`](../packages/motion) `0.1.52` — ports `motion@13.2.0`. Status data: [`packages/motion/status.json`](../packages/motion/status.json).
...
SSR / hydration: Dedicated Node-mode tests verify deterministic hook initialization without DOM effects; Chromium verifies hydration adopts that output and unmounts cleanly. Motion host-component SSR remains unsupported.
...
- motion 13.2.0 re-exports the same framer-motion release. Native MotionConfig supports scoped isValidProp inheritance and overrides.
- The complete pristine oracle passes 870 cases; five upstream cases are adapted one-for-one. Other native behavior is covered by separate conformance and differential tests; this is not a complete React Motion port.
- Native hosts animate DOM targets without writing back to external style MotionValues; component onUpdate/onAnimationStart and gesture callback/priority APIs remain unsupported.
- useTransform replacement input/range identities and Motion 13.2.0 unit-string springs with skipInitialAnimation remain limitations. See UPSTREAM.md and the complete case crosswalk.
...
[`packages/resizable-panels`](../packages/resizable-panels) `0.0.10` — ports `react-resizable-panels@4.12.4`. Status data: [`packages/resizable-panels/status.json`](../packages/resizable-panels/status.json).

Group, Panel, Separator, persistence hooks, refs, imperative APIs, layout constraints, pointer and keyboard interaction, ARIA, cursor handling, and ResizeObserver behavior are implemented against react-resizable-panels 4.12.4.
...
- The upstream React Server Components demo scenario is inapplicable because Octane does not support RSC. Client startup and SSR hydration persistence have native conformance coverage.
...
[`packages/sanity-loader`](../packages/sanity-loader) `0.1.2` — ports `@sanity/react-loader@2.2.2`. Status data: [`packages/sanity-loader/status.json`](../packages/sanity-loader/status.json).
...
- The 2.2.2 refresh changes the core dependency and React test oracle; upstream React loader source is unchanged from 2.2.1.
...
[`packages/thinking-orbs`](../packages/thinking-orbs) `0.1.2` — ports `thinking-orbs@0.3.1`. Status data: [`packages/thinking-orbs/status.json`](../packages/thinking-orbs/status.json).
