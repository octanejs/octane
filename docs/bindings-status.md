...
| [`@octanejs/animejs`](#octanejsanimejs) | `animejs@4.5.0` | Anime.js root and all published runtime subpaths are re-exported unchanged; useAnimeScope adds Octane scoped lifecycle ownership. The package.json entry exposes binding metadata. | `useAnimeScope` is an Octane-native lifecycle helper; it is not an upstream Anime.js API; The Three adapter mutates raw objects while `@octanejs/three` retains frame-loop ownership; demand and never loops require explicit invalidation or advancement; The package.json subpath reports the Octane binding identity/version rather than the upstream dependency metadata | Supported. The hook returns inert refs during server rendering and creates no Anime.js scope until its client effect runs. | 2026-09-08 |
...
Anime.js root and all published runtime subpaths are re-exported unchanged; useAnimeScope adds Octane scoped lifecycle ownership. The package.json entry exposes binding metadata.
...
- The package.json subpath reports the Octane binding identity/version rather than the upstream dependency metadata.
...
Scope/evidence last checked: 2026-09-08.
...
- 2026-09-08: all runtime subpaths and strict namespace types pass focused validation; full machine evidence is incomplete because pristine upstream JavaScript type tests fail the required strict compiler mode.
...
[`packages/aria`](../packages/aria) `0.0.46` — ports `react-aria@3.51.0`. Status data: [`packages/aria/status.json`](../packages/aria/status.json).
...
[`packages/shadcn`](../packages/shadcn) `0.0.38` — ports `shadcn-ui/ui (component registry)@7c9eaba1c0a6404c990c144a654792e3313c650d + shadcn@4.21.0`. Status data: [`packages/shadcn/status.json`](../packages/shadcn/status.json).
