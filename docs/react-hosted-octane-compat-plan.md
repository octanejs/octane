...
of `.tsx`/`.ts`/`.js`: Octane deliberately compiles project `.tsx`, so the
extension alone cannot decide those files. `.tsrx` needs no decision — in an
Octane pipeline nothing else compiles the syntax, so the extension itself is
the ownership.
...
- Project `.tsrx`: always Octane's. A project routing part of its `.tsrx`
  ...
  over an octane pragma (with a conflict warning).
- Installed/linked packages: exempt; the package-manifest `usesOctane` rule
  stays the per-package decision, so bindings need no pragmas.

The mixed pipeline is pinned end-to-end by
`tests/react-hosted/react-jsx-pipeline.test.ts`: a host module compiled by the
real automatic-runtime JSX transform (`react/jsx-runtime` emit) renders an
Octane island through `OctaneCompat`, while the Octane compiler provably
passes the host `.tsx` through untouched.
