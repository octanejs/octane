...
| DOM client/shared | Unknown-property and host-value diagnostics from `ReactDOMUnknownPropertyHook.js` and `ReactDOMComponent.js` | **Partial** | `conformance/dom-attributes.test.ts`, `conformance/aria-diagnostics.test.ts`, `conformance/server-integration-attributes-wave4d.test.ts`, and `conformance/ssr-attribute-diagnostics.test.ts`: dynamic and static true/false non-boolean values, NaN, plain objects, functions, symbols, curated casing, reserved bare `aria`, lowercase event-like functions, and module-global normalized-host-name dedupe | Add boolean-string/empty-string, `innerHTML`, string-valued `is`, unknown camelCase, singular/plural non-ARIA invalid-prop aggregation, remaining event-name, URL, and coercion variants. React keys dedupe by authored prop name before aliasing; function/symbol aggregation and some custom-element/object-coercion behavior also remain adapted rather than exact. |
...
| DOM events | Invalid listeners from `getListener.js`, `ReactDOMEventListener.js`, and `InvalidEventListeners-test.js` | **Partial, adapted propagation** | `conformance/invalid-listeners.test.ts`, `event-callback-codegen.test.ts`, and `production-error-bundle.test.ts`: exact bubble/capture prop/type render warnings (including the `false` guidance), nullish silence, dispatched errors, valid-to-invalid and invalid-to-valid replacement, ancestor continuation, and production code 46 | Production uses an adapted native-event label instead of retaining a JSX prop string. Octane isolates invalid native listeners so an ancestor continues, unlike React's propagation behavior. Non-delegated, throwing-listener, and custom-event variants remain pending. |
| DOM forms | Controlled values and actions from `ReactControlledValuePropTypes.js`, `ReactDOMInput.js`, `ReactDOMSelect.js`, `ReactDOMTextarea.js`, `ReactDOMComponent.js`, and `ReactDOMFormActions.js` | **Partial + adapted native events** | `conformance/controlled-{input,select,textarea,restore}.test.ts`, `differential/controlled-forms.test.ts`; controlled switches, falsy controlled values, missing native `onInput`, null/read-only/disabled/uncontrolled negatives, and multiple-select shape have coverage | Finish `value`/`defaultValue`, `checked`/`defaultChecked`, option/textarea child variants, form-action prop conflicts, and SSR warnings. Guidance uses native `onInput`/`change`, never React's synthetic `onChange`. |
...
| DOM SSR + resources | Host serialization, stream lifecycle, form/resource hints, and abort diagnostics from `ReactFizzConfigDOM.js`, `ReactFizzServer.js`, and `ReactDOMResourceValidation.js` | **Partial** | `conformance/float-resource-diagnostics.test.ts`, `conformance/fizz-streaming.test.ts`, `streaming-ssr.test.ts`, `resource-hints.test.ts`, and `conformance/ssr-attribute-diagnostics.test.ts`; production codes 30–45 | Invalid precedence stylesheet props, missing/empty `href`, `disabled={false}`, incompatible stylesheet-preload/inline-style collisions, and whitespace-bearing server style resource keys now have client/server evidence. Remaining preload/preinit prop conflicts, destination edge cases, and source context still need case-level audits. User abort reasons stay opaque and are never recoded. |
...
- capture/replacement listener variants and controlled-form falsy-value,
  read-only, disabled, null, and uncontrolled negative controls;
- Float stylesheet resource declassification, inline-style preload collisions,
  and server style-resource identity diagnostics.

Continue with the remaining high-frequency unknown-property, URL, and
controlled/default form conflicts before low-frequency private or legacy cases.
