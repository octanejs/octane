...
| DOM client/shared | Unknown-property and host-value diagnostics from `ReactDOMUnknownPropertyHook.js` and `ReactDOMComponent.js` | **Implemented, bounded surface** | `conformance/unknown-property-diagnostics.test.ts`, `conformance/dom-attributes.test.ts`, `conformance/server-integration-attributes-wave4d.test.ts`, and `conformance/ssr-attribute-diagnostics.test.ts`: static/dynamic boolean strings, empty resource URLs, raw `innerHTML`, string/non-string customized-built-in `is`, unknown camelCase and spelling suggestions, singular/plural invalid-value grouping, event-shaped strings, authored-name dedupe, failed/single coercion, custom-element/SVG negatives, hydration, buffered/static/streaming SSR, and production stripping | Octane intentionally retains its raw custom-element attribute model, native-event guidance, and supported `class`/`className` composition. Customized built-ins with string `is` bypass generic host-property warnings, matching the audited React surface. |
| DOM client/shared | ARIA name and casing diagnostics from `ReactDOMInvalidARIAHook.js` | **Implemented, bounded surface** | `conformance/aria-diagnostics.test.ts`: recognized ARIA 1.3 names, invalid lowercase and camelCase names, spelling suggestions, singular/plural grouping, static/dynamic/spread hosts, custom-element negatives, SVG, module-lifetime dedupe, client hydration, buffered/static/streaming SSR, and production silence | ARIA spelling and aggregation are audited across the supported host surfaces. The bare `aria` attribute uses Octane-owned guidance; custom HTML elements intentionally retain raw attribute semantics. |
| DOM client + SSR | Style name/value/coercion diagnostics from `CSSPropertyOperations.js` and `ReactFizzConfigDOM.js` | **Implemented + divergent authoring** | `conformance/css-diagnostics.test.ts`, `conformance/css-properties.test.ts`, and `production-error-bundle.test.ts`: vendor-prefix casing, trailing semicolons, NaN/Infinity, failed coercion, renderer-scoped dedupe, custom-property negatives, dynamic declarations, fully static literals, mixed static/dynamic declarations, single getter/value coercion, paired client/server hydration adoption, all public SSR renderers, and production fast-path stripping | Octane deliberately supports kebab-case CSS names and complete `cssText` strings, so React's warnings for those supported authoring forms are not applicable. |
| DOM events | Invalid listeners from `getListener.js`, `ReactDOMEventListener.js`, and `InvalidEventListeners-test.js` | **Implemented + adapted native propagation** | `conformance/invalid-listeners.test.ts`, `conformance/dom-attributes.test.ts`, `event-callback-codegen.test.ts`, and `production-error-bundle.test.ts`: bubble/capture and non-bubbling scroll/load listener diagnostics, `false` guidance, nullish silence, exact uncaught-error identity, throwing-listener ancestor continuation, valid/invalid replacement, uppercase/dashed custom events, native custom-event capture/removal/replacement, and production code 46 | Production uses an adapted native-event label instead of retaining a JSX prop string. Octane guards native listener invocations independently, so an ancestor continues after a throwing or invalid descendant listener. Non-function lowercase custom-element event props deliberately retain raw attribute semantics. |
| DOM forms | Controlled values and actions from `ReactControlledValuePropTypes.js`, `ReactDOMInput.js`, `ReactDOMSelect.js`, `ReactDOMTextarea.js`, `ReactDOMComponent.js`, and `ReactDOMFormActions.js` | **Implemented + adapted native events** | `conformance/form-diagnostics.test.ts`, `conformance/controlled-{input,select,textarea,restore}.test.ts`, and `differential/controlled-forms.test.ts`: input/checkbox/radio/textarea/select controlled/default conflicts, final spread writers, single getters, option `selected` and complex children, textarea children, function form/submitter action conflicts, missing-handler negatives, warning dedupe, hydration, descriptors, buffered/static/streaming SSR, and production silence | Guidance uses Octane's native `onInput`/`change` contract, never React's synthetic `onChange`. Function action diagnostics preserve the existing supported submission behavior and final-writer semantics. |
| DOM client + hydration + SSR | Nesting and hydration diagnostics from `validateDOMNesting.js`, `ReactFiberHydrationContext.js`, and `ReactFiberHydrationDiffs.js` | **Implemented, bounded surface** | `ssr-invalid-nesting.test.ts`, `conformance/hydration-mismatch.test.ts`, `hydration/recoverable-callback.test.ts`, and `hydration/mismatch-{value,structural}.test.ts`: client/server paragraph/anchor/form/list parser scopes, table/row/select/option restrictions, static/dynamic text and whitespace, `<tbody>` remediation, SVG/`foreignObject`/MathML/HTML integration, component/source context, root ancestry, repeat-render dedupe, escaped-entity mismatch recovery, early updates, cross-root and throwing recoverable callbacks, and production silence | Octane roots intentionally require an `Element`; React's whole-`Document` singleton-root nesting exemption is not applicable. `suppressHydrationWarning` and recoverable error routing retain Octane's existing public semantics. |
...
| DOM SSR + resources | Host serialization, stream lifecycle, form/resource hints, and abort diagnostics from `ReactFizzConfigDOM.js`, `ReactFizzServer.js`, `ReactDOMFloat.js`, and `ReactDOMResourceValidation.js` | **Implemented, bounded surface** | `conformance/resource-prop-diagnostics.test.ts`, `conformance/float-resource-diagnostics.test.ts`, `conformance/fizz-streaming.test.ts`, `streaming-ssr.test.ts`, `resource-hints.test.ts`, and `conformance/ssr-attribute-diagnostics.test.ts`; production codes 30–45 | Client/server diagnostics cover combined invalid `preload` href/options, `preinit` destination/object conflicts, optional module hint options/destinations, `preconnect` CORS type, unsupported DNS-prefetch options, valid future module-preload destinations, precedence-link declassification, inline-style collisions, server style resource keys, and production warning stripping. User abort reasons stay opaque and are never recoded. |
...
- authored-name warning dedupe and production stripping across client,
  hydration, buffered/static SSR, streaming SSR, spreads, and hoisted head
  output;
- complete supported ARIA spelling/name diagnostics, including ARIA 1.3,
  grouped unknown names, custom-element negatives, hydration, streaming, and
  production silence;
- vendor-prefix, trailing-semicolon, NaN/Infinity, and failed-coercion CSS
  diagnostics for static, dynamic, and mixed style declarations across client,
  matching hydration, and SSR, while preserving single user-value coercion,
  Octane's supported kebab-case keys, and full `cssText` strings;
- bubble/capture, scroll/load, throwing, custom-event capture/removal, and
  listener replacement variants, with Octane-native ancestor continuation;
- controlled/default conflicts across every supported form host, option and
  textarea children, function action/submitter prop conflicts, all server
  renderers, and falsy/read-only/disabled/null/uncontrolled negative controls;
- client/server HTML parser-scope, table text/whitespace, SVG/MathML integration,
  component/root ancestry, hydration mismatch, and recoverable-error diagnostics;
- complete public client/server resource-hint argument, option, destination,
  CORS, and DNS-conflict diagnostics;
- Float stylesheet resource declassification, inline-style preload collisions,
  and server style-resource identity diagnostics.

The five issue #744 DOM authoring families now have bounded executable coverage.
Unrelated low-frequency private, legacy, reconciler, and root-boundary audits
remain separate diagnostic-plan workstreams.
